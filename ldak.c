...
#define MKL 0	//1 to compile with mkl, 0 to compile without mkl, 2 to compile with openblas
...
if(mode==102||mode==103||mode==104||mode==112||mode==113||mode==137||mode==138||mode==139||mode==140||mode==192||mode==193||mode==194)
{
...
if((mode==105&&reduce==1)||mode==112||mode==114||(mode==127&&reduce==1)||(mode==128&&reduce==1)||mode==136||(mode==141&&reduce==1)||mode==151||mode==152||mode==153||mode==154||mode==159||mode==197)	//can reduce
{
...
{read_respfile(respfile, resp, num_samples_use, ids3, num_resps_use, keepresps, num_resps, respcounts, respinds, missingvalue, (mode==130||mode==132||prev!=-9999||dichot==1), pad);}
else	//make a fake phenotype (must have num_resps_use=1, mode 115, 121, 122, 125, 138 or 172)
{
...
if(num_fixed>0)	//using covar - modes 121/122/123/124/125/126/127/128/130/131/132/133/138/145/151/152/153/154/169/170/172/173/175/194
{
...
he_reg(num_samples_use, num_subs, num_covars, num_envs, num_tops, num_kins, num_regs, subindex, Y, Z, Mkins_single, kintraces, kinsums, X, Xtotal, Xstarts, Xends, Xsums, prev, respcounts[m], discenv, oversfile, ssums, num_blocks, tol, maxiter, memsave, kinstems, ids3, filename, (mode==124), trun);
}	//end of m loop
...
multi_reml(num_samples_use, num_covars, num_envs, num_tops, num_kins, num_regs, Y, Z, NULL, Mkins_single, kintraces, kinsums, NULL, 0, NULL, NULL, NULL, prev, respcounts[m], hersfile, hestart, 2, NULL, NULL, num_vects, ldlt, 0, oversfile, ssums, constrain, tol, maxiter, memsave, maxthreads, kinstems, ids3, filename, ids1, ids2, num_preds, allpreds, allal1, allal2, NULL, NULL, NULL, NULL, NULL, tkeeppreds, tcentres, NULL);
}	//end of m loop
...
if(mode==129||mode==130)	//family estimation of heritability
{
...
for(i=0;i<num_samples_use;i++)
{
for(j=0;j<num_fixed;j++){Z[order[i]+j*num_samples_use]=covar[order[i]+j*num_samples_use];}
}
...
mle_fam(num_samples_use, num_covars, Y, Z, relfile, prev, tol, maxiter, ids3, filename, constrain, (mode==130), num_blocks);
}	//end of m loop
...
{
if(strcmp(envfile,"blank")==0&&strcmp(locofile,"blank")==0)	//basic case
{
#include "singlea.c"
}
...
{
if(dichot==0)	//will do quasi-linear regression
{
#include "singlec.c"
}
else	//will do logistic regression
{
#include "singlee.c"
}
}
...
{
join_genes_reml(folder, num_genes, gnames, gstarts, gends, gparts, gchr, gbp1, gbp2, gamp, gam1, gam2, cut1, cut2, data_length, preds, keeppreds_use, magma);
}
...
{
if(fast==0)	//deal with restage, recount, save file
{
//restage indicates status (-3 - ind-hers, -1 - cv, -2 - many finals, >=0 - best final, -4 - calibration)
//recount is starting number of prediction iterations / chunks

if(restart==0)	//normal start - blank progress, set restage and recount, and make blank save file
{
...
fwrite(&restage, sizeof(int), 1, output2);
fwrite(&recount, sizeof(int), 1, output2);
fclose(output2);
sprintf(cmd, "mv %s.save.part %s.save.bin", outfile, outfile);
system(cmd);
}
else	//restarting from previous run - check progress and save file, get restage and recount
{
if(mode==151){sprintf(readstring,"\"--ridge\"");}
if(mode==152){sprintf(readstring,"\"--bolt\"");}
if(mode==153){sprintf(readstring,"\"--bayesr\"");}
if(mode==154){sprintf(readstring,"\"--elastic\"");}

printf("Checking details from previous run\n\n");
...
//check .save.bin file exists and read restage and recount
...
if(restage==-3||recount==0)
{printf("Error, you can only use \"--restart YES\" if the previous run has started to create the prediction model and has completed at least ten iterations (if running on a cluster, make sure to allocate sufficient memory and time)\n\n");exit(1);}

if(restage>=-1&&skipcv==1){printf("Error, the previous run performed cross-validation; make sure you use the same options now as when you first ran %s\n\n", readstring);exit(1);}
if(restage==-2&&skipcv==0){printf("Error, the previous run did not perform cross-validation; make sure you use the same options now as when you first ran %s\n\n", readstring);exit(1);}

fclose(input2);
}
}
else	//fast version - only need to blank progress
{
sprintf(filename,"%s.progress",outfile);
if((output=fopen(filename,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename);exit(1);}
...
//weights should contain relative expected heritability (already case if using indhers; if restarting, will read from file)
if(strcmp(indhers,"blank")==0&&restart==0)
{
#include "getindhers.c"
}

//ready to make prediction model

if(fast==0)
{
if(restart==0)	//update restage and refresh save file
{
...
if((output2=fopen(filename2,"wb"))==NULL)
{printf("Error opening %s\n\n",filename2);exit(1);}
readint=restage;fwrite(&readint, sizeof(int), 1, output2);
readint=recount;fwrite(&readint, sizeof(int), 1, output2);
fclose(output2);
sprintf(cmd, "mv %s.save.part %s.save.bin", outfile, outfile);
system(cmd);
}
...
if(strcmp(enetfile,"blank")!=0){free(ealphas);}
