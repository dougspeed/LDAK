...
//At end, num_samples_use will be number of valid offspring - keepsamps has length 3*num_samples_use, and indexes offspring, father, mother
...
if(count==0){printf("Error, none of the %d samples have phenotypes and parental IDs\n\n", num_samples_use);exit(1);}
...
if(count2==0){printf("Error, none of the %d samples have phenotypes and their parents in the data\n\n", num_samples_use);exit(1);}
...
//update offspring ids (this works because first count2 elements of indexer are in order)
for(i=0;i<count2;i++)
{
if(indexer[i]!=i)
{
