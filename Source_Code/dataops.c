...
{
//type=0/1/2 - will standardize and set missing to zero; type=3 - set missing to mean
...
if(threadend>threadstart){read_bed_fast(bedfile, data+(size_t)threadstart*num_samples_use, centres+threadstart, mults+threadstart, sqdevs+threadstart, rates+threadstart, infos+threadstart, num_samples_use, keepsamps, threadend-threadstart, keeppreds+threadstart, num_samples, num_preds, missingvalue, bedzeros, bedones, bedtwos, type);}
}
}
}
...
//type=3 - calc stats, then set missing to mean (power can vary; set mults=-9999 when trivial)
//type=4 - set missing to mean using provided centre
//type=5 - calc stats, then set missing to mean (must have power 0; set mults=-9999 only when all missing)
{
int i, j, indcount;
...
for(j=0;j<length;j++)
{
if(type==0||type==1||type==3||type==5)	//calculate centre, mult, sqdev and rate
{
...
if(type==5) //power=0 and only ignore predictor if all values NA
{
if(indcount>0){mults[j]=1;}
else{mults[j]=-9999;}
}
else    //ignore predictor if trivial
{
if(var>0)
{
...
if((type==3||type==4||type==5)&&indcount<ns)	//set missing to mean
{
for(i=0;i<ns;i++)
{
...
int fill_X(double *X, int *Xstarts, int *Xends, double *Xsums, double *Xnss, double *Xrhos, int ns, int num_rands, int num_regs, int **regindex, double *rdata, double *rnss, double *rrhos)
{
int i, j, j2, r, count;
double sumsq;
