...
if(dougvar==101){stats2=malloc(sizeof(double)*data_length);}
...
reg_covar_lin(UTY, UTZ, num_samples_use, num_fixed, 0, thetas, thetasds, thetapvas, Yadj, 0, NULL, NULL);
...
if(dougvar==101)	//compute t(G) invV G / t(G) G
{
...
for(i=0;i<num_samples_use;i++)
{
sumsq+=pow(data[i+j*num_samples_use],2);
sumsq2+=pow(UTdata[i+j*num_samples_use],2);
}
stats2[bitstart+j]=sumsq2/sumsq;
printf("pred %d has %f %f %f\n", j+1, sumsq, sumsq2, sumsq2/sumsq);
}
}
...
//compute var(invV Y) - UTY is currently root(E) t(U) Yadj
for(i=0;i<num_samples_use;i++){UTY[i]*=sweights[i];}

alpha=1.0;beta=0.0;
dgemv_("N", &num_samples_use, &num_samples_use, &alpha, U, &num_samples_use, UTY, &one, &beta, Yadj2, &one);

sum=0;sumsq=0;for(i=0;i<num_samples_use;i++){sum+=Yadj2[i];sumsq+=pow(Yadj2[i],2);}
mean=sum/num_samples_use;var=sumsq/num_samples_use-pow(mean,2);

printf("vars are %f %f\n", varphen, var);

sprintf(filename6,"%s.GVG",outfile);
...
fprintf(output6,"%s %f %f\n", preds[j], var/stats2[j], maf);
}
fclose(output6);
