...
if(num_subs>0)	//reduce to samples in at least one subset (code will work if the subsets are not distinct)
{
count=0;
...
if(drm==1||drm==2)   //exclude ind missing covariates
{
if(strcmp(covarfile,"blank")!=0)    //have quant covariates
{
covar=malloc(sizeof(double)*num_samples*num_quants);
read_covarfile(covarfile, covar, num_samples, allids3, num_quants-1, keepquants, missingvalue, 2);
...
{
flag=0;
for(j=0;j<num_quants-1;j++){flag+=(covar[i+j*num_samples]==missingvalue);}
if(flag>0){usedids[i]=0;count++;}
}
}
if(count>0){printf("Warning, %d samples are missing covariates, so will be excluded\n\n", count);}

free(covar);
printf("\n");
}

if(strcmp(factorfile,"blank")!=0) //have categorical covariates
{
count2=countcols(factorfile)-2;
covar=malloc(sizeof(double)*num_samples*count2);
count_read_factors(factorfile, covar, num_samples, allids3, -9999, missingvalue, 2);
...
{
flag=0;
for(j=0;j<count2;j++){flag+=(covar[i+j*num_samples]==0);}
if(flag>0){usedids[i]=0;count++;}
}
}
if(count>0){printf("Warning, %d samples are missing factors, so will be excluded\n", count);}
...
if((mode==131||mode==132)&&strcmp(prsfile,"blank")!=0)	//check which samples have prs
{
...
count=find_strings(allids3, num_samples, wantids, num_samples_use2, indexer, NULL, NULL, NULL, NULL, NULL, 3);
count2=0;for(i=0;i<count;i++){count2+=usedids[indexer[i]];}
if(count2==0){printf("Error, %s does not contain PRS for any of the %d samples\n\n", filename, num_samples_use);exit(1);}
if(count2<num_samples_use)
{
if(checkprs==1)
{printf("Error, %s contains PRS for only %d of the %d samples; to continue despite this error, use \"--check-PRS NO\"\n\n", filename, count2, num_samples_use);exit(1);}

printf("Warning, %s contains PRS for only %d of the %d samples (the remaining %d samples will be ignored)\n\n", filename, count2, num_samples_use, num_samples_use-count2);
...
for(i=0;i<num_samples_use2;i++){free(wantids[i]);}free(wantids);free(indexer);
}
...
if((mode==151||mode==152||mode==153||mode==154)&&tol==-9999)	//can set tolerance
{
if(num_samples_use>40000){tol=1e-6;}
else{tol=1e-4;}
}
...
if(num_subs>0)	//fill subsets - modes 102/104/123/124/141 - for modes 123/124/141 not allowed duplicates 
{
...
if(subindex[s][0]==0)
{
if(mode==123||mode==124)	//exclude empty
...
if(mode==123||mode==124||mode==141)	//must check no overlap
{
...
if(count>0)
{printf("Error, %d samples feature in more than one subset; when using \"--he\", \"--pcgc\", \"--calc-tagging\" or \"--sample-data\", subsets must not overlap\n\n", count);exit(1);}
free(usedids);
}
}
...
{printf("Error reading %s; this file should have been created when using \"--cut-gre\"\n\n", filename);exit(1);}
...
if(mode==141&&num_subs>1)	//samples must be ordered based on subsets (have ensured no overlap)
{
count=0;
for(s=0;s<num_subs;s++)
{
...
read_ids(filename, NULL, NULL, wantids2, count2, NULL, 0, 0);

count3=find_strings(wantids, num_samples, wantids2, count2, keepsamps+count, NULL, NULL, NULL, NULL, NULL, 3);
count+=count3;
for(i=0;i<count2;i++){free(wantids2[i]);}free(wantids2);
}
if(count!=num_samples_use){printf("Error 5EZ; please tell Doug %d %d\n\n", count, num_samples_use);exit(1);}
}

//make sure ids3 aligned (probably unnecessary)
for(i=0;i<num_samples_use;i++){free(ids3[i]);copy_string(ids3,i,wantids[keepsamps[i]]);}
for(i=0;i<num_samples;i++){free(wantids[i]);}free(wantids);

//make sure ids1 and ids2 aligned (probably unnecessary)
if(strcmp(famfile,datafile)!=0)
{
num_samples=countrows(famfile)-famhead;
wantids=malloc(sizeof(char*)*num_samples);
wantids2=malloc(sizeof(char*)*num_samples);
read_ids(famfile, wantids, wantids2, NULL, num_samples, NULL, famhead, 0);
}
else
{
num_samples=bgen_samples;
wantids=malloc(sizeof(char*)*num_samples);
wantids2=malloc(sizeof(char*)*num_samples);
read_ids_bgen(datafile, wantids, wantids2, NULL, num_samples, NULL, 0);
}
for(i=0;i<num_samples_use;i++){free(ids1[i]);copy_string(ids1,i,wantids[keepsamps[i]]);}
for(i=0;i<num_samples_use;i++){free(ids2[i]);copy_string(ids2,i,wantids2[keepsamps[i]]);}
for(i=0;i<num_samples;i++){free(wantids[i]);free(wantids2[i]);}free(wantids);free(wantids2);
}
...
if(use_data==1||use_data==2||use_data==3||use_data==6||use_data==7)	//will be using predictor ids, so set num_preds, num_preds_use and get predictor details
{
...
check_dups(allpreds,num_preds,bimfile,predorder,0);
...
else{printf("Warning, at least two predictors are called %s; these will be excluded\n", allpreds[predorder[j]]);}
}
if(exsame==1){allbp[predorder[found]]=-1;allbp[predorder[j]]=-1;}
wcount++;
...
if(drm==1)  //exclude chr x predictors (must be mode 131)
{
count=0;
for(j=0;j<num_preds;j++)
{
if(usedpreds[j]==1&&allchr[j]==23){usedpreds[j]=0;count++;}
}
if(count>0){printf("Warning, %d predictors on Chromosome X have been excluded (these must be analyzed separately using \"--DRM CHRX\")\n\n", count);}
}

if(mode==140)	//reduce to predictors with summary statistics
{
count=countrows(sumsfile)-1;
if(count==0){printf("Error, %s does not contain any predictors; please check the output from \"--kvik-step2\"\n\n", sumsfile);exit(1);}

wantpreds=malloc(sizeof(char*)*count);
read_strings(sumsfile, wantpreds, count, NULL, 1, 1);

indexer=malloc(sizeof(int)*num_preds);
count2=find_strings(allpreds, num_preds, wantpreds, count, indexer, NULL, NULL, NULL, predorder, NULL, 3);
if(count2==0){printf("Error, none of the %d predictors in %s are in the data; please tell Doug\n\n", count, sumsfile);exit(1);}

count3=0;for(j=0;j<num_preds;j++){count3+=usedpreds[j];}
for(j=0;j<count2;j++){usedpreds[indexer[j]]++;}
for(j=0;j<num_preds;j++){usedpreds[j]=(usedpreds[j]==2);}
count4=0;for(j=0;j<num_preds;j++){count4+=usedpreds[j];}

if(count4==0){printf("Error, none of the %d predictors in %s remain after filtering\n\n", count, sumsfile);exit(1);}
if(count4<count3){printf("Warning, %s contains summary statistics for only %d of the %d predictors (the remainder will be excluded)\n\n", sumsfile, count4, count3);}
...
printf("where the file \"subset.in\" contains approximately 600,000 predictors. You may already have such a file (e.g., a list of directly-genotyped SNPs, or those passing stringent quality control); otherwise you can generate it using the command \"--thin-common-predictors\"\n");
...
{printf("Error, the predictors appear very dense (average separation is %.2f basepairs), so you must use \"--max-cor\" to specify the pruning threshold (for dense data, we recommend \"--max-cor 0.98\", which will exclude predictors that have correlation squared above 0.98 with a non-excluded predictor; to ensure no predictors are excluded, use \"--max-cor 1\")\n\n", sum/num_preds_use);exit(1);}
else{maxcor=1;}
}
...
if(just_check(filename)!=0)
{printf("Error reading %s; this file should have been created when using %s\n\n", filename, writestring);exit(1);}

count=countrows(filename);
if(count!=num_preds_use)
{printf("Error, the number of predictors remaining (%d) does not match the number in %s (%d), indicating different data files or predictor filterings were used with %s\n\n", num_preds_use, filename, count, writestring);exit(1);}
...
{printf("Error, the predictors remaining do not match those in %s, indicating different datafiles or predictor filterings were used with %s\n\n", filename, writestring);exit(1);}
}
