...
//4 - have data, but only using ids, 5 - multiple datafiles, 6/7 - have bim (not using ids)
use_data=0;
...
{printf("Error, you must use \"--cors\" or \"--corslist\" to specify predictor-predictor correlations\n\n");exit(1);}

if(strcmp(sumsfile,"blank")==0&&strcmp(sumslist,"blank")==0){printf("Error, you must use \"--summary\" or \"--sumslist\" to provide results from single-predictor analysis\n\n");exit(1);}
}

if(mode==180)
{
if(strcmp(corname,"blank")==0)
{printf("Error, you must use \"--cors\" to specify predictor-predictor correlations\n\n");exit(1);}

if(strcmp(sumsfile,"blank")==0){printf("Error, you must use \"--summary\" to provide results from single-predictor analysis\n\n");exit(1);}
}
...
{datastems[k]=malloc(sizeof(char)*500);bimstems[k]=malloc(sizeof(char)*500);famstems[k]=malloc(sizeof(char)*500);}
...
if(use_data==6)	//modes 159 or 180 - only use bimfile, which is set below based on correlations and summary statistics
{strcpy(datafile,"blank");strcpy(bimfile,"blank");strcpy(famfile,"blank");}
if(use_data==7)	//modes 135 - only use bimfile
{strcpy(datafile,"blank");strcpy(bimfile,ubimfile);strcpy(famfile,"blank");}
...
if(power==-9999)
{
if(mode==112||mode==114||mode==127||mode==128||mode==137||mode==138||mode==141||mode==145||mode==173||num_regs>0)
{
...
for(k=0;k<num_kins;k++){kinstems[k]=malloc(sizeof(char)*500);}
...
if(just_check(filename)!=0)
{printf("Error reading %s; this file should have been created when calculating kinships\n\n", filename);exit(1);}
...
if(just_check(filename)!=0)
{
printf("Error reading %s; this file should have been created when calculating kinships; ", filename);
...
if(just_check(filename)!=0)
{printf("Error reading %s; this file should have been created when calculating kinships using \"--gxemm-free\"\n\n", filename);exit(1);}
}
}	//end of k loop
...
//correlations (earlier than expected, because must get num_cors before reading summaries)

if(strcmp(corname,"blank")==0&&strcmp(corslist,"blank")==0){num_cors=0;}
else
{
if(strcmp(corname,"blank")!=0)	//will have already been prefixed by folder
{
num_cors=1;
corstems=malloc(sizeof(char *));
corstems[0]=malloc(sizeof(char)*500);
strcpy(corstems[0], corname);
}
else	//read from file
{
num_cors=countrows(corslist);
corstems=malloc(sizeof(char *)*num_cors);
for(s=0;s<num_cors;s++){corstems[s]=malloc(sizeof(char)*500);}
...
if(fscanf(input, "%s ", filename)!=1)
{printf("Error reading Row %d of %s\n\n", s+1, corslist);exit(1);}
(void)append_check(corstems[s],filename,workdir);

for(s2=0;s2<s;s2++)	//check different to previous
{
if(strcmp(corstems[s],corstems[s2])==0)
{printf("Error reading %s; the stem %s appears twice\n\n", corslist, corstems[s]);exit(1);}
}
}
fclose(input);
}	//end of reading from file
...
for(s=0;s<num_cors;s++)	//check files
{
...
if(just_check(filename)!=0)
{
if(gigaprs==0){printf("Error reading %s; this file should have been created using \"--calc-cors\" or \"--join-cors\"\n\n", filename);exit(1);}
else{printf("Error reading %s; please make sure you have downloaded and extracted a full set of SNP-SNP correlation files (from www.dougspeed.com/giga-prs)\n\n", filename);exit(1);}
}

count=countrows(filename);
if(count!=7){printf("Error reading %s; should have seven rows (not %d), suggesting the file has been changed since creation with \"--calc-cors\" or \"--join-cors\"\n\n",filename,count);exit(1);}
...
if(ftello(input)!=(off_t)sizeof(double)*readint*9+sizeof(int)*readint+sizeof(unsigned short)*scount)
{printf("Error reading %s; should have size %jd, but instead has size %jd\n\n", filename, (off_t)sizeof(double)*readint*9+sizeof(int)*readint+sizeof(unsigned short)*scount, ftello(input));exit(1);}
fclose(input);
...
//note that will check annotations below
}	//end of s loop

if(mode==159&&num_cors>1)	//check windows are consistent (that each window in s spans at most one window in s2)
{
for(s=0;s<num_cors;s++)
{
sprintf(filename,"%s.cors.windows", corstems[s]);
...
{
sprintf(filename2,"%s.cors.windows", corstems[s2]);
count2=countrows(filename2)-1;
if(s!=s2){check_windows(filename,count,filename2,count2);}
}
}
}
...
if(multi==1&&num_resps==1)
{printf("Error, you can only use \"--multivariate YES\" when there is more than one phenotype\n\n");exit(1);}

if(multi==1&&substep>num_resps)
{printf("Error, you can not us \"--sub-step %d\", because there are only %d phenotypes\n\n", substep, num_resps);exit(1);}
...
{
if(strcmp(npheno,"ALL")==0){printf("Warning, if you want to analyse all phenotypes, you should use \"--mpheno ALL\" (and not \"--pheno-name ALL\"\n\n");}

if(check_head_ids(respfile,0)==0)
{printf("Error, you can not use \"--pheno-name\" because %s does not have a header row (the first two columns should be either \"FID IID\" or \"ID1 ID2\")\n\n", respfile);exit(1);}

mpheno=find_head(npheno, respfile, count)-1;
if(mpheno<1){printf("Error, %s does not contain a phenotype called %s\n\n", respfile, npheno);exit(1);}
}
...
if(num_resps>1)
{
printf("Error, %s contains multiple phenotypes, so you must specify one using \"--mpheno\" or \"--pheno-name\"", respfile);
if(mode==121||mode==123||mode==124||mode==126||mode==129||mode==130||(mode==131&&drm==-9999)||mode==132||mode==151||mode==152||mode==153||mode==154||mode==163)
...
if((mode==151||mode==152||mode==153||mode==154)&&loco!=0&&multi!=1)	//check whether the phenotypes are binary, and that dichot is consistent
{
...
{
if(dichot==-9999&&count4==num_resps_use)
{printf("Error, all of the responses in %s appear to be binary, so you must use either \"--binary YES\" or \"--binary NO\" to tell LDAK whether or not to use a quasi-logistic model\n\n", respfile);exit(1);}
...
if(strcmp(bulkfile,"blank")!=0) //have bulk summaries (easier to do now, so can set num_sums2)
{
count=countcols(bulkfile);
if(count<6){printf("Error, %s should have at least six columns (not %d), suggesting the file did not download properly\n\n", bulkfile, count);exit(1);}
if(count%2!=0){printf("Error, %s should have an even number of columns (not %d), suggesting the file did not download properly\n\n", bulkfile, count);exit(1);}
num_sums2=(count-4)/2;

bulknames=malloc(sizeof(char *)*num_sums2);
for(q=0;q<num_sums2;q++){bulknames[q]=malloc(sizeof(char)*500);}

if((input=fopen(bulkfile,"r"))==NULL)
{printf("Error opening %s\n\n",bulkfile);exit(1);}
if(fscanf(input, "%s %s %s %s ", readstring, readstring2, readstring3, readstring4)!=4)
{printf("Error reading Row 1 of %s\n\n", bulkfile);exit(1);}
if(strcmp(readstring,"Predictor")!=0)
{printf("Error reading %s; first element should be \"Predictor\" (not %s), suggesting the file did not download properly\n\n", bulkfile, readstring);exit(1);}
if(strcmp(readstring2,"A1")!=0)
{printf("Error reading %s; second element should be \"A1\" (not %s), suggesting the file did not download properly\n\n", bulkfile, readstring2);exit(1);}
if(strcmp(readstring3,"A2")!=0)
{printf("Error reading %s; third element should be \"A2\" (not %s), suggesting the file did not download properly\n\n", bulkfile, readstring3);exit(1);}
if(strcmp(readstring4,"A1Freq")!=0)
{printf("Error reading %s; fourth element should be \"A1Freq\" (not %s), suggesting the file did not download properly\n\n", bulkfile, readstring4);exit(1);}

for(q=0;q<num_sums2;q++)
{
if(fscanf(input, "%s %s ", readstring, readstring2)!=2)
{printf("Error reading Columns %d and %d of Row 1 of %s\n\n", 5+2*q, 6+2*q, bulkfile);exit(1);}
sprintf(bulknames[q],"%s_%s",bulkfile,readstring2);
}
...
if(strcmp(sumsfile,"blank")!=0)	//have one summary statistics (if megaprs, must also have only one set of correlations)
{
num_sums1=1;
num_sums3=1+num_sums2;

sumstems=malloc(sizeof(char *)*num_sums3);
pritraits=malloc(sizeof(int)*num_sums3);
sumpops=malloc(sizeof(int)*num_sums3);

copy_string(sumstems,0,sumsfile);
for(q=0;q<num_sums2;q++){copy_string(sumstems,1+q,bulknames[q]);}
pritraits[0]=1;
for(q=0;q<num_sums2;q++){pritraits[1+q]=0;}
sumpops[0]=0;
metasum=0;

gotfreq=check_sformat(sumsfile);
...
if(strcmp(sums2file,"blank")!=0)	//have a second summary statistic (only used for sum-cors)
{
(void) check_sformat(sums2file);
...
if(strcmp(sumslist,"blank")!=0)	//have a list of summary statistics (only used for meta-analyse and mega-prs)
{
if(mode==135)   //meta-analyse
{
num_sums3=countrows(sumslist);
sumstems=malloc(sizeof(char *)*num_sums3);
pritraits=malloc(sizeof(int)*num_sums3);
sumpops=malloc(sizeof(int)*num_sums3);

for(q=0;q<num_sums3;q++){sumstems[q]=malloc(sizeof(char)*500);}
for(q=0;q<num_sums3;q++){pritraits[q]=0;sumpops[q]=0;}

if((input=fopen(sumslist,"r"))==NULL)
{printf("Error opening %s\n\n",sumslist);exit(1);}
for(q=0;q<num_sums3;q++)
{
if(fscanf(input, "%s%c", filename, &readchar)!=2)
{printf("Error reading Row %d of %s\n\n", q+1, filename);exit(1);}

if(append_check(sumstems[q],filename,workdir)!=0)
{printf("Error reading summary statistics file %s; check file exists and is not empty\n\n", sumstems[q]);exit(1);}

for(q2=0;q2<q;q2++)	//check name different to previous
{
if(strcmp(sumstems[q],sumstems[q2])==0)
{printf("Error reading %s; the file %s appears twice\n\n", sumslist, sumstems[q]);exit(1);}
}
...
count2=countcols(sumslist);
if(count2!=3)
{
if(num_cors==1)
{
printf("Error, %s should have three columns (not %d); the first column should provide the names of the summary statistics files, the second column should indicate which summary statistics are focal (\"F\"), primary (\"P\") and secondary (\"S\"), while the third column should provide the indexes of the corresponding predictor-predictor correlations (for this analysis, the final column will always be 1, because there is only one set of correlations)\n\n", sumslist, count2);
printf("For example, suppose %s contains the following five lines:\nfile1 F 1\nfile2 P 1\nfile3 P 1\nfile4 S 1\nfile5 S 1\n\nThis indicates that file1 contains the (only) focal summary statistics, file2 and file3 contains primary summary statistics, while file4 and file5 contains secondary summary statistics (all summary statistics are matched to the correlations with stem %s)\n\n", sumslist, corstems[0]);
}
else
{
printf("Error, %s should have three columns (not %d); the first column should provide the names of the summary statistics files, the second column should indicate which summary statistics are focal (\"F\"), primary (\"P\") and secondary (\"S\"), while the third column should provide the indexes of the corresponding predictor-predictor correlations (an integer between 1 and %d, because there are %d sets of correlations)\n\n", sumslist, count2, num_cors, num_cors);
printf("For example, suppose %s contains the following five lines:\nfile1 F 1\nfile2 P 1\nfile3 P 2\nfile4 S 1\nfile5 S 2\n\nThis indicates that file1 contains the (only) focal summary statistics, file2 and file3 contains primary summary statistics, while file4 and file5 contains secondary summary statistics; meanwhile, the summary statistics in file1, file2 and file4 are matched to the correlations with stem %s, while those in file3 and file5 are matched to the correlations with stem %s\n\n", sumslist, corstems[0], corstems[1]);
}
exit(1);
}

//read names, types and pops, and check names different
num_sums1=countrows_plus(sumslist,3);
num_sums3=num_sums1+num_sums2;

sumstems=malloc(sizeof(char *)*num_sums3);
sumstems2=malloc(sizeof(char *)*num_sums3);
pritraits=malloc(sizeof(int)*num_sums3);
pritraits2=malloc(sizeof(int)*num_sums3);
sumpops=malloc(sizeof(int)*num_sums3);
sumpops2=malloc(sizeof(int)*num_sums3);

for(q=0;q<num_sums1;q++){sumstems[q]=malloc(sizeof(char)*500);sumstems2[q]=malloc(sizeof(char)*500);}
for(q=0;q<num_sums2;q++){copy_string(sumstems,num_sums1+q,bulknames[q]);copy_string(sumstems2,num_sums1+q,bulknames[q]);}
for(q=0;q<num_sums2;q++){pritraits[num_sums1+q]=0;pritraits2[num_sums1+q]=0;}

if((input=fopen(sumslist,"r"))==NULL)
{printf("Error opening %s\n\n",sumslist);exit(1);}
for(q=0;q<num_sums1;q++)
{
if(fscanf(input, "%s %s %d%c", filename, readstring, &readint, &readchar)!=4)
{
if(num_cors==1)
{
printf("Error reading Row %d of %s; the first column should provide the names of the summary statistics files, the second column should indicate which summary statistics are focal (\"F\"), primary (\"P\") and secondary (\"S\"), while the third column should provide the indexes of the corresponding predictor-predictor correlations (for this analysis, the final column will always be 1, because there is only one set of correlations)\n\n", q+1, sumslist);
printf("For example, suppose %s contains the following five lines:\nfile1 F 1\nfile2 P 1\nfile3 P 1\nfile4 S 1\nfile5 S 1\n\nThis indicates that file1 contains the focal summary statistics, file2 and file3 contain primary summary statistics, while file4 and file5 contain secondary summary statistics (all summary statistics are matched to the correlations with stem %s)\n\n", sumslist, corstems[0]);
}
else
{
printf("Error reading Row %d of %s; the first column should provide the names of the summary statistics files, the second column should indicate which summary statistics are focal (\"F\"), primary (\"P\") and secondary (\"S\"), while the third column should provide the indexes of the corresponding predictor-predictor correlations (an integer between 1 and %d, because there are %d sets of correlations)\n\n", q+1, sumslist, num_cors, num_cors);
printf("For example, suppose %s contains the following five lines:\nfile1 F 1\nfile2 P 1\nfile3 P 2\nfile4 S 1\nfile5 S 2\n\nThis indicates that file1 contains the (only) focal summary statistics, file2 and file3 contain primary summary statistics, while file4 and file5 contain secondary summary statistics; meanwhile, the summary statistics in file1, file2 and file4 are matched to the correlations with stem %s, while those in file3 and file5 are matched to the correlations with stem %s\n\n", sumslist, corstems[0], corstems[1]);
}
exit(1);
}

if(append_check(sumstems2[q],filename,workdir)!=0)
{printf("Error reading summary statistics file %s; check file exists and is not empty\n\n", sumstems2[q]);exit(1);}

//get type - can be F, P or S (or maybe M)
flag=0;
if(strcmp(readstring,"F")==0){pritraits2[q]=2;flag=1;}
if(strcmp(readstring,"P")==0){pritraits2[q]=1;flag=1;}
if(strcmp(readstring,"S")==0){pritraits2[q]=0;flag=1;}
if(strcmp(readstring,"M")==0){pritraits2[q]=3;flag=1;}
if(flag==0)
{
if(num_sums1==1){printf("Error reading %s; the second element should be \"F\" or \"M\" (not %s)\n\n", sumslist, readstring);}
else{printf("Error reading Row %d of %s; the second element should be \"F\", \"P\", \"S\" or \"M\" (not %s), indicating which summary statistics are focal, primary, secondary and multi-ancestry\n\n", q+1, sumslist, readstring);}
exit(1);
}

if(readint<1||readint>num_cors)
{
if(num_cors==1){printf("Error reading Row %d of %s; the third element should be 1 (not %d)\n\n", q+1, sumslist, readint);exit(1);}
else{printf("Error reading Row %d of %s; the third element should be between 1 and %d (not %d)\n\n", q+1, sumslist, num_cors, readint);}
exit(1);
}
sumpops2[q]=readint-1;

for(q2=0;q2<q;q2++)	//check name different to previous
{
if(strcmp(sumstems2[q],sumstems2[q2])==0)
{printf("Error reading %s; the file %s appears twice\n\n", sumslist, sumstems2[q]);exit(1);}
}
}
fclose(input);

//set metasum
metasum=0;for(q=0;q<num_sums1;q++){metasum+=(pritraits2[q]==3);}
if(metasum>1){printf("Error reading %s, there can only be at most one set of multi-ancestry summary statistics\n\n", sumslist);exit(1);}

if(metasum==1)  //check no focals, then switch pritraits from 3 to 2
{
count=0;for(q=0;q<num_sums1;q++){count+=(pritraits2[q]==2);}
if(count>0){printf("Error reading %s, you can not have both focal summary statistics and multi-ancestry summary statistics\n\n", sumslist);exit(1);}
for(q=0;q<num_sums1;q++)
{
if(pritraits2[q]==3){pritraits2[q]=2;}
}
}

//work out how many analyses
num_focals=0;for(q=0;q<num_sums1;q++){num_focals+=(pritraits2[q]==2);}
if(num_focals==0){printf("Error reading %s, there are no focal summary statistics\n\n", sumslist);exit(1);}

//check the focals are assigned to different correlations
for(s=0;s<num_cors;s++)
{
count=0;for(q=0;q<num_sums1;q++){count+=(pritraits2[q]==2&&sumpops2[q]==s);}
if(count>1){printf("Error reading %s, more than one focal summary statistic is assigned to the correlations with stem %s\n\n", sumslist, corstems[s]);exit(1);}
}

//load up sumstems, pritraits and sumpops ready for first analysis (cur_focals will be zero)
count=0;
for(q=0;q<num_sums1;q++)
{
count+=(pritraits2[q]==2);
if(count==cur_focal+1){break;}
}

strcpy(sumstems[0],sumstems2[q]);
pritraits[0]=1;
sumpops[0]=sumpops2[q];
count=1;
for(q2=0;q2<num_sums1;q2++)
{
if(q2!=q)
{
strcpy(sumstems[count],sumstems2[q2]);
pritraits[count]=(pritraits2[q2]!=0);
sumpops[count]=sumpops2[q2];
count++;
}
}

//check format and set gotfreq
flag=0;for(q=0;q<num_sums1;q++){flag+=check_sformat(sumstems[q]);}

gotfreq=(flag==num_sums1);
if(checkfreq==1&&gotfreq==0)
{
if(flag==0){printf("Error, none of the %d summary statistics contain a column named \"A1Freq\", so it is not possible to use \"--check-frequencies YES\"\n\n", num_sums1);}
else{printf("Error, only %d of the %d summary statistics contain a column named \"A1Freq\", so it is not possible to use \"--check-frequencies YES\"\n\n", flag, num_sums1);}
exit(1);
}
...
{printf("Error, it is not possible to use \"--save-all-models YES\" when there are multiple sets of focal summary statistics\n\n");exit(1);}
if(prsvar==1&&num_focals>1)
{printf("Error, it is not possible to use \"--PRS-variance YES\" when there are multiple sets of focal summary statistics\n\n");exit(1);}

if(metasum==1)
{
printf("Note that the focal summary statistics are assumed to come from a multi-ancestry analysis\n");
if(find_head("A1Freq", sumstems[0], -1)==-1)
{printf("Error, %s does not contain a column named \"A1Freq\", which is required to infer the ancestry proportions\n\n", sumstems[0]);exit(1);}
}
...
for(q=0;q<num_sums1;q++)
{
if(pritraits2[q]==0){printf("%s contains secondary summary statistics; these are matched to the correlations with stem %s\n", sumstems2[q], corstems[sumpops2[q]]);}
if(pritraits2[q]==1){printf("%s contains primary summary statistics; these are matched to the correlations with stem %s\n", sumstems2[q], corstems[sumpops2[q]]);}
if(pritraits2[q]==2){printf("%s contains focal summary statistics; these are matched to the correlations with stem %s\n", sumstems2[q], corstems[sumpops2[q]]);}
}
}
printf("\n");
}
...
//now deal with some left over bits from megaprs / gigaprs

if(use_data==6)	//set bimfile based on correlations for the first set of summary statistics
{sprintf(bimfile,"%s.cors.bim", corstems[sumpops[0]]);}

if(gigaprs==1&&useann!=0)	//check annotations file exists for focal correlations, has correct size, and get num_parts
{
sprintf(filename,"%s.cors.root", corstems[sumpops[0]]);
read_integers(filename, &readint, 1, NULL, 2, 4, 0);

sprintf(filename,"%s.cors.annotations", corstems[sumpops[0]]);
if(just_check(filename)!=0)
{printf("Error reading %s; please make sure you have downloaded and extracted a full set of SNP-SNP correlation files (from www.dougspeed.com/giga-prs)\n\n", filename);exit(1);}

count=countrows(filename);
if(count!=readint+1)
{printf("Error reading %s; should have %d rows (not %d), suggesting the file did not download properly or does not match the SNP-SNP correlations\n\n",filename, readint+1, count);exit(1);}

num_parts=countcols(filename)-2;
}
...
keepquants=malloc(sizeof(int)*(count-2));
if(strcmp(covarnums,"blank")==0&&strcmp(covarnames,"blank")==0)	//no filtering
{
num_quants=count-1;
for(j=0;j<count-2;j++){keepquants[j]=j;}
}
else	//filtering
{
if(strcmp(covarnums,"blank")!=0)	//filtering based on numbers
{num_quants=1+find_covar_numbers(covarnums, keepquants, count-2, covarfile);}
else	//filtering based on names
{num_quants=1+find_covar_names(covarnames, keepquants, count-2, covarfile);}
}
}
...
if(mode==173&&num_envs!=1)
{printf("Error, when using \"--make-phenos\", you can only have one environmental variable (not %d)\n\n", num_envs);exit(1);}
}
else
{num_envs=0;}
...
if(strcmp(factorfile,"blank")!=0)	//have factors (will work out how many later)
{
...
if(strcmp(rovarfile,"blank")!=0)	//have random covariates (will not add an intercept)
{
count=countcols(rovarfile);
if(count<3){printf("Error, %s should have at least three columns (not %d)\n\n", rovarfile, count);exit(1);}
checkcols(rovarfile,count);

keepruants=malloc(sizeof(int)*(count-2));
num_ruants=count-2;
for(j=0;j<count-2;j++){keepruants[j]=j;}
}
else{num_ruants=0;}

if(strcmp(ractorfile,"blank")!=0)	//have random factors (will work out how many later)
{
count=countcols(ractorfile);
if(count<3){printf("Error, %s should have at least three columns (not %d)\n\n", ractorfile, count);exit(1);}
checkcols(ractorfile,count);
}
...
if(just_check(filename)!=0)
{printf("Error reading %s; this file should have been created using \"--solve-gre\"\n\n", filename);exit(1);}
count=countrows(filename);
...
if(fscanf(input, "Num_Randoms %d ", &num_rands)!=1)
{printf("Error reading Row 2 of %s (should begin \"Num_Randoms\"), suggesting the file has been changed since creation when performing REML\n\n",remlfile);exit(1);}

if(fscanf(input, "Num_Regions %d ", &num_scores)!=1)
{printf("Error reading Row 3 of %s (should begin \"Num_Regions\"), suggesting the file has been changed since creation when performing REML\n\n",remlfile);exit(1);}
...
{printf("Error reading Row 4 of %s (should begin \"Num_Top_Predictors\"), suggesting the file has been changed since creation when performing REML\n\n",remlfile);exit(1);}
...
{printf("Error reading Row 5 of %s (should begin \"Num_Covariates\"), suggesting the file has been changed since creation when performing REML\n\n",remlfile);exit(1);}
...
{printf("Error reading Row 6 of %s (should begin \"Num_Environments\"), suggesting the file has been changed since creation when performing REML\n\n",remlfile);exit(1);}
...
{printf("Error reading Row 7 of %s (should begin \"Blupfile\"), suggesting the file has been changed since creation when performing REML\n\n",remlfile);exit(1);}
if(strcmp(readstring,"none")!=0)
{
strcpy(blupfile2,readstring);
if(append_check(blupfile,blupfile2,workdir)!=0)
{printf("Error reading blupfile %s; this file should have been created when performing REML\n\n", blupfile);exit(1);}
...
{printf("Error reading Row 8 of %s (should begin \"Regfile\"), suggesting the file has been changed since creation when performing REML\n\n",remlfile);exit(1);}
if(strcmp(readstring,"none")!=0)
{
strcpy(regfile2,readstring);
if(append_check(regfile,regfile2,workdir)!=0)
{printf("Error reading regfile %s; this file should have been created when performing REML\n\n", regfile);exit(1);}
...
{printf("Error reading Row 9 of %s (should begin \"Coeffsfile\"), suggesting the file has been changed since creation when performing REML\n\n",remlfile);exit(1);}
if(append_check(cofile,cofile2,workdir)!=0)
{printf("Error reading coeffsfile %s; this file should have been created when performing REML\n\n", cofile);exit(1);}
fclose(input);
...
if(just_check(filename)!=0)
{printf("Error reading %s; this file should have been created using \"--adjust-grm\"; if you are not using adjusted kinships, use \"--adjusted NO\"\n\n", filename);exit(1);}
if((input=fopen(filename,"r"))==NULL)
{printf("Error opening %s\n\n",filename);exit(1);}
...
if(strcmp(relfile,"blank")!=0)  //used for modes 129/130 (and 229/230) and 131+drm or 151+fastgwa=1
{
count=countcols(relfile);
if((mode==129||mode==130||mode==229||mode==230)&&count!=5&&count!=6){printf("Error, %s should have either five or six columns (not %d), containing distinct pairs of samples, their estimated relatedness, and perhaps also their environmental similarity\n\n", relfile, count);exit(1);}
if((drm!=-9999||fastgwa==1)&&count!=5){printf("Error, %s should have five columns (not %d), containing pairs of samples and their estimated relatedness\n\n", relfile, count);exit(1);}
...
if(strcmp(sexfile,"blank")!=0)    //check sexfile - should either have three columns, or a column labelled sex
{
count=countcols(sexfile);
if(count<3){printf("Error, %s should have at least three columns (not %d)\n\n", sexfile, count);exit(1);}

head=check_head_ids(sexfile,0);
if(head==0)
{
if(count>3){printf("Error, %s contains more than three columns but does not have a header row (the first two columns should be either \"FID IID\" or \"ID1 ID2\")\n\n", sexfile);exit(1);}
}
else //check there is a column labelled sex
{
count2=find_head("Sex", sexfile, count)-2;
if(count2<0){printf("Error, %s does not have a column named \"Sex\"\n\n", sexfile);exit(1);}
}
}
...
if(strcmp(transfile,"blank")!=0)	//check files exist and correct size
{
...
for(k=0;k<num_tags;k++){tagstems[k]=malloc(sizeof(char)*500);}
...
if(strcmp(xaglist,"blank")!=0)	//have xaglist
{
num_tags=countrows(xaglist);
if(num_tags==1){printf("Error, %s must contain at least two cross-ancestry tagging files (not one)\n\n", xaglist);exit(1);}

tagstems=malloc(sizeof(char *)*num_tags);
for(k=0;k<num_tags;k++){tagstems[k]=malloc(sizeof(char)*500);}

if((input=fopen(xaglist,"r"))==NULL)
{printf("Error opening %s\n\n",xaglist);exit(1);}
for(k=0;k<num_tags;k++)
{
if(fscanf(input, "%s ", filename)!=1)
{printf("Error reading Row %d of %s\n\n", k+1, xaglist);exit(1);}
if(append_check(tagstems[k],filename,workdir)!=0)
{printf("Error reading cross-ancestry tagging file %s; check file exists and is not empty\n\n", tagstems[k]);exit(1);}

for(k2=0;k2<k;k2++)	//check different to previous
{
if(strcmp(tagstems[k],tagstems[k2])==0)
{printf("Error reading %s; the file %s appears twice\n\n", xaglist, tagstems[k]);exit(1);}
}
}
fclose(input);
}
...
if(strcmp(xagfile,"blank")!=0)	//check correct size and set num_parts and parttype
{
count=countcols(xagfile);
if(count!=12)
{printf("Error, %s should have at 12 columns (not %d), suggesting the file has been changed since creation with \"--calc-tagging\"\n\n", xagfile, count);exit(1);}

count=countrows_min(xagfile,6);
if(count<6)
{printf("Error, %s should have at least six rows, suggesting the file has been changed since creation with \"--calc-tagging\"\n\n", xagfile);exit(1);}

if((input=fopen(xagfile,"r"))==NULL)
{printf("Error opening %s\n\n",xagfile);exit(1);}
if(fscanf(input, "%s %s %s %s %s %s %s %s %s ", readstring, readstring2, readstring2, readstring2, readstring2, readstring2, readstring2, readstring2, readstring2)!=9)
{printf("Error reading Row 1 of %s\n\n", xagfile);exit(1);}
if(strcmp(readstring,"Predictor")!=0)
{printf("Error reading %s; first element should be \"Predictor\" (not %s), suggesting the file has been changed since creation with \"--calc-tagging\"\n\n", xagfile, readstring);exit(1);}
...
if(strcmp(wayfile,"blank")!=0)	//check have files, check sizes and get num_parts
{
...
sprintf(filename,"%s.pathway.sums", wayfile);
if(just_check(filename)!=0)
{printf("Error reading %s; this file should have been created using \"--calc-taggings\"\n\n", filename);exit(1);}
...
sprintf(filename,"%s.pathway.tagging", wayfile);
if(just_check(filename)!=0)
{printf("Error reading %s; this file should have been created using \"--calc-taggings\"\n\n", filename);exit(1);}
...
for(j=0;j<count;j++)
{
if(fscanf(input, "%d%c", &readint, &readchar)!=2)
...
if(strcmp(betafile,"blank")!=0)	//check correct size and get num_phenos
{
count=countcols(betafile);
if(count!=7)
{printf("Error, %s should have seven columns (not %d)\n\n", betafile, count);exit(1);}
checkcols(betafile,count);

count=countrows(betafile);
read_integers(betafile, &num_phenos, 1, NULL, 1, count-1, 0);
}
...
if(just_check(filename)!=0)
{printf("Error reading %s; this file should have been created using \"--pseudo-summaries\"\n\n", filename);exit(1);}
...
if(countcols(bestfile)!=10){printf("Error, %s should have ten columns (not %d), suggesting the file has been changed since creation with \"--mega-prs\"\n\n", bestfile, countcols(bestfile));exit(1);}
...
if(strcmp(fracfile,"blank")!=0)	//either bolt, bayesr, elastic or mega with ptype 4, 5, 6 or 7
{
...
if(ptype==4)	//bolt
{
if(count!=2){printf("Error, %s should have two columns (not %d), providing p then f2\n\n", fracfile, count);exit(1);}
}
if(ptype==5)	//bayes
{
if(count!=4){printf("Error, %s should have four columns (not %d), providing the fractions for the point mass and the small, medium and large Gaussian distributions\n\n", fracfile, count);exit(1);}
}
...
if(just_check(filename)!=0)
{printf("Error reading %s; this file should have been created when using \"--pca\"\n\n", filename);exit(1);}
count=countcols(filename);
if(count<3)
...
if(just_check(filename)!=0)
{printf("Error reading %s; this file should have been created using \"--pca\"\n\n", filename);exit(1);}
...
if(just_check(filename)!=0)
{printf("Error reading %s; this file should have been created using \"--decompose\"\n\n", filename);exit(1);}
...
if(strcmp(scorefile,"blank")!=0)	//check correct size and get num_scores (and maybe num_blocks)
{
count=countcols(scorefile);
if(count<5)
{printf("Error, %s should have least five columns (not %d): \"Predictor\", \"A1\", \"A2\" and \"Centre\", followed by effect sizes for one or more profiles (note that the \"Centre\" column should provide average A1 allele counts, or \"NA\" when these are unknown)\n\n", scorefile, count);exit(1);}
...
if(strcmp(readstring,"Predictor")!=0)
{printf("Error, the first column of %s should be named \"Predictor\" (not %s)\n\n", scorefile, readstring);exit(1);}
if(strcmp(readstring2,"A1")!=0)
{printf("Error, the second column of %s should be named \"A1\" (not %s)\n\n", scorefile, readstring2);exit(1);}
if(strcmp(readstring3,"A2")!=0)
{printf("Error, the third column of %s should be named \"A2\" (not %s)\n\n", scorefile, readstring3);exit(1);}
if(strcmp(readstring4,"Centre")!=0)
{printf("Error, the fourth column of %s should be named \"Centre\" (not %s)\n\n", scorefile, readstring4);exit(1);}
fclose(input);
...
if(strcmp(morefile,"blank")!=0)	//check correct size and get num_scores
{
count=countcols(morefile);
if(count<5)
{printf("Error, %s should have least five columns (not %d): \"Predictor\", \"A1\" and \"A2\", followed by centres and effect sizes for each set of profiles (note that the \"Centre\" column should provide average A1 allele counts)\n\n", morefile, count);exit(1);}
if(count%2!=1)
{printf("Error, %s should have an one number of columns (not %d): \"Predictor\", \"A1\" and \"A2\", followed by centres and effect sizes for each set of profiles (note that the \"Centre\" column should provide average A1 allele counts)\n\n", morefile, count);exit(1);}
checkcols(morefile,count);

if((input=fopen(morefile,"r"))==NULL)
{printf("Error opening %s\n\n",morefile);exit(1);}
if(fscanf(input, "%s %s %s %s", readstring, readstring2, readstring3, readstring4)!=4)
{printf("Error reading first four elements of %s\n\n", morefile);exit(1);}

if(strcmp(readstring,"Predictor")!=0)
{printf("Error, the first column of %s should be named \"Predictor\" (not %s)", morefile, readstring);exit(1);}
if(strcmp(readstring2,"A1")!=0)
{printf("Error, the second column of %s should be named \"A1\" (not %s)", morefile, readstring2);exit(1);}
if(strcmp(readstring3,"A2")!=0)
{printf("Error, the third column of %s should be named \"A2\" (not %s)", morefile, readstring3);exit(1);}
fclose(input);

num_scores=(count-3)/2;

count=countrows_min(morefile,2);
if(count==1){printf("Error, %s does not contain any predictors (only headers)\n\n", morefile);exit(1);}
}
