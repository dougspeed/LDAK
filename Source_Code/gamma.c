...
//Code I've found for evaluating the digamma, trigamma and incomplete gamma functions and sampling from gamma dist (with rate 1)
...
{
int count;
double shape2,u2,d,c,x,v,u,v3,x2;

if(shape>=1)    //generate gamma(shape) directly
{
shape2=shape;
u2=1;
}
...
u=genrand_real2();
x2=x*x;
if (u<1-0.0331*x2*x2){return (d*v3*u2);}
if(log(u)<0.5*x2+d*(1-v3+log(v3))){return(d*v3*u2);}
