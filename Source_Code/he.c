...
//double *DY, *EDY, *DYYT, *DYYT2, *DTD3, sum3, sum4;
...
//DY=malloc(sizeof(double)*ns*total);
//EDY=malloc(sizeof(double)*ns*total);
//DYYT=malloc(sizeof(double)*total*total);
//DYYT2=malloc(sizeof(double)*total*total);
//DTD3=malloc(sizeof(double)*total*total);
}
...
for(p=0;p<num_blocks;p++)
{
start=(double)(p)*ns/num_blocks;
end=(double)(p+1)*ns/num_blocks;
...
for(k=0;k<total;k++)
{
for(i=0;i<ns;i++){DY[i+k*ns]=Yadj[indexer2[i]];}
}
*/
...
//add in contribution to DY (do not consider subsets)
//for(k=0;k<total;k++){DY[i+k*ns]+=Dtemp[i2+k*ns]*Yadj[indexer2[i2]];DY[i2+k*ns]+=Dtemp[i2+k*ns]*Yadj[indexer2[i]];}
}}	//end i2 and i loop
printf("\n");
...
//now get EDY - start by setting EDY=DY
/*
for(k=0;k<total;k++)
{
for(i=0;i<ns;i++){EDY[i+k*ns]=DY[i+k*ns];}
}

//loop through, filling a sample's worth of contributions to EDY at a time
scount=0;
for(i=1;i<ns;i++)
{
//load up first i-1 rows of Dtemp with scaled kinships
...
for(k2=0;k2<total;k2++)
{
EDY[i+k*ns]+=hers[k2]*Dtemp[i2+k2*ns]*DY[i2+k*ns];
EDY[i2+k*ns]+=hers[k2]*Dtemp[i2+k2*ns]*DY[i+k*ns];
}
}
scount++;
}}	//end i2 and i loop
...
for(k2=0;k2<total;k2++)
{
DYYT[k+k2*total]=0;for(i=0;i<ns;i++){DYYT[k+k2*total]+=DY[i+k*ns]*EDY[i+k2*ns];}
}
}

//variance matrix should be inv(DTD) DYYT inv(DTD)/2
for(k=0;k<total;k++)
{
for(k2=0;k2<total;k2++){DTD[k+k2*total]=DTDs[k+k2*total];}
}
(void)eigen_invert(DTD, total, DTD2, total, DYYT, 1);

for(k=0;k<total;k++)
{
for(k2=0;k2<total;k2++){DTD[k+k2*total]=DTDs[k+k2*total];DYYT2[k+k2*total]=DYYT[k2+k*total];}
}
(void)eigen_invert(DTD, total, DTD2, total, DYYT2, 1);
*/
...
//free(DY);free(EDY);free(DYYT);free(DYYT2);
}
