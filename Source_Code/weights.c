...
current=read_data_fly(datafile, dtype, data+(size_t)shuffle*num_samples_use, NULL, num_samples_use, keepsamps, bitstart+shuffle, bitend, keeppreds_use, datainputgz, current, num_samples, num_preds, genskip, genheaders, genprobs, bgen_indexes, missingvalue, -9999, -9999, nonsnp, maxthreads);
...
if(simplex==0)
{
flag=solve_quad(tally3, cors, tally2, bitlength, filename, fudge);
}
else
{
//calculate the weights (using fudge if necessary)
#if MET==0
flag=solve_simplex_new(tally3, cors, tally2, bitlength, maxiter, maxtime, filename, fudge);
#else
flag=solve_simplex(tally3, cors, tally2, bitlength, maxiter, maxtime, filename, fudge);
#endif
}
...
{fprintf(output2, "%.6f %.2f %.3f 1 %.3f\n", tally3[j], tally1[j], tally2[j], cors2[j]);}
fclose(output2);
...
free(data);
free(cors);free(cors2);free(tally1);free(tally2);free(tally3);
