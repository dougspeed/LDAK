...
mg=malloc(sizeof(double*)*num_extra);
for(r=0;r<num_extra;r++){mg[r]=malloc(sizeof(double)*ns);}
mg2=malloc(sizeof(double*)*num_extra);
for(r=0;r<num_extra;r++){mg2[r]=malloc(sizeof(double)*ns);}
...
hers[0]=.5;shares[0]=1;
for(r=1;r<total;r++){hers[r]=.5/num_extra;shares[r]=1.0/num_extra;}
...
for(r=0;r<num_extra;r++)	//get I_k TD - similarly, get even if variance constrained
{
...
for(r=0;r<num_extra;r++)	//get traces[1+r] = tr(invT I_k)
{
traces[1+r]=0;
...
for(r=0;r<num_extra;r++)	//now noise x regions
{
if(fixed[1+r]<3)
{
...
//print update
sum=0;for(r=0;r<num_extra;r++){sum+=hers[1+r];}
...
for(r=0;r<num_extra;r++){printf("%.4f\t", hers[1+r]);}
printf("%.4f\t", sum);
...
for(r=0;r<num_extra;r++){fprintf(output, "%.6f\t", hers[1+r]);}
...
if(nlost>=num_extra){printf("All heritabilities are constrained\n");break;}
if(count>0)
{
...
if(num_regs>0||num_tops>0)	//load up effects for regions (saved in TD) and tops (saved in thetas)
{
...
if(num_extra==1){fprintf(output2, "LRT_Stat %.4f\nLRT_P %.4e\n", lrtstat, lrtpva);}
...
for(r=0;r<num_regs;r++){fprintf(output2, "Her_R%d %.6f %.6f %.2f %.6f %.6f\n", r+1, hers[1+num_rands+r], hersds[1+num_rands+r], Xsums[num_rands+r], hers[1+num_rands+r]/Xsums[num_rands+r]*1000000, hersds[1+num_rands+r]/Xsums[num_rands+r]*1000000);}
...
for(r=0;r<num_regs;r++){fprintf(output4, "Share_R%d %.6f %.6f %.6f %.6f %.6f\n", r+1, shares[1+num_rands+r], sharesds[1+num_rands+r], Xsums[num_rands+r]/sum, shares[1+num_rands+r]/Xsums[num_rands+r]*sum, sharesds[1+num_rands+r]/Xsums[num_rands+r]*sum);}
fclose(output4);
...
for(r=0;r<num_regs;r++){fprintf(output5, "Var_C%d %.6f %.6f\n", r+1, vars[1+r], varsds[1+r]);}
for(r=0;r<num_rands;r++){fprintf(output5, "Var_R%d %.6f %.6f\n", r+1, vars[1+num_rands+r], varsds[1+num_rands+r]);}
...
for(r=0;r<num_extra;r++){fprintf(output7, "0\t%.6f\t", mg[r][i]);}
fprintf(output7, "\n");
}
fclose(output7);
}
...
if(num_extra==1){fprintf(output, "LRT_Stat %.4f\nLRT_P %.4e\n", lrtstat, lrtpva);}
...
for(r=0;r<num_regs;r++){fprintf(output, "Her_R%d %.6f %.6f %.2f %.6f %.6f\n", r+1, hers[1+num_rands+r]*factor, hersds[1+num_rands+r]*factor, Xsums[num_rands+r], hers[1+num_rands+r]/Xsums[num_rands+r]*1000000*factor, hersds[1+num_rands+r]/Xsums[num_rands+r]*1000000*factor);}
...
for(r=0;r<num_extra;r++){fprintf(output, "0\t%.6f\t", mg[r][i]*pow(factor,.5));}
fprintf(output, "\n");
}
fclose(output);
}
...
for(r=0;r<num_extra;r++){free(mg[r]);}free(mg);
for(r=0;r<num_extra;r++){free(mg2[r]);}free(mg2);
if(num_regs>0||num_tops>0)
{
