...
//see if any arguments are --help or --h
flag=0;
for(count=1;count<argc;count++)
{
if(strcmp(argv[count],"--help")==0||strcmp(argv[count],"--h")==0){flag=1;break;}
}

if(flag==1)
{
printf("Here is a list of main arguments (this message is shown because one of the arguments to LDAK is \"--help\", \"-help\", \"--h\" or \"-h\")\n\n");
...
printf("--calc-kins-direct <output> - construct a kinship matrix (GRM)\n\
--reml <output> - regress phenotype on one or more kinship matrices\n\
--fast-reml <output> - same as --reml, except much faster\n\
--calc-blups <output> - get BLUP effect size estimates (using results from --reml or --fast-reml)\n\
--he <output> - perform Haseman-Elston Regression (an alternative to --reml or --fast-reml)\n\
...
printf("--quant-her <output> - estimate family heritability for quantitative phenotypes\n\
--tetra-her <output> - estimate family heritability for binary phenotypes\n\n");
...
--mega-prs <output> - construct a prediction model from summary statistics using self-computed correlations\n\
--giga-prs <output> - construct a prediction model from summary statistics using pre-computed correlations\n\n");
...
printf("--filter <output> - filter samples based on relatedness\n\
...
printf("--calc-stats <output> - calculate allele frequencies, call-rates and info scores\n\
...
if(strcmp(argv[count],"--meta-analyse")==0)
{
if(mode!=-9999){print_empty(readstring,argv[count]);exit(1);}
mode=135;strcpy(outfile2,argv[count+1]);found=1;strcpy(readstring,argv[count]);
}
...
mode=140;sprintf(outfile2,"%s.step3",argv[count+1]);sprintf(prsfile2,"%s.step1",argv[count+1]);sprintf(assfile2,"%s.step2",argv[count+1]);amb=1;
...
{
if(mode!=-9999){print_empty(readstring,argv[count]);exit(1);}
mode=159;gigaprs=0;strcpy(outfile2,argv[count+1]);found=1;strcpy(readstring,argv[count]);
}

if(strcmp(argv[count],"--quick-prs")==0)
{printf("Sorry, \"--quick-prs\" no longer exists; please use instead \"--giga-prs\"\n\n");exit(1);}

if(strcmp(argv[count],"--giga-prs")==0)
{
if(mode!=-9999){print_empty(readstring,argv[count]);exit(1);}
mode=159;gigaprs=1;strcpy(outfile2,argv[count+1]);found=1;strcpy(readstring,argv[count]);
}
...
if(strcmp(argv[count],"--impute-summaries")==0)
{
if(mode!=-9999){print_empty(readstring,argv[count]);exit(1);}
...
if(strcmp(argv[count+1],"AUTOSOMES")==0){onechr=-1;}
if(strcmp(argv[count+1],"ODD")==0){onechr=-3;}
if(strcmp(argv[count+1],"EVEN")==0){onechr=-2;}
if(strcmp(argv[count+1],"X")==0){onechr=23;}
if(strcmp(argv[count+1],"Y")==0){onechr=24;}
if(onechr==-4)
{
if(sscanf(argv[count+1],"%d%c", &onechr, &readchar)!=1)
{printf("Error, %s should be a non-negative integer, AUTOSOMES (to use only Chr 1-22), X (to use only Chr 23), Y (to use only Chr 24), ODD or EVEN (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(onechr<=0)
{printf("Error, %s should be a non-negative integer, AUTOSOMES (to use only Chr 1-22), X (to use only Chr 23), Y (to use only Chr 24), ODD or EVEN (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
}
found=1;
...
if(strcmp(argv[count],"--split-par")==0)
{
xpar=-1;
if(strcmp(argv[count+1],"NONE")==0){xpar=0;}
if(strcmp(argv[count+1],"ALL")==0){xpar=1;}
if(strcmp(argv[count+1],"hg19")==0){xpar=19;}
if(strcmp(argv[count+1],"hg38")==0){xpar=38;}
if(xpar==-1){printf("Error, %s should be hg19, hg38, NONE or ALL (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
if(strcmp(argv[count],"--sumslist")==0)
{strcpy(sumslist2,argv[count+1]);found=1;}

if(strcmp(argv[count],"--bulk-summaries")==0)
{strcpy(bulkfile2,argv[count+1]);found=1;}

if(strcmp(argv[count],"--fixed-n")==0)
{printf("Sorry, it is no longer possible to use \"--fixed-n\" (you should instead ensure the summary statistic files contain a column labelled \"n)\"\n\n");exit(1);}
...
if(strcmp(argv[count],"--ascertainment2")==0)
{
if(sscanf(argv[count+1],"%lf%c", &ascer2, &readchar)!=1)
{printf("Error, %s should be a number within (0,1) (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(ascer2<=0||ascer2>=1){printf("Error, %s should a number within (0,1) (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
if(strcmp(argv[count],"--random-covar")==0)
{strcpy(rovarfile2,argv[count+1]);found=1;}

if(strcmp(argv[count],"--random-factors")==0)
{strcpy(ractorfile2,argv[count+1]);found=1;}
...
if(strcmp(argv[count],"--square")==0)
{
square=-1;
if(strcmp(argv[count+1],"YES")==0){square=1;}
if(strcmp(argv[count+1],"NO")==0){square=0;}
if(square==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
if(strcmp(argv[count],"--pathfile")==0)
...
if(strcmp(argv[count],"--truncate-fly")==0)
{
...
if(strcmp(argv[count],"--check-PRS")==0)
{
checkprs=-1;
if(strcmp(argv[count+1],"YES")==0){checkprs=1;}
if(strcmp(argv[count+1],"NO")==0){checkprs=0;}
if(checkprs==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
drm=-1;
if(strcmp(argv[count+1],"AUTOSOMES")==0){drm=1;}
if(strcmp(argv[count+1],"CHRX")==0){drm=2;}
if(drm==-1){printf("Error, %s should be AUTOSOMES or CHRX (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--sexfile")==0)
{strcpy(sexfile2,argv[count+1]);found=1;}
...
if(adjpreds==-1){printf("Error, %s should be YES, NO or PARTIAL (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
if(strcmp(argv[count],"--transform")==0)
...
if(strcmp(argv[count],"--cross-taglist")==0)
{strcpy(xaglist2,argv[count+1]);found=1;}
...
if(strcmp(argv[count],"--cross-tagfile")==0)
{strcpy(xagfile2,argv[count+1]);found=1;}

if(strcmp(argv[count],"--pathways")==0)
{strcpy(wayfile2,argv[count+1]);found=1;}
...
if(strcmp(argv[count],"--check-size")==0)
{
checksize=-1;
if(strcmp(argv[count+1],"YES")==0){checksize=1;}
if(strcmp(argv[count+1],"NO")==0){checksize=0;}
if(checksize==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--check-MCMC")==0)
{
checkmcmc=-1;
if(strcmp(argv[count+1],"YES")==0){checkmcmc=1;}
if(strcmp(argv[count+1],"NO")==0){checkmcmc=0;}
if(checkmcmc==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
if(strcmp(argv[count],"--overlapping-samples")==0)
{
oversamp=-1;
if(strcmp(argv[count+1],"YES")==0){oversamp=1;}
if(strcmp(argv[count+1],"NO")==0){oversamp=0;}
if(oversamp==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
if(verbose==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
if(multi==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
{printf("Sorry, it is no longer possible to use \"--grm-sparse\"; you should instead use \"--relatives\" (see www.dougspeed.com/relatives-files for details of the required file format)\n\n");exit(1);}
...
if(strcmp(argv[count],"--sub-step")==0)
{
if(strcmp(argv[count+1],"CUT")==0)
{substep=-1;}
else
{
if(strcmp(argv[count+1],"JOIN")==0)
{substep=0;}
else
{
if(sscanf(argv[count+1],"%d%c", &substep, &readchar)!=1)
{printf("Error, %s should be followed by \"START\" or a positive integer (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(substep<=0){printf("Error, %s should be followed by \"CUT\", \"JOIN\" or a positive integer (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
}
}
found=1;
}
...
if(strcmp(argv[count],"--betas")==0)
{strcpy(betafile2,argv[count+1]);found=1;}
...
if(strcmp(argv[count],"--use-annotations")==0)
{
useann=-1;
if(strcmp(argv[count+1],"YES")==0){useann=1;}
if(strcmp(argv[count+1],"NO")==0){useann=0;}
if(useann==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--conditional")==0)
{
condition=-1;
if(strcmp(argv[count+1],"YES")==0){condition=1;}
if(strcmp(argv[count+1],"NO")==0){condition=0;}
if(condition==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--fine-map")==0)
{
finemap=-1;
if(strcmp(argv[count+1],"YES")==0){finemap=1;}
if(strcmp(argv[count+1],"NO")==0){finemap=0;}
if(finemap==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--num-effects")==0)
{
if(sscanf(argv[count+1],"%d%c", &Lnum, &readchar)!=1)
{printf("Error, %s should be a positive integer (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(Lnum<=0){printf("Error, %s should be a positive integer (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--num-variances")==0)
{
if(sscanf(argv[count+1],"%d%c", &num_grids, &readchar)!=1)
{printf("Error, %s should be an integer greater than 1 (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(num_grids<=1){printf("Error, %s should be an integer greater than 1 (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--min-variance")==0)
{
if(sscanf(argv[count+1],"%lf%c", &grid_min, &readchar)!=1)
{printf("Error, %s should be should be a positive number (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(grid_min<=0){printf("Error, %s should be a positive number (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--max-variance")==0)
{
if(sscanf(argv[count+1],"%lf%c", &grid_max, &readchar)!=1)
{printf("Error, %s should be should be a positive number (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(grid_max<=0){printf("Error, %s should be a positive number (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--CS-coverage")==0)
{
if(sscanf(argv[count+1],"%lf%c", &csalpha, &readchar)!=1)
{printf("Error, %s should be a number within [0.5,1) (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(csalpha<0.5||csalpha>=1){printf("Error, %s should be a number within [0.5,1) (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--summary-imputation")==0)
{
impsums=-1;
if(strcmp(argv[count+1],"YES")==0){impsums=1;}
if(strcmp(argv[count+1],"NO")==0){impsums=0;}
if(impsums==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
if(strcmp(argv[count+1],"LDpred")==0){ptype=8;}
if(ptype==-1){printf("Error, %s should be \"lasso-sparse\", \"lasso\", \"ridge\", \"bolt\", \"bayesr\", \"bayesr-shrink\", \"--elastic\" or \"--mega\" (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--extra-parameters")==0)
{
exparam=-1;
if(strcmp(argv[count+1],"YES")==0){exparam=1;}
if(strcmp(argv[count+1],"NO")==0){exparam=0;}
if(exparam==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--ensemble")==0)
{
ensemble=-1;
if(strcmp(argv[count+1],"YES")==0){ensemble=1;}
if(strcmp(argv[count+1],"NO")==0){ensemble=0;}
if(ensemble==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--save-all-models")==0)
{
megasave=-1;
if(strcmp(argv[count+1],"YES")==0){megasave=1;}
if(strcmp(argv[count+1],"NO")==0){megasave=0;}
if(megasave==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
if(strcmp(argv[count],"--check-assignments")==0)
{
checkpops=-1;
if(strcmp(argv[count+1],"YES")==0){checkpops=1;}
if(strcmp(argv[count+1],"NO")==0){checkpops=0;}
if(checkpops==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
if(strcmp(argv[count],"--max-frequency-difference")==0)
{
if(sscanf(argv[count+1],"%lf%c", &maxfreq, &readchar)!=1)
{printf("Error, %s should be a positive number (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(maxfreq<=0){printf("Error, %s should be a positive number (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
{
resfix=-1;
if(strcmp(argv[count+1],"YES")==0){resfix=1;}
if(strcmp(argv[count+1],"NO")==0){resfix=0;}
if(resfix==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--min-her")==0)
{
if(sscanf(argv[count+1],"%lf%c", &minher, &readchar)!=1)
{printf("Error, %s should be a number within [0,1] (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(minher<0||minher>1){printf("Error, %s should be a number within [0,1] (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--MCMC-solve")==0)
{
mcmcfinal=-1;
if(strcmp(argv[count+1],"YES")==0){mcmcfinal=1;}
if(strcmp(argv[count+1],"NO")==0){mcmcfinal=0;}
if(mcmcfinal==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--MCMC-chains")==0)
{
if(sscanf(argv[count+1],"%d%c", &num_chains, &readchar)!=1)
{printf("Error, %s should be a positive integer (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(num_chains<=0){printf("Error, %s should be a positive integer (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--MCMC-burn")==0)
{
if(sscanf(argv[count+1],"%d%c", &num_burn, &readchar)!=1)
{printf("Error, %s should be a positive integer (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(num_burn<0){printf("Error, %s should be a non-negative integer (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--MCMC-iterations")==0)
{
if(sscanf(argv[count+1],"%d%c", &num_mcmc, &readchar)!=1)
{printf("Error, %s should be a positive integer (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(num_mcmc<=0){printf("Error, %s should be a positive integer (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
if(strcmp(argv[count],"--MCMC-step")==0)
{
if(sscanf(argv[count+1],"%d%c", &num_step, &readchar)!=1)
{printf("Error, %s should be a positive integer (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(num_step<=0){printf("Error, %s should be a positive integer (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--MTAG-correlation")==0)
{
if(sscanf(argv[count+1],"%lf%c", &mtagcor, &readchar)!=1)
{printf("Error, %s should be a non-negative number (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(mtagcor<0){printf("Error, %s should be a non-negative number (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--MTAG-percent")==0)
{
if(sscanf(argv[count+1],"%lf%c", &mtagpercent, &readchar)!=1)
{printf("Error, %s should be a non-negative number (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(mtagpercent<0){printf("Error, %s should be a non-negative number (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--MTAG-meta")==0)
{
mtagmeta=-1;
if(strcmp(argv[count+1],"YES")==0){mtagmeta=1;}
if(strcmp(argv[count+1],"NO")==0){mtagmeta=0;}
if(mtagreg==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--MTAG-regional")==0)
{
mtagreg=-1;
if(strcmp(argv[count+1],"YES")==0){mtagreg=1;}
if(strcmp(argv[count+1],"NO")==0){mtagreg=0;}
if(mtagreg==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--MTAG-force-all")==0)
{
mtagforce=-1;
if(strcmp(argv[count+1],"YES")==0){mtagforce=1;}
if(strcmp(argv[count+1],"NO")==0){mtagforce=0;}
if(mtagforce==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--MTAG-copy")==0)
{
mtagcopy=-1;
if(strcmp(argv[count+1],"YES")==0){mtagcopy=1;}
if(strcmp(argv[count+1],"NO")==0){mtagcopy=0;}
if(mtagcopy==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
if(strcmp(argv[count],"--multi-scorefile")==0)
{strcpy(morefile2,argv[count+1]);found=1;}
...
if(strcmp(argv[count],"--GxE")==0)
{
gxe=-1;
if(strcmp(argv[count+1],"YES")==0){gxe=1;}
if(strcmp(argv[count+1],"NO")==0){gxe=0;}
if(gxe==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--odd-half-her")==0)
{
oddhalf=-1;
if(strcmp(argv[count+1],"YES")==0){oddhalf=1;}
if(strcmp(argv[count+1],"NO")==0){oddhalf=0;}
if(oddhalf==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
{printf("Sorry, \"--AUC\" has been replaced by \"--binary\"\n\n");exit(1);}
...
if(strcmp(argv[count],"--exclude-inconsistent")==0)
{
exincons=-1;
if(strcmp(argv[count+1],"YES")==0){exincons=1;}
if(strcmp(argv[count+1],"NO")==0){exincons=0;}
if(exincons==-1){printf("Error, %s should be YES or NO (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
if(strcmp(argv[count],"--resample")==0)
{
if(sscanf(argv[count+1],"%d%c", &resample, &readchar)!=1)
{printf("Error, %s should be a positive integer (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(resample<=0){printf("Error, %s should be a positive integer (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}

if(strcmp(argv[count],"--recombination-rate")==0)
{
if(sscanf(argv[count+1],"%lf%c", &recomb, &readchar)!=1)
{printf("Error, %s should be a number within [0,1] (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(recomb<0||recomb>1){printf("Error, %s should be a number within [0,1] (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
{
if(sscanf(argv[count+1],"%lf%c", &cutoff2, &readchar)!=1)
{printf("Error, %s should be a positive number (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
if(cutoff2<=0){printf("Error, %s should be a positive number (not %s)\n\n", argv[count], argv[count+1]);exit(1);}
found=1;
}
...
#if MET==2
if(mode==101||mode==102||mode==103||mode==104||mode==105)
{printf("Error, this version of LDAK was not compiled with QSOPT, so can not be used to calculate weightings\n\n");exit(1);}
#endif
