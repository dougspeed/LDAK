...
pedeffs=malloc(sizeof(double*)*num_resps_use);
...
if(verbose==1||substep==-1)	//save estimate
{
...
//decide whether high structure and/or if using pedigree PRS - note that useped is currently set to zero
...
if(dougvar==102){exit(1);}

////////

if(useped==1)	//construct the pedigree prs - if dichot=1, will use alt logistic (can not be here if using substep)
{
...
if(strcmp(relfile,"blank")==0)	//need to find relatives
{
...
relats[num_rels]=kins[(size_t)i2+(i2-bitstart)*num_samples_use];
num_rels++;
...
relats[num_rels]=kins[(size_t)i+(i2-bitstart)*num_samples_use];
num_rels++;
}
}
}
...
//adjust off-diagonals for winners curse
#pragma omp parallel for private(i) schedule(static)
for(i=0;i<num_rels;i++)
{
if(firsts[i]!=seconds[i]){relats[i]=secant_trun(relats[i], pow(var,.5), value, 0.01, 50, 0);}
}
}
else	//read relatives from file
{
printf("Reading details for %d related pairs from %s\n", maxpairs, relfile);
num_rels=read_relfile(relfile, maxpairs, firsts, seconds, relats, NULL, num_samples_use, ids3, 1);
}

if(strcmp(sampwfile,"blank")!=0)	//scale kinships by sweights - not currently used
{
for(i=0;i<num_rels;i++){relats[i]*=sweights[firsts[i]]*sweights[seconds[i]];}
}
...
/*
sprintf(filename3,"%s.pairs", outfile);
if((output3=fopen(filename3,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename3);exit(1);}
for(i=0;i<num_rels;i++){fprintf(output3,"%s %s %s %s %f\n", ids1[firsts[i]], ids2[firsts[i]],  ids1[seconds[i]], ids2[seconds[i]], relats[i]);}
fclose(output3);
...
sparse_cgd(num_samples_use, total, num_rels, firsts, seconds, relats, cX, cR, num_resps_use, NULL, Yadj, dichot, nullweights, 1, 0.0001, tryhers[j], polates+j*num_resps_use, nmcmc, gaussian, -9999, NULL, NULL, NULL, NULL, NULL, -9999, NULL, NULL, NULL, NULL);
}
...
sparse_cgd(num_samples_use, total, num_rels, firsts, seconds, relats, cX, cR, num_resps_use, pedhers, Yadj, dichot, nullweights, 2, 0.0001, -9999, NULL, -9999, NULL, ncal, cdata, cmults, pedgammas, pedsds, pedeffs, -9999, NULL, NULL, NULL, NULL);

//extract prs (if excluding the PRS, set to zero and ensure scaling is 1)
...
if(dichot==0)	//prs is Y - invV Y (1-h2) - remember to scale - becomes sample-specific if using sample weights 
{
value=pow(pedscales[m],-1);
for(i=0;i<num_samples_use;i++)
{
if(strcmp(sampwfile,"blank")!=0){value=pow(pedscales[m]*sweights[i],-1);}   //not currently used

pedprs[i+m*num_samples_use]=(Yadj[i+m*num_samples_use]-cX[(size_t)p*num_samples_use+i]*(1-pedhers[m]))*value;
}
}
...
if(sumsq!=sumsq||isnan(sumsq)){printf("i %d has %f %f her %f\n", i+1, Yadj[keeptest[i]+m*num_samples_use], cX[(size_t)p*num_samples_use+keeptest[i]], pedhers[m]);exit(1);}
}
...
if(fastgwa==0)	//get best power - currently must have dichot=1
{
if(power!=-9999)
{
...
fprintf(output3,"Multivariate NO\n");
fclose(output3);

for(m=0;m<num_resps_use;m++)	//save LOCO PRS and details (have already scaled if necessary)
{
...
if(pedhers[m]==0)	//prs are zero
{
for(i=0;i<num_samples_use;i++){fprintf(output3, "%s %s 0\n", ids1[i], ids2[i]);}
}
else
{
for(i=0;i<num_samples_use;i++){fprintf(output3, "%s %s %.4f\n", ids1[i], ids2[i], pedprs[i+m*num_samples_use]);}
}
fclose(output3);
...
fprintf(output3,"Effect_Calibration %.4f\n", pedeffs[m]);
...
free(tryhers);
free(pedtops);free(pedscales);free(pedhers);free(pedprs);free(pedgammas);free(pedsds);free(pedeffs);free(pedmse);
