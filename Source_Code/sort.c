...
//type=0 - quiet (just get order), type=1 - error, type=2 - warn
{
int j, wcount;
char *rs;
...
if(type==2)
{
if(wcount<5){printf("Warning, %s appears twice in %s\n", sptrs[j].ptr, filename);}
wcount++;
}
}
}

if(wcount>5){printf("In total, %d predictors appear twice in %s\n", wcount, filename);}
if(wcount>0)
{
rs=malloc(sizeof(char)*1000000);
printf("Will append an index to the names to ensure they are unique\n\n");
for(j=0;j<length;j++)
{
strcpy(rs,str[j]);
free(str[j]);
str[j]=malloc(sizeof(char)*(strlen(rs)+10));
sprintf(str[j], "%s_%d", rs, j+1);
}
free(rs);
}
...
//note that if duplicates in stra, then elements of strb can be found multiple times
//predordera and predorderb might contain -1s, in which case the search should stop
{
int a, b, count;
...
for(a=0;a<lengtha;a++){founds[a]=-1;}
