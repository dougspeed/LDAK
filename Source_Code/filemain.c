...
{
int j, count, bcount, lcount, mcount, ncount, scount, tcount;

char *rc, *rs, *rm, *rbp, *ra1, *ra2;

FILE *input;

rc=malloc(sizeof(char)*1000000);rs=malloc(sizeof(char)*1000000);
rm=malloc(sizeof(char)*1000000);rbp=malloc(sizeof(char)*1000000);
ra1=malloc(sizeof(char)*1000000);ra2=malloc(sizeof(char)*1000000);


if((input=fopen(bimfile,"r"))==NULL)
{printf("Error opening %s\n\n",bimfile);exit(1);}

bcount=0;mcount=0;ncount=0;scount=0;tcount=0;lcount=0;
for(j=0;j<length;j++)
{
...
if(strcmp(ra1,ra2)==0)	//exclude predictors with matching alleles - alleles will be redundant
{
if(type==1)
{
if(tcount<5){printf("Warning, Predictor %s has matching alleles (%s) and will be ignored\n", preds[j], ra1);}
tcount++;
}
bp[j]=-1;
strcpy(ra1,"X");
...
{
if(exlong==1)	//not allowed long alleles - alleles will be redundant
{
...
if(tcount>5){printf("In total, %d predictors have matching alleles\n", tcount);}
...
int read_bimfile_lite(char *bimfile, int *chr, char **preds, double *bp, char *al1, char *al2, int length)
//assumes bimfile created by LDAK, so performs few checks
{
int j, scount;
char *rs, *rm;
...
rs=malloc(sizeof(char)*1000000);rm=malloc(sizeof(char)*1000000);
...
for(j=0;j<length;j++)
{
if(fscanf(input, "%d %s %s %lf %c %c ", chr+j, rs, rm, bp+j, al1+j, al2+j)!=6)
{printf("Error reading Row %d of %s\n\n", j+1, bimfile);exit(1);}

//save name
copy_string(preds,j,rs);

if(j>0)	//check for duplicates
{
if(chr[j]==chr[j-1]&&bp[j]==bp[j-1]){scount++;}
}
}	//end of j loop
fclose(input);

if(scount==1){printf("Warning, one pair of predictors in %s has the same basepair\n", bimfile);}
if(scount>1){printf("Warning, %d pairs of predictors in %s have the same basepair\n", scount, bimfile);}
...
char *rc, *rs, *ra1, *ra2, *rr;
char *buffer, *buffptr;
size_t size, remaining, index;
short idlen, readshort;
int eof, offset, readint,  allen, next;
unsigned int readuns;
...
if(remaining<1000000&&eof!=1) //reload buffer (probably very conservative threshold, but not a problem)
{
if(remaining>0)  //still have some non-processed data
{memmove(buffer, buffptr, remaining);index+=buffptr-buffer;}
remaining+=fread(buffer+remaining, 1, size-remaining, input);
buffptr=buffer;
eof=(remaining<size);
}
...
//if(fread(&readint, 4, 1, input)!=1)
{printf("Error reading first value for Predictor %d of %s\n\n", j+1, bgenfile);exit(1);}
readint=*(int *)buffptr;buffptr+=4;remaining-=4;
if(readint!=count2){printf("Error, %s seems to be corrupted (the numbers of samples are not consistent) - %d and %d\n\n", bgenfile, count2, readint);exit(1);}
}
...
//if(fread(&idlen, 2, 1, input)!=1)
{printf("Error reading second value for Predictor %d of %s\n\n", j+1, bgenfile);exit(1);}
idlen=*(short  *)buffptr;buffptr+=2;remaining-=2;
if(remaining<idlen)
//if(fread(rs, 1, idlen, input)!=idlen)
{printf("Error reading third value for Predictor %d of %s\n\n", j+1, bgenfile);exit(1);}
memcpy(rs, buffptr, idlen);buffptr+=idlen;remaining-=idlen;
...
//if(fread(rc, 1, idlen, input)!=idlen)
{printf("Error reading seventh value for Predictor %d of %s\n\n", j+1, bgenfile);exit(1);}
memcpy(rc, buffptr, idlen);buffptr+=idlen;remaining-=idlen;
rc[idlen]='\0';
...
//if(fread(&readuns, 4, 1, input)!=1)
{printf("Error reading eighth value for Predictor %d of %s\n\n", j+1, bgenfile);exit(1);}
readuns=*(unsigned int  *)buffptr;buffptr+=4;remaining-=4;
...
//if(fread(&allen, 4, 1, input)!=1)
{printf("Error reading first allele for Predictor %d of %s\n\n", j+1, bgenfile);exit(1);}
allen=*(int *)buffptr;buffptr+=4;remaining-=4;
if(remaining<allen)
//if(fread(ra1, 1, allen, input)!=allen)
{printf("Error reading first allele for Predictor %d of %s\n\n", j+1, bgenfile);exit(1);}
memcpy(ra1, buffptr, allen);buffptr+=allen;remaining-=allen;
ra1[allen]='\0';
...
//if(fread(ra2, 1, allen, input)!=allen)
{printf("Error reading second allele for Predictor %d of %s\n\n", j+1, bgenfile);exit(1);}
memcpy(ra2, buffptr, allen);buffptr+=allen;remaining-=allen;
ra2[allen]='\0';
}
else	//can be more (or less?) than two alleles
{
...
//if(fread(&readshort, 2, 1, input)!=1)
{printf("Error reading ninth value for Predictor %d of %s\n\n", j+1, bgenfile);exit(1);}
readshort=*(short  *)buffptr;buffptr+=2;remaining-=2;
...
for(k=2;k<readshort;k++)	//read/skip the remaining alleles (will not use)
{
if(remaining<4)
//if(fread(&allen, 4, 1, input)!=1)
{printf("Error reading second allele for Predictor %d of %s\n\n", j+1, bgenfile);exit(1);}
buffptr+=4;remaining-=4;
if(remaining<allen)
//if(fread(rr, 1, allen, input)!=allen)
{printf("Error reading Allele %d for Predictor %d of %s\n\n", k+1, j+1, bgenfile);exit(1);}
buffptr+=allen;remaining-=allen;
}
}

//save the file location
//bgen_indexes[j]=ftello(input);
bgen_indexes[j]=index+buffptr-buffer;

//work out how far to next variant
if(bgen_layout==1)	//for layout 1, length depends on compression
{
if(bgen_comp==0)	//must skip 6n
{next=6*count2;}
...
//if(fread(&readint, 4, 1, input)!=1)
{printf("Error reading storage size for Predictor %d of %s\n\n", j+1, bgenfile);exit(1);}
next=*(int *)buffptr;buffptr+=4;remaining-=4;
}
}
else	//read length to skip (does not depend on compression)
{
...
//skip to next
if(remaining>next)  //have already read
{buffptr+=next;remaining-=next;}
else    //have not read
{fseeko(input, next-remaining, SEEK_CUR);remaining=0;index+=buffptr-buffer+next;}
...
//exlong=0 - allowed long alleles, exlong=1 - not allowed
{
int j, lcount, ncount, scount, tcount, size;
...
rs=malloc(sizeof(char)*1000000);rs2=malloc(sizeof(char)*1000000);
rbp=malloc(sizeof(char)*1000000);ra1=malloc(sizeof(char)*1000000);ra2=malloc(sizeof(char)*1000000);
...
j=0;ncount=0;scount=0;tcount=0;lcount=0;
...
if(count3<length)
{printf("Warning, %s contains weights for only %d of the %d predictors; the remainder will be given weight zero\n\n", weightsfile, count3, length);}
...
if(count2<length)
{printf("Warning, %s contains infos for only %d of the %d predictors; the remainder will be given info zero\n\n", infosfile, count2, length);}
...
int check_sformat(char *sumsfile)
{
int count;
...
{printf("Error, %s should have a column named \"Predictor\"\n\n", sumsfile);exit(1);}
if(find_head("A1", sumsfile, count)==-1)
{printf("Error, %s should have a column named \"A1\"\n\n", sumsfile);exit(1);}
if(find_head("A2", sumsfile, count)==-1)
{printf("Error, %s should have a column named \"A2\"\n\n", sumsfile);exit(1);}

if(find_head("Z", sumsfile, count)==-1&&(find_head("BETA", sumsfile, count)==-1||find_head("SE", sumsfile, count)==-1)&&(find_head("b", sumsfile, count)==-1||find_head("se", sumsfile, count)==-1))
{
if((find_head("Stat", sumsfile, count)==-1&&find_head("P", sumsfile, count)==-1)||find_head("Direction", sumsfile, count)==-1){printf("Error, %s should have a column named \"Z\", two columns named \"BETA\" and \"SE\", two columns named \"Stat\" and \"Direction\", or two columns named \"P\" and \"Direction\"\n\n", sumsfile);exit(1);}
}

if(find_head("n", sumsfile, count)==-1&&find_head("N", sumsfile, count)==-1)
{printf("Error, %s should have a column named \"n\"\n\n", sumsfile);exit(1);}

if(find_head("A1Freq", sumsfile, count)!=-1||find_head("freq", sumsfile, count)!=-1){return(1);}
else{return(0);}
}

////////

void read_bulkfile(char *bulkfile, int num_sums, double **Mnss, double **Mchis, double **Mrhos, double **Ma1freq, int length, char **preds, char *al1, char *al2, char *filename)
{
int j, j2, j3, q, count, count2, count3, found, ccount, lcount;
int *indexer, *indexer2;

float *values1, *values2, *a1freqb;
char **gotpreds, *gotal1, *gotal2;

float readfloat;
char readchar, *rs, *rs2, *rs3;

FILE *input;

rs=malloc(sizeof(char)*1000000);
rs2=malloc(sizeof(char)*1000000);
rs3=malloc(sizeof(char)*1000000);


//read all summaries, doing some basic checks

count=countrows_plus(bulkfile,4+2*num_sums)-1;
gotpreds=malloc(sizeof(char*)*count);
gotal1=malloc(sizeof(char)*count);
gotal2=malloc(sizeof(char)*count);
values1=malloc(sizeof(float)*count*num_sums);
values2=malloc(sizeof(float)*count*num_sums);
a1freqb=malloc(sizeof(float)*count);
...
for(j=0;j<count;j++)
{
if(fscanf(input, "%s %s %s %f ", rs, rs2, rs3, &readfloat)!=4)
{printf("Error reading first four elements of Row %d of %s\n\n", j+2, bulkfile);exit(1);}

if(strlen(rs2)+strlen(rs2)>2)	//long alleles - ignore
{
if(lcount<5){printf("Warning, Predictor %s has multi-character alleles (%s and %s) so will be ignored\n", rs, rs2, rs3);}
lcount++;
readchar=0;while(readchar!=10){readchar=10;(void)fscanf(input, "%c", &readchar);}
...
if(readfloat<0||readfloat>1)
{printf("Error, the frequency for Predictor %s should be within [0,1] (not %.6f)\n\n", rs, readfloat);exit(1);}

//copy in names, alleles and frequency
copy_string(gotpreds,found,rs);
gotal1[found]=rs2[0];
gotal2[found]=rs3[0];
a1freqb[found]=readfloat;

for(q=0;q<num_sums;q++)
{
if(fscanf(input, "%f %f ", values1+found+q*count, values2+found+q*count)!=2)
{printf("Error reading the statistics for Trait %d in Row %d of %s\n\n", q+1, j+2, bulkfile);exit(1);}
if(values1[found+q*count]<0){printf("Error reading %s; the sample size for Trait %d in Row %d should be non-negative (not %.6f)\n\n", bulkfile, q+1, j+1, values1[found+q*count]);exit(1);}
}
found++;
}
}	//end of j loop
...
indexer=malloc(sizeof(int)*length);
indexer2=malloc(sizeof(int)*length);
count2=find_strings(preds, length, gotpreds, found, indexer, indexer2, NULL, bulkfile, NULL, NULL, 3);
...
else	//alleles consistent, so can use
{
for(q=0;q<num_sums;q++)
{
Mnss[q][j2]=values1[j3+q*count];
Mchis[q][j2]=pow(values2[j3+q*count],2);
if(values2[j3+q*count]>0){Mrhos[q][j2]=pow(Mchis[q][j2]/(Mchis[q][j2]+Mnss[q][j2]),.5);}
else{Mrhos[q][j2]=-pow(Mchis[q][j2]/(Mchis[q][j2]+Mnss[q][j2]),.5);}
Ma1freq[q][j2]=a1freqb[j3];
if(al1[j2]!=gotal1[j3]){Mrhos[q][j2]*=-1;Ma1freq[q][j2]=1-a1freqb[j3];}
}
}
count3++;
}
...
if(count3==0){printf("\nError, %s contains summary statistics for none of the %d predictors\n\n", bulkfile, length);exit(1);}
if(count3<length){printf("Warning, %s contains summary statistics for only %d of the %d predictors\n", bulkfile, count3, length);}
...
free(gotal1);free(gotal2);free(values1);free(values2);free(a1freqb);
free(indexer);free(indexer2);
}

////////

void read_sumsfile(char *sumsfile, double *nss, double *chis, double *rhos, double *a1freq, int length, char **preds, char *al1, char *al2, char *filename, int amb, double scaling, int checksums)
{
//checksums=0 - allowed missing predicitons; checksums=1 - not allowed, but user can change
//checksums=-9999 - never allowed, checksums=2 - kvikstep3 not trait 1, checksums=3 - megaprs not trait 1
...
cols[3]=-9999;
cols[5]=-9999;
if(find_head("Z", sumsfile, count2)!=-1){cols[3]=find_head("Z", sumsfile, count2);cols[5]=cols[3];zsp=0;}
else
{
if(find_head("BETA", sumsfile, count2)!=-1){cols[3]=find_head("SE", sumsfile, count2);cols[5]=find_head("BETA", sumsfile, count2);zsp=1;}
else
{
if(find_head("b", sumsfile, count2)!=-1){cols[3]=find_head("se", sumsfile, count2);cols[5]=find_head("b", sumsfile, count2);zsp=1;}
else
{
...
{
if(find_head("Stat", sumsfile, count2)!=-1){cols[3]=find_head("P", sumsfile, count2);cols[5]=find_head("Direction", sumsfile, count2);zsp=3;}
}}}}

if(find_head("n", sumsfile, count2)!=-1){cols[4]=find_head("n", sumsfile, count2);}
else{cols[4]=find_head("N", sumsfile, count2);}

if(cols[0]==-9999){printf("Error 23XXGa, please tell doug (%d)\n\n", cols[0]);}
if(cols[1]==-9999){printf("Error 23XXGb, please tell doug (%d)\n\n", cols[1]);}
if(cols[2]==-9999){printf("Error 23XXGc, please tell doug (%d)\n\n", cols[2]);}
if(cols[3]==-9999){printf("Error 23XXGd, please tell doug (%d)\n\n", cols[3]);}
if(cols[4]==-9999){printf("Error 23XXGe, please tell doug (%d)\n\n", cols[4]);}
if(cols[5]==-9999){printf("Error 23XXGf, please tell doug (%d)\n\n", cols[5]);}

cols[6]=-9999;
if(find_head("A1Freq", sumsfile, count2)!=-1){cols[6]=find_head("A1Freq", sumsfile, count2);}
else
{
if(find_head("freq", sumsfile, count2)!=-1){cols[6]=find_head("freq", sumsfile, count2);}
}
...
rhos[j2]=signsb[j3]*pow(chisb[j3]/(chisb[j3]+nssb[j3]),.5);	//should really adjust denom for num covar in gwas
...
if(amb==1&&acount>0){printf("Warning, %d predictors have ambiguous alleles\n", acount);}
...
{
if(checksums==2)
{printf("\nError, %s contains summary statistics for only %d of the %d predictors; this should not happen, please can you tell Doug\n\n", sumsfile, count3, length);exit(1);}
...
{
if(checksums!=3){printf("Have found summary statistics for all %d predictors\n", length);}
else{printf("%s contains summary statistics for all %d predictors\n", sumsfile, length);}
}
...
void read_genefile(char *genefile, char **gnames, int *gchr, double *gbp1, double *gbp2, int *gstrand, int type)
//type=0 - duplicates allowed, type=1 - not allowed
{
...
rs=malloc(sizeof(char)*1000000);rc=malloc(sizeof(char)*1000000);
rbp1=malloc(sizeof(char)*1000000);rbp2=malloc(sizeof(char)*1000000);
...
//check whether names are unique (if not, append index)
if(type==0){check_dups(gnames, count, genefile, NULL, 2);}
else{check_dups(gnames, count, genefile, NULL, 1);}
...
void read_centres(char *centresfile, double *centres, int length, char **preds, char *al1, char *al2, char *bimfile, int type)
//type=0 - read a1 freq, type=2 - read mafs
{
...
if(count2==0)
{
if(type==0){printf("Error, %s does not contain centres for any of the %d predictors\n\n", centresfile, length);exit(1);}
else{printf("Error, %s does not contain MAFs for any of the %d predictors\n\n", centresfile, length);exit(1);}
}
if(count2<length)
{
if(type==0){printf("Error, %s contains centres for only %d of the %d predictors\n", centresfile, count2, length);exit(1);}
else{printf("Warning, %s contains MAFs for only %d of the %d predictors; the remainder will be assumed to have MAF zero\n", centresfile, count2, length);}
}

if(type==1) //set centres to zero, in case some missing)
for(j=0;j<length;j++){centres[j]=0;}
...
if(ra1[0]==ra2[0])
{printf("Error reading %s; both alleles for Predictor %s are %c\n\n", centresfile, rs, ra1[0]);exit(1);} 
...
{printf("Error reading %s; the centre on Row %d does not appear to be numeric (%s)\n\n", centresfile, head+j+1, readstring);exit(1);}
if(value!=value||isinf(value))
{printf("Error reading %s; the centre on Row %d does not appear to be numeric (%s)\n\n", centresfile, head+j+1, readstring);exit(1);}
}
else
{
if(strcmp(readstring,"NA")==0)
{printf("Error reading %s; MAF on Row %d is NA\n\n", centresfile,  head+j+1);exit(1);}
if(sscanf(readstring, "%lf%c", &value, &readchar)!=1)
{printf("Error reading %s; the MAF on Row %d does not appear to be numeric (%s)\n\n", centresfile, head+j+1, readstring);exit(1);}
if(value!=value||isinf(value))
{printf("Error reading %s; the MAF on Row %d does not appear to be numeric (%s)\n\n", centresfile, head+j+1, readstring);exit(1);}

if(value<0||value>1)
{printf("Error reading %s; the MAF on Row %d is not within [0,1] (%s)\n\n", centresfile, head+j+1, readstring);exit(1);}
}
...
{
if(value<0.5){centres[j2]=value;}
else{centres[j2]=1-value;}
}
...
{printf("Warning, %s contains (positive) values for only %d of the %d predictors; the remainder will be given probabilty zero\n", probsfile, count4, num_preds_use);}
...
if(count2<length)
{printf("Warning, %s contains heritabilities for only %d of the %d predictors; the remainder will be given heritability zero\n\n", indhers, count2, length);}
for(j=0;j<length;j++){weights[j]=0;}
...
if(resptemp[m]!=resptemp[m]||isinf(resptemp[m]))
{printf("Error reading %s; Element %d of Row %d does not appear to be numeric (%s)\n\n", respfile, 3+m, head+i+1, readstring);exit(1);}
}
}
...
//type=0 - reading phenotypes normally, type=1/2 - reading first/second of a pair of phenotypes, type=3 - reading transformed phenotypes
{
...
if(type==3){printf("Reading transformed phenotypes for %d samples from %s\n", count, respfile);}
...
if(count2==0){printf("Error, can not find any of these samples\n\n");exit(1);}
if(type==3&&count2<ns){printf("Error, can only find %d of the %d samples, suggesting the file has been changed since creation with \"--ridge\", \"--bolt\", \"--bayesr\" or \"--elastic\"\n\n", count2, ns);exit(1);}
...
if(type!=3)	//check whether binary
{
for(m=0;m<num_resps_use;m++)
{
n0=0;n1=0;n2=0;nm=0;
...
if(type!=3)	//get counts and check variances
{
count=0;
for(m=0;m<num_resps_use;m++)
{
...
if(indcount==0){printf("Error reading %s; Phenotype %d has no non-missing values\n\n", respfile, keepresps[m]+1);exit(1);}
...
{
if(num_resps_use==1){printf("There are %d missing values\n", ns-indcount);}
else
{
if(m<10){printf("There are %d missing values for Phenotype %d \n", ns-indcount, m+1);}
}
count++;
}
}	//end of m loop
...
void read_sexfile(char *sexfile, int *sexes, int ns, char **ids1, char **ids2, char **ids3)
{
int i, i2, i3, count, count2, count3, head;
double *sexesb;
...
//have already checked that file either has a header (with one name sex) or three columns
head=check_head_ids(sexfile,0);

if(head==0){count3=2;}
else{count3=find_head("Sex", sexfile, -1);}

count=countrows(sexfile)-head;
printf("Reading sex for %d samples from %s\n", count, sexfile);

wantids=malloc(sizeof(char*)*count);
indexer=malloc(sizeof(int)*count);
indexer2=malloc(sizeof(int)*count);
sexesb=malloc(sizeof(double)*count);

read_ids(sexfile, NULL, NULL, wantids, count, NULL, head, 0);
read_values(sexfile, sexesb, count, NULL, count3+1, head, 1);
count2=find_strings(wantids, count, ids3, ns, indexer, indexer2, sexfile, NULL, NULL, NULL, 3);
if(count2==0){printf("Error, can not find any of these samples\n\n");exit(1);}
if(count2<count){printf("Warning, can only find %d of these samples\n\n", count2);}

//set all sexes to missing
for(i=0;i<ns;i++){sexes[i]=-9999;}
...
i2=indexer[i];
i3=indexer2[i];
if(sexesb[i2]!=1&&sexesb[i2]!=2&&sexesb[i2]!=-9999){printf("Error, %s %s has value %.4f; sex should be coded as 1 (male) or 2 (female)\n\n", ids1[i3], ids2[i3], sexesb[i2]);exit(1);
}
sexes[i3]=(int)sexesb[i2];
}

count2=0;for(i=0;i<ns;i++){count2+=(sexes[i]!=-9999);}
if(count2<ns){printf("Warning, %s only provides sex for %d of the %d samples\n\n", sexfile, count2, ns);}

for(i=0;i<count;i++){free(wantids[i]);}free(wantids);
free(indexer);free(indexer2);free(sexesb);
}	//end of read_sexfile

////////

void read_covarfile(char *covarfile, double *covar, int ns, char **ids3, int num_covars, int *keepcovars, double missingvalue, int type)
//type=0 - fixed effects (check colinearity), type=1 - random effects (no check, but centre)
//type=2 - just read (will then filter out missing)
{
int i, j, j2, count, count2, count3, found, indcount, head;
...
//see which individuals are available (for types 0/1, require all to be present)
head=check_head_ids(covarfile,0);
count=countrows(covarfile)-head;
if(type==0||type==1){printf("Reading %d covariates for %d samples from %s\n", num_covars, count, covarfile);}
else{printf("Checking %d covariates for %d samples from %s\n", num_covars, count, covarfile);}
...
if((type==0||type==1)&&count2<ns){printf("Error, %s only contains covariates for %d of the %d samples (you should either edit the file or use \"--keep\" or \"--remove\" to exclude samples without covariates)\n\n", covarfile, count2, ns);exit(1);}

if(type==2) //set all covar to missing
{
...
if(i==indexer[found])	//using this row
{
for(j=0;j<count3;j++)
{
if(fscanf(input, "%s ", readstring)!=1)
{printf("Error reading Element %d of Row %d of %s\n\n", 3+j, head+i+1, covarfile);exit(1);}
if(strcmp(readstring,"NA")==0){readdouble=missingvalue;}
else
{
...
if(readdouble!=readdouble||isinf(readdouble))
{printf("Error reading %s; Element %d of Row %d does not appear to be numeric (%s); if the covariates are categorical, you should use \"--factors\" instead of \"--covar\"\n\n", covarfile, 3+j, head+i+1, readstring);exit(1);}
}

if(keepcovars[j]!=-1)	//using this covariate
{covar[indexer2[found]+keepcovars[j]*ns]=readdouble;}
}
found++;
if(found==count2){break;}
}
...
if(type==0||type==1)    //keep going
{
//get counts, means and variances, then set missing to mean and fill stands, maybe compute correlations
...
if(type==1)	//centre
{
printf("Random effect %d: subtract %f\n", j+1, mean);
for(i=0;i<ns;i++){covar[i+j*ns]-=mean;}
}
}
...
if(covar[indexer2[count2]+j*ns]!=covar[indexer2[count2]+j*ns]||isinf(covar[indexer2[count2]+j*ns]))
{printf("Error reading %s; Element %d of Row %d does not appear to be numeric (%s)\n\n", envfile, 3+j, head+i+1, readstring);exit(1);}
}
}
count2++;
...
{
//type=0, count factors (num_cats=-9999), type=1, read factors (have num_cats), type=2, check missing
int i, j, count, count2, found, count3, head;
char **factors;
...
if((type==0||type==1)&&count2<ns){printf("Error, %s only contains factors for %d of the %d samples (you should either edit the file or use \"--keep\" or \"--remove\" to exclude samples without factors)\n\n", factorfile, count2, ns);exit(1);}

if(type==2) //set all factors to missing
{
...
//read all factors
factors=malloc(sizeof(char*)*ns*count3);
...
copy_string(factors, indexer2[found]+j*ns, rs);
}
found++;
if(found==count2){break;}
}
...
if(type==2) //covar indicates which are missing
{
for(j=0;j<count3;j++)
{
for(i=0;i<ns;i++){covar[i+j*ns]=strcmp(factors[i+j*ns],"NA");}
}
}
...
for(i=1;i<ns;i++)
{
if(strcmp(sptrs[i-1].ptr,sptrs[i].ptr)!=0){found++;}
if(type==1&&found>0){covar[sptrs[i].index+(count2+found-1)*ns]=1;}
}

if(found>ns/2){printf("Error, Factor %d has too many distinct values (%d); should the factor instead be included as a quantitative covariate? (i.e., provided using \"--covar\" instead of \"--factors\")\n\n", j+1, 1+found);exit(1);}

if(j<10&&type==0)
{
if(found==0){printf("Warning, Factor %d has the same value for all samples\n", j+1);}
...
for(i=0;i<count;i++){free(wantids[i]);}free(wantids);
free(indexer);free(indexer2);
for(i=0;i<count3*ns;i++){free(factors[i]);}free(factors);
...
if(count2==0){printf("Error, can not find any of these samples\n\n");exit(1);}
if(count2<ns){printf("Error, %s only contains weightings for %d of the %d samples (if this is intentional, use \"--keep\" or \"--remove\" to exclude samples without weightings from the analysis)\n\n", sampwfile, count2, ns);exit(1);}
...
int read_relfile(char *relfile, int maxpairs, int *firsts, int *seconds, double *relats, double *relats2, int ns, char **ids3, int type)
//type=0 - tetraher/quanther, type=1 - fastgwa
{
int i, count, num_rels;

int *indexer, *indexer2, *usedids, *usedids2, *idsorder;
char **wantids, **wantids2;
double *readdoubles, *readdoubles2;
struct sorting_string *sptrs;


indexer=malloc(sizeof(int)*maxpairs);
indexer2=malloc(sizeof(int)*maxpairs);
usedids=malloc(sizeof(int)*maxpairs);
usedids2=malloc(sizeof(int)*ns);
idsorder=malloc(sizeof(int)*ns);

wantids=malloc(sizeof(char*)*maxpairs);
wantids2=malloc(sizeof(char*)*maxpairs);
readdoubles=malloc(sizeof(double)*maxpairs);
readdoubles2=malloc(sizeof(double)*maxpairs);

//get idsorder
sptrs=malloc(sizeof(struct sorting_string)*ns);
for(i=0;i<ns;i++){sptrs[i].ptr=ids3[i];sptrs[i].index=i;}
qsort(sptrs, ns, sizeof(struct sorting_string), compare_sorting_string);
for(i=0;i<ns;i++){idsorder[i]=sptrs[i].index;}
free(sptrs);

//read all pairs and relatedness
read_ids(relfile, NULL, NULL, wantids, maxpairs, NULL, 0, 0);
read_ids(relfile, NULL, NULL, wantids2, maxpairs, NULL, 0, 2);
read_values(relfile, readdoubles, maxpairs, NULL, 5, 0, 0);
if(relats2!=NULL){read_values(relfile, readdoubles2, maxpairs, NULL, 6, 0, 0);}

count=0;for(i=0;i<maxpairs;i++){count+=(strcmp(wantids[i], wantids2[i])==0);}
if(count>0)
{
if(type==0){printf("Error, %d rows contain estimates of self-relatedness\n\n", count);exit(1);}
else{printf("Warning, %d rows contain estimates of self-relatedness\n", count);}
}

count=0;for(i=0;i<maxpairs;i++){count+=(readdoubles[i]<0);}
if(count>0){printf("Error, %d pairs of samples have negative relatedness\n\n", count);}

count=0;for(i=0;i<maxpairs;i++){count+=(readdoubles[i]>1);}
if(count>0){printf("Warning, %d pairs of samples have relatedness greater than one\n", count);}

if(relats2!=NULL)
{
count=0;for(i=0;i<maxpairs;i++){count+=(readdoubles2[i]<0);}
if(count>0){printf("Error, %d pairs of samples have negative environmental similarity\n\n", count);}

count=0;for(i=0;i<maxpairs;i++){count+=(readdoubles2[i]>1);}
if(count>0){printf("Warning, %d pairs of samples have environmental similarity greater than one\n", count);}
}

//work out which pairs are both present
for(i=0;i<maxpairs;i++){usedids[i]=0;}

count=find_strings(wantids, maxpairs, ids3, ns, indexer, NULL, NULL, NULL, NULL, idsorder, 3);
if(count==0)
{
if(type==0){printf("Error, none of the %d samples in Columns 1 and 2 have phenotypes\n\n", maxpairs);}
else{printf("Error, none of the %d samples in Columns 1 and 2 are in the data\n\n", maxpairs);}
exit(1);
}
for(i=0;i<count;i++){usedids[indexer[i]]++;}

count=find_strings(wantids2, maxpairs, ids3, ns, indexer2, NULL, NULL, NULL, NULL, idsorder, 3);
if(count==0)
{
if(type==0){printf("Error, none of the %d samples in Columns 3 and 4 have phenotypes\n\n", maxpairs);}
else{printf("Error, none of the %d samples in Columns 3 and 4 are in the data\n\n", maxpairs);}
exit(1);
}
for(i=0;i<count;i++){usedids[indexer2[i]]++;}

num_rels=0;for(i=0;i<maxpairs;i++){num_rels+=(usedids[i]==2);}
if(num_rels==0)
{
if(type==0){printf("Error, none of the %d pairs of samples both have phenotypes\n", maxpairs);exit(1);}
else{printf("Error, none of the %d pairs of samples are both in the data\n\n", maxpairs);}
exit(1);
}
if(num_rels<maxpairs)
{
if(type==0){printf("Warning, only %d of the %d pairs of samples both have phenotypes\n", num_rels, maxpairs);}
else{printf("Warning, only %d of the %d pairs of samples are both in the data\n", num_rels, maxpairs);}
}

if(num_rels<maxpairs)   //squeeze down wantids and readdoubles
{
count=0;
...
readdoubles[count]=readdoubles[i];
if(relats2!=NULL){readdoubles2[count]=readdoubles2[i];}
}
count++;
}}
}

//fill firsts, seconds and relats
count=find_strings(wantids, num_rels, ids3, ns, NULL, firsts, NULL, NULL, NULL, idsorder, 3);
if(count!=num_rels){printf("Error IIDD1, please tell doug %d %d\n\n", count, num_rels);exit(1);}
count=find_strings(wantids2, num_rels, ids3, ns, NULL, seconds, NULL, NULL, NULL, idsorder, 3);
if(count!=num_rels){printf("Error IIDD2, please tell doug %d %d\n\n", count, num_rels);exit(1);}
for(i=0;i<num_rels;i++){relats[i]=readdoubles[i];}
if(relats2!=NULL)
{
for(i=0;i<num_rels;i++){relats2[i]=readdoubles2[i];}
}

if(type==1) //deal with self-pairs
{
for(i=0;i<ns;i++){usedids2[i]=0;}
for(i=0;i<num_rels;i++){usedids2[firsts[i]]+=(firsts[i]==seconds[i]);}

count=0;for(i=0;i<ns;i++){count+=(usedids2[i]==1);}
if(count>0&&count<ns){printf("Error, %s contains estimates of self-relatedness for only %d of the %d individuals (it should either contain estimates for all individuals, or none)\n\n", relfile, count, ns);exit(1);}

if(count==0)    //add in self pairs
{
printf("Warning, %s does not contain self-relatedness estimates, so these are assumed to be one\n", relfile);
for(i=0;i<ns;i++){firsts[num_rels]=i;seconds[num_rels]=i;relats[num_rels]=1;num_rels++;}
}
}

printf("\n");

free(indexer);free(indexer2);free(usedids);free(usedids2);free(idsorder);
for(i=0;i<maxpairs;i++){free(wantids[i]);free(wantids2[i]);}free(wantids);free(wantids2);free(readdoubles);free(readdoubles2);
...
rs2=malloc(sizeof(char)*1000000);
rs3=malloc(sizeof(char)*1000000);
rs4=malloc(sizeof(char)*1000000);
...
void read_ann(char *partpref, int q, int **pindexes, double **pweights, int length, int *keeppreds_use, int num_preds, char **allpreds, int *predorder, int partition)
{
...
if(partition==-9999){sprintf(filename, "%s%d", partpref, q+1);}
else{sprintf(filename, "%s%d", partpref, 100*(partition-1)+q+1);}
count=countrows(filename);
...
int get_her_model(int num_parts, char *partpref, int **pindexes, double **pweights, int length, int *keeppreds_use, int num_preds, char **allpreds, int *predorder, int parttype, int backpart, int allone, int partition)
{
...
for(q=0;q<num_parts;q++){read_ann(partpref, q, pindexes, pweights, length, keeppreds_use, num_preds, allpreds, predorder, partition);}
printf("\n");
...
int get_windows(char *windfile, int length, int *keeppreds_use, int *blockstarts, int *blockends, int count)
{
int j, found, total;
int *indexer, *indexer2;
...
indexer=malloc(sizeof(int)*count);
indexer2=malloc(sizeof(int)*count);
read_integers(windfile, indexer, count, NULL, 2, 1, 0);
read_integers(windfile, indexer2, length, NULL, 3, 1, 0);
for(j=0;j<count;j++){indexer[j]--;}

//find window of first predictor
found=0;while(keeppreds_use[0]>=indexer2[found]){found++;}
blockstarts[0]=0;
blockends[0]=1;
total=1;
...
for(j=1;j<length;j++)
{
if(keeppreds_use[j]<indexer2[found]){blockends[total-1]=j+1;}
else
{
total++;
while(keeppreds_use[j]>=indexer2[found]){found++;}
blockstarts[total-1]=j;
blockends[total-1]=j+1;
}
}
...
//get number of rows, and check sensible
count2=countrows(betafile)-1;
if(count2%num_phenos){printf("Error, the number of predictors in %s (%d) is not a multiple of the number of phenotypes (%d)\n\n", betafile, count2, num_phenos);exit(1);}
printf("Reading effect sizes for %d predictors across %d phenotypes from %s\n", count2, num_phenos, betafile);
count=count2/num_phenos;
...
for(m=0;m<num_phenos;m++)
{
//get indexes for phenotype m
read_strings(betafile, gotpreds, count, NULL, 2, 1+m*count);
count2=find_strings(gotpreds, count, preds, length, NULL, indexer, betafile, NULL, NULL, NULL, 3);

if(count2==0){printf("Error, none of the %d predictors for Phenotype %d are in the data\n\n", count, m+1);exit(1);}
if(count2<count){printf("Error, only %d of the %d predictors for Phenotype %d are in the data\n", count2, count, m+1);exit(1);}

for(j=0;j<length;j++){effs[j+m*length]=0;}

//open betafile and skip the header rows
if((input=fopen(betafile,"r"))==NULL)
{printf("Error opening %s\n\n", betafile);exit(1);}
for(j=0;j<1+m*count;j++)
{
readchar=0;while(readchar!=10){readchar=10;(void)fscanf(input, "%c", &readchar);}
}
...
{
//read row
if(fscanf(input, "%d %s %s %s %s %lf %s", &readint, rs, ra1, ra2, readstring, &value, readstring)!=7)
{printf("Error reading Row %d of %s\n\n", m*count+j+1, betafile);exit(1);}

//check phenotype and alleles
if(readint!=m+1){printf("Error reading Row %d of %s, phenotype is %d (not %d)\n\n", m*count+j+1, betafile, readint, m+1);exit(1);}
if(strlen(ra1)+strlen(ra2)>2)
{printf("Error reading %s; Predictor %s has multi-character alleles (%s and %s)\n\n", betafile, rs, ra1, ra2);exit(1);}
if(ra1[0]==ra2[0])
{printf("Error reading %s; both alleles for Predictor %s are %c\n\n", betafile, rs, ra1[0]);exit(1);} 
if((ra1[0]!=al1[indexer[j]]&&ra1[0]!=al2[indexer[j]])||(ra2[0]!=al1[indexer[j]]&&ra2[0]!=al2[indexer[j]]))
{printf("Error reading %s; alleles for predictor %s (%c and %c) are not consistent with those in %s (%c and %c)\n\n", betafile, rs, ra1[0], ra2[0], bimfile, al1[indexer[j]], al2[indexer[j]]);exit(1);}

//load up effect size
if(al1[indexer[j]]==ra1[0]){effs[indexer[j]+m*length]=value;}
else{effs[indexer[j]+m*length]=-value;}
}	//end of j loop
