...
if(mpheno!=-1)
{
printf("When performing cross-validation, will use %d training samples and %d test samples\n", count, count2);
...
{
printf("For phenotype %d, will use %d training samples and %d test samples\n", m+1, count, count2);
if(count<3){printf("Error, unable to continue with fewer than three training samples\n\n");exit(1);}
if(count2<3){printf("Error, unable to continue with fewer than three test samples\n\n");exit(1);}
}
}
...
if(substep==-1)	//save indexes
{
sprintf(filename2,"%s.multivariate.bin", outfile);
if((output2=fopen(filename2,"ab"))==NULL)
{printf("Error re-opening %s\n\n",filename2);exit(1);}

fwrite(&num_train, sizeof(int), 1, output2);
fwrite(&num_test, sizeof(int), 1, output2);
fwrite(keeptrain, sizeof(int), num_train, output2);
fwrite(keeptest, sizeof(int), num_test, output2);
...
fseeko(input2, (off_t)sizeof(int)*4, SEEK_SET);

if(fread(&num_train, sizeof(int), 1, input2)!=1)
{printf("Error reading number of training samples from %s\n\n", filename2);exit(1);}
if(fread(&num_test, sizeof(int), 1, input2)!=1)
{printf("Error reading number of test samples from %s\n\n", filename2);exit(1);}
if(fread(keeptrain, sizeof(int), num_train, input2)!=num_train)
{printf("Error reading training samples from %s\n\n", filename2);exit(1);}
if(fread(keeptest, sizeof(int), num_test, input2)!=num_test)
{printf("Error reading test samples from %s\n\n", filename2);exit(1);}

fclose(input2);

printf("When performing cross-validation, will use up to %d training samples and %d test samples\n\n", num_train, num_test);
}
