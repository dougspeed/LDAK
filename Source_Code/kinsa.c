...
count2=find_strings(wantids, count, ids3, num_samples_use, NULL, keepsamps2, NULL, NULL, NULL, NULL, 1);
...
{
#pragma omp parallel for private(j, i, maf) schedule(static)
for(j=0;j<bitlength;j++)
{
...
if(square==1)	//add back on centres, then subtract one
{
...
{
if(passqc==0)
{printf("Error, all %d predictors were excluded because they were trivial\n\n", count);exit(1);}
else
{printf("Error, all %d predictors were excluded because they were trivial or failed quality control\n\n", count);exit(1);}
}
else
{
if(count>0)
...
if(strcmp(sampwfile,"blank")!=0)	//scale by sweights
{
if(single==0)
{
for(j=0;j<num_samples_use;j++)
{
for(i=0;i<num_samples_use;i++){kins[(size_t)j*num_samples_use+i]*=sweights[i]*sweights[j];}
}
}
else
{
for(j=0;j<num_samples_use;j++)
{
for(i=0;i<num_samples_use;i++){kins_single[(size_t)j*num_samples_use+i]*=sweights[i]*sweights[j];}
}
}
}
...
if(single==0){write_kins(outfile, kins, NULL, num_samples_use, ids1, ids2, 1, preds, keeppreds_use, centres, mults, weights, al1, al2, exps, data_length, datafile, power, kingz, kinraw, 1+2*square);}
else{write_kins(outfile, NULL, kins_single, num_samples_use, ids1, ids2, 1, preds, keeppreds_use, centres, mults, weights, al1, al2, exps, data_length, datafile, power, kingz, kinraw, 1+2*square);}
}
...
if(dosage!=-9999){free(keepsamps2);}
