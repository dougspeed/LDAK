...
if(permute==1){permute_double(data+j*num_samples_use,num_samples_use);}

if(resample==-9999)	//normal case
{
for(i=0;i<num_samples_use;i++)
{
value=data[(size_t)j*num_samples_use+i];
...
{
value=data[(size_t)j*num_samples_use+origins[i]];
...
//if(value==2){onechar+=(0<<(2*(i%4)));}
if(value==missingvalue){onechar+=(1<<(2*(i%4)));}
if(i%4==3||i==resample-1){fwrite(&onechar, sizeof(unsigned char), 1, output3);}
}
if(mode==182)
{
if(value!=missingvalue)
...
{
value=ps[0][(size_t)j*num_samples_use+origins[i]];
if(value==0||value==1){fprintf(output3,"%d ", (int)value);}
else{fprintf(output3,"%.3f ", value);}
value=ps[1][(size_t)j*num_samples_use+origins[i]];
if(value==0||value==1){fprintf(output3,"%d ", (int)value);}
else{fprintf(output3,"%.3f ", value);}
value=1-ps[0][(size_t)j*num_samples_use+origins[i]]-ps[1][(size_t)j*num_samples_use+origins[i]];
if(value==0||value==1){fprintf(output3,"%d ", (int)value);}
else{fprintf(output3,"%.3f ", value);}
}
}
