...
if(resample!=-9999){origins=malloc(sizeof(int)*resample);}
...
if(resample!=-9999)	//set starting origins
{
for(i=0;i<resample;i++){origins[i]=(int)(genrand_real1()*num_samples_use);}
}
...
if(resample==-9999){sprintf(filename2,"%s.stats",outfile);}
else{sprintf(filename2,"%s.stats.approx",outfile);}
...
for(j=0;j<bitlength;j++)
{
if(resample!=-9999)	//resample origins
{
for(i=0;i<resample;i++)
{
if(genrand_real1()<recomb){origins[i]=(int)(genrand_real1()*num_samples_use);}
}
}
...
fprintf(output5, "0 0 0 D\n");
for(i=0;i<resample;i++)
{fprintf(output5, "Ind%d Ind%d 0 0\n", i+1, i+1);}
fclose(output5);
}
else
{
...
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename5);exit(1);}
for(i=0;i<resample;i++)
{fprintf(output5, "Ind%d Ind%d 0 0 0 0\n", i+1, i+1);}
fclose(output5);
}
}
...
if(resample==-9999){sprintf(filename7,"%s.missing",outfile);}
else{sprintf(filename7,"%s.missing.approx",outfile);}
