...
//get alleles
along1=malloc(sizeof(char*)*count);
along2=malloc(sizeof(char*)*count);
read_strings(sumsfile, along1, count, NULL, cols[1]+1, 1);
read_strings(sumsfile, along2, count, NULL, cols[2]+1, 1);

for(j=0;j<count;j++)
{
if(strlen(along1[j])+strlen(along2[j])>2)
{printf("Error reading %s; Predictor %s has multi-character alleles (%s and %s)\n\n", sumsfile, preds[j], along1[j], along2[j]);exit(1);}
al1[j]=along1[j][0];
al2[j]=along2[j][0];
}
for(j=0;j<count;j++){free(along1[j]);free(along2[j]);}free(along1);free(along2);
...
read_sumsfile(sumsfile, nss, chis, rhos, NULL, count, preds, al1, al2, NULL, amb, fixn, scaling, 2, 0, sformat);
...
value4=secant_trun(fabs(rhos[j]), value3, value*value3, tol, maxiter, 1);
