...
if(sandwich==1){printf("Will use the sandwich estimator of effect size variance, and not scale test statistics\n\n");}
...
if(drm==1){printf("Performing DRM regression for autosomal SNPs\n\n");}
if(drm==2){printf("Performing DRM regression for Chr X SNPs\n\n");}

if(strcmp(prsfile,"blank")==0)
{
flag=0;
if(strcmp(covarfile,"blank")==0&&strcmp(factorfile,"blank")==0){printf("Consider using \"--covar\" and/or \"--factors\" to provide quantitative or categorical covariates\n");flag=1;}
if(strcmp(topfile,"blank")==0&&strcmp(envfile,"blank")==0&&families==0&&trios==0&&duos==00&&drm==0)
...
if(strcmp(pathfile,"blank")!=0){printf("Will also cut predictors into pathways based on the gene sets provided in %s\n\n", pathfile);}
...
if(mode==141)
{
if(num_subs==0&&strcmp(genpreds,"blank")==0)
{
...
{
if(num_subs==2){printf("Calculating a cross-ancestry tagging file\n\n");}
else
{
if(partition==-9999){printf("Calculating tagging files for %d pathways\n\n", num_parts);}
else{printf("Calculating tagging files for %d pathways (Partition %d out of %d)\n\n", num_parts, partition, 1+(num_anns-1)/100);}
}
}
...
if(num_subs==0&&strcmp(printfile,"blank")==0)
...
if(num_subs==0&&strcmp(herfile,"blank")==0)
...
{printf("Joining %d cross-ancestry tagging files\n\n", num_tags);}
...
if(mode==146)
{
if(strcmp(wayfile,"blank")==0)
{
if(num_parts==0){printf("Estimating heritability from summary statistics\n");}
else
{
if(parttype==0)
{
...
{
if(divide==-9999){printf("Estimating heritabilities from summary statistics with %d partitions\n", num_parts);}
...
printf(" (change this using \"--genomic-control\" and/or \"--intercept\")\n\n");
...
if(cutoff2!=-9999){printf("Will be truncating predictors with chi-squared test statistic above %.2f\n\n", cutoff2);}
...
{
if(num_parts==1){printf("Estimating genetic correlation from summary statistics\n");}
else
{
if(parttype==0)
{
if(num_parts==2){printf("Estimating genetic correlation from summary statistics with the base category and one annotation\n");}
else{printf("Estimating genetic correlation from summary statistics with the base category and %d annotations\n", num_parts-1);}
}
else
{printf("Estimating genetic correlation from summary statistics with %d partitions\n", num_parts);}
}
}
else{printf("Estimating genetic correlation using a cross-ancestry tagging file\n");}
...
if(oversamp==1){printf("Allowing for overlap between the individuals used to compute the sets of summary statistics (if you are sure there is no overlap, you can use \"--overlapping-samples NO\"\n\n");}
else{printf("Assuming there is no overlap between the individuals used to compute the sets of summary statistics (to allow for overlap, use \"--overlapping-samples YES\"\n\n");}

if(cutoff!=-9999){printf("Will be excluding predictors which individually explain at least %.6f of phenotypic variance\n\n", cutoff);}
else{printf("Warning, estimates of heritability can be biased by the presence of strong-effect loci, so either identify these yourself then use \"--exclude\", or use \"--cutoff\" to specify the maximum variance explained\n\n");}

if(cutoff2!=-9999){printf("Will be truncating predictors which individually explain at least %.4f of phenotypic variance\n\n", cutoff2);}
...
if(multi==0)
{
if(loco==0){printf("Constructing ridge regression PRS\n\n");}
else{printf("Constructing LOCO ridge regression PRS\n\n");}
}
else
{
if(loco==0){printf("Constructing multivariate ridge regression PRS\n\n");}
else{printf("Constructing mulitivariate LOCO ridge regression PRS\n\n");}

if(substep==-1){printf("Will stop after estimating variance components and the phenotype transformation\n\n");}
}
...
if(fprs==0){printf("Will exclude the LOCO polygenic contribution if their estimated mean-squared error is above 0.995 (to always include, use \"--force-PRS YES\")\n\n");}
else{printf("Will always include the LOCO polygenic contribution, regardless of their estimated accuracy\n\n");}

printf("When constructing PRS, will scan the data at most %d times (change this using \"--num-scans\")\n\n", nscan);

if(her==-9999){printf("All heritability estimates must be between 0.01 and %.4f (change the upper bound using \"--max-her\")\n\n", maxher);}

if(her==-9999&&nmcmc==-9999)
...
{
if(substep>0){printf("Restarting the analysis for Phenotype %d out of %d\n\n", substep, num_resps_use);}
else{printf("Joining results for %d phenotypes\n\n", num_resps_use);}
}
}
...
if(multi==0)
{
if(loco==0){printf("Constructing Bolt-LMM PRS\n\n");}
else{printf("Constructing LOCO Bolt-LMM PRS\n\n");}
}
else
{
if(loco==0){printf("Constructing multivariate Bolt-LMM PRS\n\n");}
else{printf("Constructing mulitivariate LOCO Bolt-LMM PRS\n\n");}
...
if(multi==0)
{
if(loco==0){printf("Constructing BayesR PRS\n\n");}
else{printf("Constructing LOCO BayesR PRS\n\n");}
}
else
{
if(loco==0){printf("Constructing multivariate BayesR PRS\n\n");}
else{printf("Constructing mulitivariate LOCO BayesR PRS\n\n");}
...
if(multi==0)
{
if(loco==0){printf("Constructing elastic net PRS\n\n");}
else{printf("Constructing LOCO elastic net PRS\n\n");}
}
else
{
if(loco==0){printf("Constructing multivariate elastic net PRS\n\n");}
else{printf("Constructing mulitivariate LOCO elastic net PRS\n\n");}
...
if(minobs>0){printf("Will exclude predictors with callrate below %.4f (change this threshold using \"--min-obs\")\n", minobs);}
if(minmaf>0||minobs>0){printf("\n");}
...
if(gigaprs==0)
{
if(num_focals==1){printf("Constructing a MegaPRS PRS");}
else{printf("Constructing %d MegaPRS PRS", num_focals);}
if(num_parts>0)
{
if(parttype==0)
{
...
{
if(num_focals==1){printf("Constructing a GigaPRS PRS");}
else{printf("Constructing %d GigaPRS PRS", num_focals);}
if(useann==1){printf(" using a base category and %d annotations (to ignore the annotations, add \"--use-annotations NO\")\n\n", num_parts);}
else{printf(" (ignoring the SNP annotations)\n\n");}
}
...
if(ptype==8){printf("Will consider LDpred models;");}
...
if(mcmcfinal==0){printf("Will estimate final effect sizes using variational Bayes, stopping when the approximate log likelihood changes by less than %.2e (use \"--threshold\" to change the convergence threshold, or \"--MCMC-solve YES\" to switch to MCMC)\n\n", tol);}
else{printf("Will use %d MCMC chains, each with %d burn-ins followed by %d iterations (change these values using \"--MCMC-chains\", \"--MCMC-burn\" and \"--MCMC-iterations\")\n\n", num_chains, num_burn, num_mcmc);}
}
else
{
printf("Will compute posterior inclusion probabilities and construct credible sets using a probability threshold of %.4f (change the threshold using \"--CS-coverage\")\n\n", csalpha);
}

if(condition==1){printf("Will perform a conditional analysis using the significance threshold %.e (change this using \"--cutoff\")\n\n", cutoff);}
else{printf("To perform conditional analysis, add \"--conditional YES\"\n\n");}

if(prsvar==1){printf("Will record effect sizes every %dth iteration (after burn-in); change the frequency using \"--MCMC-step\n\n", num_step);} 

if(her==-9999){printf("The heritability estimate must be between 0.01 and %.4f (change the upper bound using \"--max-her\")\n\n", maxher);}
...
if(checkfreq==1)
{printf("Will exclude a predictor if the difference between its allele frequency in the summary statistics and the correlations is greater than %.4f (use \"--check-frequencies NO\" to turn off this filtering, or \"--max-frequency-difference\" to change the threshold)\n\n", maxfreq);}

if(impsums==1){printf("Will impute summary statistics when necessary (to instead ignore the corresponding predictors, use \"--summary-imputation NO\")\n\n");}

if(shrink<1){printf("Will scale predictor-predictor correlations by %.4f (change this value using \"--shrink\")\n\n", shrink);}

count=0;for(q=1;q<num_sums3;q++){count+=(pritraits[q]==0);}
if(count>0&&mtagforce==0){printf("Will only use secondary summary statistics that have estimated (absolute) correlation at least %.4f and that increase the effective sample size by a least %.2f%% (change these thresholds using \"--MTAG-correlation\" and \"--MTAG-percent\")\n\n", mtagcor, mtagpercent);}
}

if(mode==160)
{
printf("Using a validation dataset to identify the most accurate of %d PRS\n\n", num_scores);
}
...
else{printf("Calculating a PRS and its corresponding SEs (the latter are based on %d intermediate PRS)\n\n", num_scores-1);}
...
{
printf("Calculating multi-ancestry scores\n\n");
...
if(prsvar==0&&savecounts==0&&strcmp(scorefile,"blank")!=0){printf("If you require counts (how many predictors contribute to each profile) add \"--save-counts YES\"\n\n");}
}
...
if(strcmp(covarfile,"blank")!=0){printf("Have %d covariates which collectively explain %.4f of the total phenotypic variation\n\n", num_quants-1, cher);}

if(gxe==1){printf("Will scale causal predictors by the environmental variable in %s\n\n", envfile);}
...
if(dichot==0){printf("Will compute the correlation, correlation squared, mean squared error and mean absolute error; if the phenotype is binary, add \"--binary YES\" to also compute the area under curve\n\n");}
...
if(mode==180)
{
printf("Imputing summary statistics in %s using the correlations with stem %s\n\n", sumsfile, corname);
}
...
if(resample!=-9999){printf("Will semi-randomly generate %d samples\n\n", resample);}
