...
if(count3==0)
{printf("Ideally each section should have length less than the number of samples (%d), and this is the case\n\n", ns);}
else
{printf("Ideally each section should have length less than the number of samples (%d); instead %d exceed this length\nConsider using \"--section-kb\", \"--section-length\" or \"--section-cm\" to reduce section sizes and perhaps \"--buffer-kb\", \"--buffer-length\" or \"--buffer-cm\" to reduce buffer sizes\n\n", ns, count3);}
...
int solve_simplex(double *tally3, double *cors, double *tally2, int length, int maxiter, double maxtime, char *filename, int fudge)
{
...
for(j=0;j<length;j++)
{
if(tally2[j]>0){tally3[j]=pow(tally2[j],-1);}
else{tally3[j]=0;}
}
...
for(j=0;j<length;j++)
{glp_set_row_bnds(lp,1+j,GLP_UP, 0.0,1);}
...
int solve_simplex_new(double *tally3, double *cors, double *tally2, int length, int maxiter, double maxtime, char *filename, int fudge)
{
int j, k, count;
...
//solve simplex
//want to get cors |x| = 1
//Cx < 1+a and Cx >1-a with a +tive, then minimize sum(a)
//where A = [C -I] (< 1)
//	    [C +I] (> 1)
...
for(j=0;j<length;j++){rhs[j]=1;rhs[length+j]=1;}
...
int solve_quad(double *tally3, double *cors, double *tally2, int length, char *filename, int fudge)
{
int j, k, flag;
double *cors2, *infos2;
