...
{
if(num_parts==1||parttype==1)	//solve directly
{solve_sums(stats, likes, cohers, cohers2, influs, num_parts, gcon, cept, num_blocks, count, ncv, cvindex, cvexps, stags, svars, ssums, snss, schis, tol, maxiter, chisol, 0, filename);}
else
{
//solve first for base
svars2=malloc(sizeof(double*));svars2[0]=malloc(sizeof(double)*count);
ssums2=malloc(sizeof(double*));ssums2[0]=malloc(sizeof(double)*(3));
stats2=malloc(sizeof(double)*(1+gcon+cept+1+1));

for(j=0;j<count;j++){svars2[0][j]=svars[num_parts-1][j];}
ssums2[0][0]=ssums[num_parts-1][num_parts-1];
ssums2[0][1]=ssums[num_parts-1][num_parts];
ssums2[0][2]=ssums[num_parts-1][num_parts+1];

printf("Solving for only the base category\n");
solve_sums(stats2, NULL, NULL, NULL, NULL, 1, gcon, cept, -9999, count, ncv, cvindex, NULL, stags, svars2, ssums2, snss, schis, tol, maxiter, chisol, 1, filename);

//load coefficients into stats and solve for full model
for(q=0;q<num_parts;q++){stats[q]=0;}
stats[num_parts-1]=stats2[0];
if(gcon==1){stats[num_parts]=stats2[1];}
if(cept==1){stats[num_parts+gcon]=stats2[1+gcon];}

printf("Solving the full model\n");
solve_sums(stats, likes, cohers, cohers2, influs, num_parts, gcon, cept, num_blocks, count, ncv, cvindex, cvexps, stags, svars, ssums, snss, schis, tol, maxiter, chisol, 2, filename);

free(svars2[0]);free(svars2);free(ssums2[0]);free(ssums2);free(stats2);
}
}
...
solve_sums(stats2, NULL, NULL, NULL, NULL, num_reds, gcon, cept, -9999, count, ncv, cvindex, NULL, stags, svars2, ssums2, snss, schis, tol, maxiter, chisol, 1, filename);
...
solve_sums(stats, likes, NULL, NULL, influs, num_parts, gcon, cept, -9999, count, ncv, cvindex, cvexps, stags, svars, ssums, snss, schis, tol, maxiter, chisol, 3, filename);
...
{solve_sums(stats, likes, cohers, cohers2, influs, num_parts, gcon, cept, num_blocks, count, ncv, cvindex, cvexps, stags, svars, ssums, snss, schis, tol, maxiter, chisol, 4, filename);}
}
...
{solve_sums(stats, likes, cohers, cohers2, influs, divide, gcon, cept, num_blocks, count, ncv, cvindex, cvexps, stags, svars2, ssums2, snss, schis, tol, maxiter, chisol, 0, filename);}
...
solve_sums(stats, likes, cohers, cohers2, influs, divide, gcon, cept, num_blocks, count, ncv, cvindex, cvexps, stags, svars2, ssums2, snss, schis, tol, maxiter, chisol, 5, filename);}
...
if((output=fopen(filename,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename);exit(1);}
fprintf(output,"Component Heritability SE Influence SE\n");
for(q=0;q<num_parts_use;q++)
{
...
if((output2=fopen(filename2,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename2);exit(1);}
fprintf(output2,"Component Heritability SE\n");
for(q=0;q<num_parts_use;q++)
{
...
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename3);exit(1);}
fprintf(output3,"Component Share SE\n");
for(q=0;q<num_parts_use;q++)
{
...
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename4);exit(1);}
fprintf(output4,"Component Share SE Expected Enrichment SE Z\n");
for(q=0;q<num_parts_use;q++)
{
...
if(value2!=-9999){fprintf(output4,"%.6f %.6f %.6f %.6f %.6f ", value, value2, ssums_use[q][num_parts_use+1], value/ssums_use[q][num_parts_use+1], value2/ssums_use[q][num_parts_use+1]);}
else{fprintf(output4,"%.6f NA %.6f %.6f NA ", value, ssums_use[q][num_parts_use+1], value/ssums_use[q][num_parts_use+1]);}
...
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename5);exit(1);}
fprintf(output5,"Component Heritability SE Influence SE\n");
for(q=0;q<num_parts_use;q++)
{
...
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename6);exit(1);}
fprintf(output6,"Component Heritability SE\n");
for(q=0;q<num_parts_use;q++)
{
...
if((output3=fopen(filename3,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename3);exit(1);}
fprintf(output3,"Power Her_All SE Null_Chisq_Likelihood Alt_Chisq_Likelihood Observed Expected\n");
...
solve_sums(stats2, NULL, NULL, num_bases, gcon, cept, -9999, count, ncv, cvindex, NULL, stags, svars2, ssums2, snss, schis, tol, sumiter, chisol, 4, ldsc, filename);
...
solve_sums(stats, likes, cohers, num_parts_use, gcon, cept, num_blocks, count, ncv, cvindex, cvexps, stags, svars3, ssums3, snss, schis, tol, sumiter, chisol, 5, ldsc, filename);
