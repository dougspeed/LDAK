...
//WARNING - not allowed to have random number generators within pragma (relevant in lites.c)
...
//gcc source/ldak_nomkl.c source/libqsopt.linux.a -o ldak.out -lblas -llapack -lm -lz

//for a static version, consider something like
//gcc source/ldak_nomkl.c source/libqsopt.linux.a -o ldak.out -lc libraries/liblapack.a libraries/libblas.a libraries/libc.a libraries/libgfortran.a libraries/libm.a libraries/libz.a -static

//On a MAC, compile using 
//gcc source/ldak_nomkl.c source/libqsopt.mac.a -o ldak.out -lblas -llapack -lm -lz
//you may have to add --framework accelerate to this this command, and/or first run xcode-select --install

//if you get errors with the qsopt library, you can instead try
//gcc source/ldak_nomkl_noqsopt.c -o ldak.out -lblas -llapack -lm -lz

///////////////////////////

//note that the variables MKL and MET are now set in ldak_mkl.c, ldak_nomkl.c or ldak_nomkl_noqsopt.c
//#define MKL 1	//1 to compile with mkl, 0 to compile without mkl, 2 to compile with AOCL
//#define MET 0	//0 for qsopt, 1 for glpk, 2 for neither (for glpk, edit compilation line to include source/glpk.h)
...
if(MKL==0){printf("Please note that this version is not compiled with MKL, which will result in longer runtimes\n\n");}
if(MET==2){printf("Please note that this version is not compiled with QSOPT, so is unable to calculate LDAK weightings\n\n");}
...
if(strcmp(prsfile,"blank")!=0)  //check prsfile - if mode=134, switch to 131 or 132, if mode=140 get power and set pvafile and sumsfile
{
#include "switch.c"
}
...
while(1)    //this loop is redundant if not using megaprs (mode=159)
{
if(mode==159&&num_focals>1)
{
printf("Performing the analysis for Target %d out of %d\n\n", cur_focal+1, num_focals);

if(cur_focal==0)   //before first run, save power and her
{
powersave=power;
hersave=her;
}
else    //before later runs, update sumstems, pritraits, sumpops and bimfile, reset power and her
{
count=0;
...
for(q=0;q<num_sums2;q++){pritraits[num_sums1+q]=0;}
...
if(use_data==1||use_data==6||use_data==7)	//reduce predictors to data_length and keeppreds_use
{
#include "setdl.c"
}
...
if(use_data==1||use_data==6||use_data==7)	//(normal case) sort scalings and pvalues, perhaps reduce, then sort summaries
{
...
read_centres(centresfile, centres, data_length, preds, al1, al2, bimfile, 0);
...
{
if(her>=1)
{printf("Warning, this must be less than one; will rescale pre-predictor heritabilities so that they sum to %.4f\n", maxher);her=maxher;}
if(her>maxher)
{printf("Warning, very high values can affect convergence; will rescale pre-predictor heritabilities so that they sum to %.4f\n", maxher);her=maxher;}
}
printf("\n");
}

if(strcmp(sumsfile,"blank")!=0||strcmp(sumslist,"blank")!=0)	//must be mode 121, 138, 140, 158, 159, 172, 179 or 180 (so not sum-hers or sum-cors)
//for 121, 138, 140, 172, 179, checksums is -9999 (no missing allowed); for 158, 159, can be 0 (missing allowed) or 1 (missing possible); for 180 is 0
{
...
read_sumsfile(sumstems[0], nss, chis, rhos, a1freq, data_length, preds, al1, al2, bimfile, amb, scaling, checksums);
...
if(mode==159&&num_sums3>1&&checksize==1&&mean<10000)
{
if(checksize==1){printf("Error, when using multiple summary statistics, we strongly recommend that the focal summary statistics have average sample size over 10000 (you can override this error by adding \"--check-size NO\")\n\n");exit(1);}
printf("Warning, we recommend that the focal summary statistics have average sample size over 10000\n\n");
}
...
{
printf("Warning, there are a large number of covariates (%d); usually this number should be much smaller than the number of samples (%d)\n\n", num_covars, num_samples_use);
if(strcmp(factorfile,"blank")!=0){printf("Should one or more factors instead be included as a quantitative covariates? (i.e., provided using \"--covar\" instead of \"--factors\")\n\n");}
}
...
{read_covarfile(covarfile, covar+num_samples_use, num_samples_use, ids3, num_quants-1, keepquants, missingvalue, 0);}
...
read_sumsfile(sumsfile, tnss, tchis, trhos, NULL, num_tops, tpreds, tal1, tal2, bimfile, amb, scaling, -9999);
...
if(mode==121) //using rovar (random covariates)
{
//work out number of categories
if(strcmp(ractorfile,"blank")!=0){num_rats=count_read_factors(ractorfile, NULL, num_samples_use, ids3, -9999, missingvalue, 0);}
else{num_rats=0;}

//get number of covariates
num_rands=num_ruants+num_rats;
if(num_rands>=num_samples_use/2)
{
printf("Warning, there are very many random covariates (%d); the total number should normally be much smaller than the number of individuals (%d)\n\n", num_rands, num_samples_use);
if(strcmp(ractorfile,"blank")!=0){printf("Should one or more factors instead be included as a quantitative covariates? (i.e., provided using \"--random-covar\" instead of \"--random-factors\")\n\n");}
}

rovar=malloc(sizeof(double)*num_samples_use*num_rands);

if(strcmp(rovarfile,"blank")!=0)  //read quantitative covariates
{read_covarfile(rovarfile, rovar, num_samples_use, ids3, num_ruants, keepruants, missingvalue, 1);}

if(strcmp(ractorfile,"blank")!=0) //read categorical covariates
{
(void)count_read_factors(ractorfile, rovar+num_ruants*num_samples_use, num_samples_use, ids3, num_rats, missingvalue, 1);

//save a combined covariate file
sprintf(filename,"%s.random.combined",outfile);
if((output=fopen(filename,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename);exit(1);}
fprintf(output,"FID IID");
for(j=0;j<num_ruants;j++){fprintf(output," Quant_%d", j+1);}
for(j=num_ruants;j<num_rands;j++){fprintf(output," Indicator_%d", j-num_ruants);}
fprintf(output,"\n"); 

for(i=0;i<num_samples_use;i++)
{
fprintf(output,"%s %s", ids1[i], ids2[i]);
for(j=0;j<num_ruants;j++){fprintf(output," %.4e", rovar[i+j*num_samples_use]);}
for(j=num_ruants;j<num_rands;j++){fprintf(output," %d", (int)rovar[i+j*num_samples_use]);}
fprintf(output,"\n"); 
}
fclose(output);

if(strcmp(rovarfile,"blank")!=0){printf("Note that a combined random covariate file has been saved to %s (this contains both the quantitative covariates and the indicator variables corresponding to the factors; it would be equivalent to repeat this analysis replacing \"--random-covar %s\" and \"--random-factors %s\" with \"--random-covar %s\")\n\n", filename, rovarfile, ractorfile, filename);}
else{printf("Note that a combined random covariate file has been saved to %s (this contains the indicator variables corresponding to the factors; it would be equivalent to repeat this analysis replacing \"--random-factors %s\" with \"--random-covar %s\")\n\n", filename, ractorfile, filename);}
}
}
...
read_sumsfile(sumsfile, rnss, rchis, rrhos, NULL, rnum_preds_use, rpreds, ral1, ral2, bimfile, amb, scaling, -9999);
...
else  //might be shrinking (mode=121)
{
...
{
printf("Shrinking off-diagonals of kinship matrix with stem %s by %.4f\n\n", kinstems[k], shrink);
for(i2=0;i2<num_samples_use;i2++)
{
for(i=0;i<i2;i++){Mkins[k][(size_t)i2*num_samples_use+i]*=shrink;}
for(i=i2+1;i<num_samples_use;i++){Mkins[k][(size_t)i2*num_samples_use+i]*=shrink;}
}
}
if(shrink>1)	//inflate diagonal terms
{
printf("Scaling diagonals of kinship matrix with stem %s by %.4f\n\n", kinstems[k], shrink);
for(i=0;i<num_samples_use;i++){Mkins[k][(size_t)i*num_samples_use+i]*=shrink;}
}
}
}
}
...
//scale roots so the mean is one (might help stability)
sum=0;for(i=0;i<num_samples_use;i++){sum+=readdoubles[i];;}
mean=pow(sum/num_samples_use,-1);
...
if(num_rands+num_regs>0)	//will be using Xs
{
X=malloc(sizeof(double)*num_samples_use*(num_rands+rdata_length));
Xstarts=malloc(sizeof(int)*(num_rands+num_regs));
Xends=malloc(sizeof(int)*(num_rands+num_regs));
Xsums=malloc(sizeof(double)*(num_rands+num_regs));

if(strcmp(sumsfile,"blank")!=0)	//must have num_rands=0
{
...
//fill X (might be empty)
if(strcmp(sumsfile,"blank")==0)	//can have both rands and regions
{
//fill start of X (maybe permuted)
for(j=0;j<num_rands;j++)
{
sumsq=0;
for(i=0;i<num_samples_use;i++){X[i+j*num_samples_use]=rovar[order[i]+j*num_samples_use];sumsq+=pow(rovar[order[i]+j*num_samples_use],2);}
Xstarts[j]=j;
Xends[j]=j+1;
Xsums[j]=sumsq/num_samples_use;
}

//fill end of X (not permuted)
Xtotal=fill_X(X+num_rands*num_samples_use, Xstarts+num_rands, Xends+num_rands, Xsums+num_rands, NULL, NULL, num_samples_use, num_rands, num_regs, regindex, rdata, NULL, NULL);
}
else	//can only have regions (and num_resps_use=1)
{Xtotal=fill_X(X, Xstarts, Xends, Xsums, Xnss, Xrhos, num_samples_use, 0, num_regs, regindex, rdata, rnss, rrhos);}
...
if(num_kins==0&&num_rands+num_regs>0&&shortcut==1)	//do quick version - might have summary stats
{
if(strcmp(sumsfile,"blank")==0)
{fast_reml(num_samples_use, num_covars, num_envs, num_tops, num_rands, num_regs, Y, Z, X, Xtotal, Xstarts, Xends, Xsums, NULL, NULL, prev, respcounts[m], constrain, shrink, strip, tol, maxiter, filename, ids1, ids2, num_preds, allpreds, allal1, allal2, rkeeppreds, regindex, rcentres, rmults, rweights, tkeeppreds, tcentres);}
else
{fast_reml(num_samples_use, 1, 0, 0, 0, num_regs, Y, Z, X, Xtotal, Xstarts, Xends, Xsums, Xnss, Xrhos, prev, -9999, constrain, shrink, strip, tol, maxiter, filename, ids1, ids2, num_preds, allpreds, allal1, allal2, rkeeppreds, regindex, rcentres, rmults, rweights, tkeeppreds, tcentres);}
}
else	//do full version
{multi_reml(num_samples_use, num_covars, num_envs, num_tops, num_kins, num_rands, num_regs, Y, Z, Mkins, NULL, kintraces, kinsums, sweights, X, Xtotal, Xstarts, Xends, Xsums, prev, respcounts[m], hersfile, hestart, shortcut, U, E, 0, -9999, discenv, oversfile, ssums, constrain, tol, maxiter, memsave, maxthreads, kinstems, ids3, filename, ids1, ids2, num_preds, allpreds, allal1, allal2, rkeeppreds, regindex, rcentres, rmults, rweights, tkeeppreds, tcentres, NULL, missingvalue);}
}	//end of m loop

free(order);free(sweights);free(Y);free(Z);
if(num_rands+num_regs>0){free(X);free(Xstarts);free(Xends);free(Xsums);}
...
//fill X (not permuted)
Xtotal=fill_X(X, Xstarts, Xends, Xsums, NULL, NULL, num_samples_use, 0, num_regs, regindex, rdata, NULL, NULL);
...
if(mode==126)	//fast-reml - not using randoms, regions nor summaries
{
...
multi_reml(num_samples_use, num_covars, num_envs, num_tops, num_kins, 0, num_regs, Y, Z, NULL, Mkins_single, kintraces, kinsums, sweights, NULL, 0, NULL, NULL, NULL, prev, respcounts[m], hersfile, hestart, 2, NULL, NULL, num_vects, ldlt, 0, oversfile, ssums, constrain, tol, maxiter, memsave, maxthreads, kinstems, ids3, filename, ids1, ids2, num_preds, allpreds, allal1, allal2, NULL, NULL, NULL, NULL, NULL, tkeeppreds, tcentres, NULL, missingvalue);
}	//end of m loop
...
{
if(num_kins==0&&strcmp(envfile,"blank")==0&&strcmp(prsfile,"blank")==0&&families==0&&trios==0&&duos==0&&drm==0)	//basic case
{
#include "singlea.c"
}
...
if(window_kb>25)	//do in two steps
{
if(window_kb!=-9999)
{
if(window_cm!=-9999)
...
if(strcmp(pathfile,"blank")==0) //normal case
{
...
else    //have pathways - must have part_length -9999, bychr=1, genefile, and therefore exactly num_genes genes
{
//get number of pathways
count=countrows(pathfile);

//check names are unique
wantpreds=malloc(sizeof(char*)*count);
read_strings(pathfile, wantpreds, count, NULL, 1, 0);
check_dups(wantpreds,count,pathfile,NULL,1);
for(j=0;j<count;j++){free(wantpreds[j]);}free(wantpreds);

//get pathway lengths (plus one)
plengths=malloc(sizeof(int)*count);
fillcols(pathfile, plengths, count);

//check lengths, then subtract one
for(k=0;k<count;k++)
{
if(plengths[k]==0){printf("Error, Row %d of %s contains zero elements; this should not happen, so can you please tell Doug\n\n", k+1, pathfile);exit(1);}
if(plengths[k]==1){printf("Error, Row %d of %s contains only one element; all rows should start with the pathway name, then contain one or more genes\n\n", k+1, pathfile);exit(1);}
plengths[k]--;
}


cut_genes(gchr, gbp1, gbp2, gnames, gstrand, gstarts, gends, data_length, chr, preds, bp, weights, genefile, -9999, -9999, up_buffer, down_buffer, minweight, overlap, pvafile, pvalues, -9999, 0, 4, folder, datafile, bimfile, extract, -9999);

//now deal with pathways
rs=malloc(sizeof(char)*1000000);
rs2=malloc(sizeof(char)*1000000);
usedpreds=malloc(sizeof(int)*data_length);

if((input=fopen(pathfile,"r"))==NULL)
{printf("Error opening %s\n\n",pathfile);exit(1);}

sprintf(filename2,"%spathway.names",folder);
if((output2=fopen(filename2,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename2);exit(1);}

sprintf(filename3,"%spathway.details",folder);
if((output3=fopen(filename3,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename3);exit(1);}
fprintf(output3,"Pathway Num_Genes Num_Predictors\n");

found=0;
for(k=0;k<count;k++)
{
if(k%100==0){printf("Mapping predictors for Pathway %d of %d\n", k+1, count);}

indexer=malloc(sizeof(int)*plengths[k]);
wantpreds=malloc(sizeof(char*)*plengths[k]);

if(fscanf(input,"%s ",rs)!=1)
{printf("Error reading first element of Row %d of %s\n\n", k+1, pathfile);exit(1);}

for(j=0;j<plengths[k];j++)
{
if(fscanf(input,"%s ",rs2)!=1)
{printf("Error reading Element %d of Row %d of %s\n\n", j+2, k+1, pathfile);exit(1);}
copy_string(wantpreds,j,rs2);
}

//check unique
check_dups(wantpreds, plengths[k], rs, NULL, 1);

count2=find_strings(gnames, num_genes, wantpreds, plengths[k], indexer, NULL, NULL, NULL, NULL, NULL, 3);
if(count2==0)
{
if(wcount<5){printf("Warning, none of the %d genes in %s are in %s\n", plengths[k], rs, genefile);}
wcount++;
}
...
{
count3=0;
for(j=0;j<data_length;j++){usedpreds[j]=0;}
for(j2=0;j2<count2;j2++)
{
if(gstarts[indexer[j2]]!=-9999)
{
for(j=gstarts[indexer[j2]];j<gends[indexer[j2]];j++){usedpreds[j]=1;}
count3++;
}
}

if(count3>0) //save pathway predictors, name and details
{
sprintf(filename,"%spathway%d",folder, found+1);
if((output=fopen(filename,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename);exit(1);}
count4=0;
for(j=0;j<data_length;j++)
{
if(usedpreds[j]==1){fprintf(output, "%s\n", preds[j]);count4++;}
}
fclose(output);

fprintf(output2,"%s\n", rs);
fprintf(output3,"%s %d %d\n", rs, count3, count4);

found++;
}
else
{
if(wcount<5){printf("Warning, none of the %d genes in %s contain predictors\n", plengths[k], rs);}
wcount++;
}
}

free(indexer);
for(j=0;j<plengths[k];j++){free(wantpreds[j]);}free(wantpreds);
}
if(wcount>5){printf("In total, %d pathways were excluded because they contain no predictors\n", wcount);}
printf("\n");
...
if(found==0){printf("Error, none of the %d pathways contain predictors\n\n", count);exit(1);}
if(found==count){printf("All %d pathways contain predictors; ", count);}
else{printf("%d of the %d pathways contain predictors; ", found, count);}
printf("the predictor lists have been saved with prefix %spathway, while the pathway names and details are saved in %s and %s\n\n", folder, filename2, filename3);

free(plengths);
free(rs);free(rs2);free(usedpreds);
}
...
{
if(kvikparity!=0)   //will update outfile for each analysis, and update power, pvafile, sumsfile for analyses 2 onwards
{strcpy(outfile2,outfile);}

for(m=0;m<(kvikparity==0)+kvikparity;m++)
{
if(kvikparity!=0)
{
sprintf(outfile,"%s.pheno%d",outfile2,m+1);

if(m==0){printf("Performing the analyses for Phenotype 1 of %d\n\n", kvikparity);}
else //must update power, pvalues and summaries
{
printf("-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --\n\n");
printf("Repeating the analyses for Phenotype %d of %d\n\n", m+1, kvikparity);
power=powers[m];
sprintf(pvafile,"%s.pheno%d.pvalues",assfile,m+1);
sprintf(sumsfile,"%s.pheno%d.summaries",assfile,m+1);
...
printf("Reading summary statistics from %s\n", sumsfile);
read_sumsfile(sumsfile, nss, chis, rhos, a1freq, data_length, preds, al1, al2, bimfile, amb, scaling, 2);
printf("First few stats and ns are: %s %.3f %.1f", preds[0], chis[0], nss[0]);
for(j=1;j<data_length;j++){if(j<3){printf(" | %s %.3f %.1f", preds[j], chis[j], nss[j]);}}
printf("\n\n");
}
...
//allocate and get gene details (this uses outfile)
#include "readdetails.c"

//perform analysis (this uses outfile)
#include "genes.c"
...
//free allocations from readdetails.c
for(g=0;g<num_genes;g++){free(gnames[g]);}free(gnames);
...
if(strcmp(xaglist,"blank")!=0)
{
#include "joinb.c"
}
if(strcmp(pathlist,"blank")!=0)
{
#include "joinc.c"
}
}
...
if(seed==-9999)	//update random seed for uniform generator (can't remember why I put this here)
{
...
///////////////////////////
//main frees - excludes allocations from readdetails.c, switch.c and parsefiles.c
///////////////////////////

//allocations from getnums.c (leave tkeeppreds for later)
...
if(use_data==1||use_data==2||use_data==3||use_data==6||use_data==7)
{
...
//allocations from findfamilies.c, findtrios.c, findduos.c

if(mode==131&&(families==1||trios==1||duos!=0)){free(famindex);free(famcounts);}

//top predictors from top of ldak.c (variables were not all allocated together)
...
//regions from top of ldak.c (variables were not all allocated together)
...
if(use_data==1||use_data==6||use_data==7)
{
free(keeppreds_use);
...
//use_data from top of ldak.c

if(use_data==1||use_data==6||use_data==7)
{
free(centres);free(mults);free(sqdevs);free(rates);free(infos);free(weights);free(pvalues);
if(strcmp(sumsfile,"blank")!=0||strcmp(sumslist,"blank")!=0){free(nss);free(chis);free(rhos);free(a1freq);}
}

//resps from top of ldak.c

if(num_resps_use>0){free(resp);free(respcounts);}
if(mode==229||mode==230){free(keepresps2);free(resp2);free(respcounts2);}

//covar (and rovar / offsets / thetas) from top of ldak.c

if(mode==121||mode==122||mode==123||mode==124||mode==125||mode==126||mode==127||mode==128||mode==129||mode==130||mode==229||mode==230||mode==131||mode==132||mode==133||mode==138||mode==140||mode==151||mode==152||mode==153||mode==154||mode==156||mode==164||mode==169||mode==170||mode==172||mode==173||mode==175||mode==194){free(covar);}
if(mode==121){free(rovar);}
if(mode==132){free(offsets);}
if(strcmp(cofile,"blank")!=0){free(thetas);}

//kinships (and eigens) from top of ldak.c

if(num_kins>0&&(mode==120||mode==121||mode==123||mode==124||mode==126||mode==131||mode==133||mode==138))
{
...
//decide whether to leave while loop
if(mode!=159){break;}
if(mode==159)
{
if(num_focals==1){break;}

if(cur_focal==num_focals) //combine effects then break
{
#include "collect.c"
break;
}
...
///////////////////////////
//secondary frees - allocations from readdetails.c, switch.c and parsefiles.c
///////////////////////////

//allocations from readdetails.c (not mode=140)
...
//allocation from switch.c

if(mode==140&&kvikparity!=0){free(powers);}

//datafile, kinships and resp allocations from parsefiles.c
...
if(num_resps_use>0){free(keepresps);}

//more allocations from parsefiles.c

if(strcmp(bulkfile,"blank")!=0)
{
for(q=0;q<num_sums2;q++){free(bulknames[q]);}free(bulknames);
}

if(strcmp(sumsfile,"blank")!=0||strcmp(sumslist,"blank")!=0)
{
for(q=0;q<num_sums3;q++){free(sumstems[q]);}free(sumstems);
free(pritraits);free(sumpops);
}

if(mode==159&&strcmp(sumslist,"blank")!=0)
{
for(q=0;q<num_sums3;q++){free(sumstems2[q]);}free(sumstems2);
free(pritraits2);free(sumpops2);
}

if(strcmp(covarfile,"blank")!=0){free(keepquants);}

if(strcmp(rovarfile,"blank")!=0){free(keepruants);}
...
if(num_cors>0)
{
for(s=0;s<num_cors;s++){free(corstems[s]);}free(corstems);
}
