...
{
//type=0/1/2 - will standardize and set missing to zero; type=3 - set missing to mean (must be calculating scores)
//type=0 - already have stats, type=1 - get stats with power 0, type=2 - get stats with power -1, type=3 - get stats with power 0
...
if(type==1||type==2||type==3)	//need to get stats
{
...
centres[j]=mean;
if(type==3) //have power 0 and only ignore predictor if all values NA
{
if(indcount>0){mults[j]=1;}
else{mults[j]=-9999;}
}
else    //have power 0 or -1, and ignore predictor if trivial
{
if(var>0)
{
if(type==1){mults[j]=1;}
...
size_t sourceLenb, destLenb, result;

unsigned char flags[4];
char *buffer, *buffptr;
size_t size, size2, totalread, remaining, index;
short readshort;
int hblock, bgen_comp, bgen_layout, eof, readint, readint2;
...
//for layout 2, need (fairly accurate) estimates of max size for both compressed or uncompressed
maxLen2a=num_samples;
maxLen2b=5*num_samples;
...
//size 2 is the (maximum) size required for each predictor
if(bgen_layout==1)
{
if(bgen_comp==0)    //uncompressed - size2 is exactly 6n
{size2=6*num_samples;}
else    //compressed - size2 must be at most 4+maxLen1
{size2=4+maxLen1;}
}
else
{
if(bgen_comp==0)    //uncompressed - size2 is hopefully at most 4+maxLen2b
{size2=4+maxLen2b;}
else    //compressed - size2 is hopeuflly at most 4+maxLen2a
{size2=4+maxLen2a;}
}
...
if(bgen_comp==1||bgen_comp==2)    //must allocate space for uncompressed data
{
if(bgen_layout==1){uncompdata1=malloc(sizeof(Bytef)*6*num_samples);}
else{uncompdata2=malloc(sizeof(Bytef)*maxLen2b);}
}

////////

//load up buffer starting with first predictor (must have at least one)
if(fseeko(input, bgen_indexes[keeppreds[start]], SEEK_SET)!=0)
{printf("Error reading %s; unable to find Predictor %d\n\n", bgenfile, keeppreds[start]+1);exit(1);}
index=bgen_indexes[keeppreds[start]];
totalread=fread(buffer, 1, size, input);
eof=(totalread<size);

wcount=0;vcount=0;
for(j=start;j<end;j++)
{
//how much remaining in buffer
if(bgen_indexes[keeppreds[j]]-index<totalread){remaining=totalread-(bgen_indexes[keeppreds[j]]-index);}
else{remaining=0;}

if(remaining<size2&&eof!=1)    //load up buffer
{
if(fseeko(input, bgen_indexes[keeppreds[j]], SEEK_SET)!=0)
{printf("Error reading %s; unable to find Predictor %d\n\n", bgenfile, keeppreds[j]+1);exit(1);}
index=bgen_indexes[keeppreds[j]];
totalread=fread(buffer, 1, size, input);
eof=(totalread<size);
remaining=totalread;
}

buffptr=buffer+(bgen_indexes[keeppreds[j]]-index);

if(bgen_layout==1)	//read 3n two-byte probabilities, and convert to dosages
{
if(bgen_comp==0)	//uncompressed
{
if(remaining<6*num_samples)
{printf("Error reading probabilities for Predictor %d from %s\n\n", keeppreds[j]+1, bgenfile);exit(1);}
uncompdata1=(Bytef *)buffptr;buffptr+=6*num_samples;remaining-=6*num_samples;
}
else	//compressed
{
if(remaining<4)
...
compdata1=(Bytef *)buffptr;buffptr+=readint;remaining-=readint;

if(bgen_comp==1)    //zlib compressed
{
...
else	//zstd compressed
{
sourceLenb=(size_t)(readint);
destLenb=(size_t)(6*num_samples);
result=ZSTD_decompress(uncompdata1, destLenb, compdata1, sourceLenb);
if(ZSTD_isError(result)){printf("Error decompressing probabilities for Predictor %d from %s\n\n", keeppreds[j]+1, bgenfile);exit(1);}
}
}
...
{printf("Error reading storage size for Predictor %d from %s\n\n", keeppreds[j]+1, bgenfile);exit(1);}
readint2=*(int *)buffptr;buffptr+=4;remaining-=4;

if(readint2>(int)maxLen2b)  //need to update sizes, then maybe update buffer and reload buffer
{
...
if(remaining<readint2)  //need to start again for this predictor
{
if(fseeko(input, bgen_indexes[keeppreds[j]], SEEK_SET)!=0)
{printf("Error reading %s; unable to find Predictor %d\n\n", bgenfile, keeppreds[j]+1);exit(1);}
totalread=fread(buffer, 1, size, input);
eof=(totalread<size);
remaining=totalread-4;
buffptr=buffer+4;
}
}

if(remaining<readint2)
//if(fread(uncompdata2, 1, readint2, input)!=readint2)
{printf("Error reading probabilities for Predictor %d from %s\n\n", keeppreds[j]+1, bgenfile);exit(1);}
uncompdata2=(Bytef *)buffptr;buffptr+=readint2;remaining-=readint2;
}
...
if(readint-4>(int)maxLen2a)  //need to update sizes, then maybe update buffer and reload buffer
{
...
if(remaining<readint)  //need to start again for this predictor
{
if(fseeko(input, bgen_indexes[keeppreds[j]], SEEK_SET)!=0)
{printf("Error reading %s; unable to find Predictor %d\n\n", bgenfile, keeppreds[j]+1);exit(1);}
totalread=fread(buffer, 1, size, input);
...
if(remaining<readint-4)
//if(fread(compdata2, 1, readint-4, input)!=readint-4)
{printf("Error reading compressed probabilities for Predictor %d from %s\n\n", keeppreds[j]+1, bgenfile);exit(1);}
compdata2=(Bytef *)buffptr;buffptr+=readint-4;remaining-=(readint-4);
...
{
sourceLenb=(size_t)(readint-4);
destLenb=(size_t)readint2;
result=ZSTD_decompress(uncompdata2, destLenb, compdata2, sourceLenb);
if(ZSTD_isError(result)){printf("Error decompressing probabilities for Predictor %d from %s\n\n", keeppreds[j]+1, bgenfile);exit(1);}
}
}
...
free(buffer);
if(bgen_comp==1||bgen_comp==2)
{
if(bgen_layout==1){free(uncompdata1);}
else{free(uncompdata2);}
}
...
if(buffptr[0]=='N'&&buffptr[1]=='A'&&(buffptr[2]==' '||buffptr[2]=='\0'))
{datatemp[i]=missingvalue;buffptr+=2;}
else
{
datatemp[i]=strtod(buffptr, &buffptr2);
if(buffptr==buffptr2)
{printf("Error reading %s; unknown value (%s) for Predictor %d, Individual %d\n", genfile, readstring, genskip+current+1, i+1);exit(1);}
buffptr=buffptr2;
}
...
while(buffptr[0]==9||buffptr[0]==32){buffptr++;}
