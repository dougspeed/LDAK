...
//if using pathways (genpreds!=NULL), will have reduce=0 and coverage=0 (and will not save the matrix)
...
if(partition!=-9999)    //skip top rows
{
for(j=0;j<100*(partition-1);j++)
{
...
{
for(q=0;q<num_parts;q++){sprintf(catlabels[q],"Pathway_%d",100*(partition-1)+q+1);}
}
}
}
else
...
addpart=get_her_model(num_parts, partpref, pindexes, pweights, data_length, keeppreds_use, num_preds, allpreds, predorder, parttype, backpart, allone, partition);
...
{
anal_warn(bitmax, 3*bitmax);
cors=malloc(sizeof(double)*bitmax*bitmax);
cors2=malloc(sizeof(double)*bitmax*bitmax);
cors3=malloc(sizeof(double)*bitmax*bitmax);
}
...
if(num_subs==0){tally3=malloc(sizeof(double)*data_length*(num_parts+addpart+addgenic));}
else{tally3=malloc(sizeof(double)*data_length*3);}
...
else{tally3[j]=0;tally3[j+data_length]=0;tally3[j+2*data_length]=0;}
if(cover==1)
{
...
if(partition==-9999){sprintf(filename,"%s.progress",outfile);}
else{sprintf(filename,"%s.partition%d.progress",outfile,partition);}
if((output=fopen(filename,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename);exit(1);}
fclose(output);

if(num_subs==0&&strcmp(genpreds,"blank")==0){sprintf(filename2,"%s.tagging", outfile);}
else
{
if(num_subs==2){sprintf(filename2,"%s.cross.tagging", outfile);}
else
{
if(partition==-9999){sprintf(filename2,"%s.base.tagging", outfile);}
else{sprintf(filename2,"%s.partition%d.base.tagging", outfile,partition);}
}
}
if((output2=fopen(filename2,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename2);exit(1);}
...
{
if(num_subs==2){fprintf(output2," Subset_1 Subset_2 Cross_Term\n");}
else{fprintf(output2," Base Genic\n");}
}

if(num_subs==0&&strcmp(genpreds,"blank")==0)	//save matrix
{
...
if(num_subs==0)	//normal case, no subsets
{
//get correlation
...
if(strcmp(genpreds,"blank")==0&&vindex[j]==1)
{
...
else	//have two subsets (coverage=0)
{
//get correlation across all
alpha=1.0/num_samples_use;beta=0.0;
dgemm_("T", "N", &bitlength, &bitlength, &num_samples_use, &alpha, data, &num_samples_use, data, &num_samples_use, &beta, cors, &bitlength);

//standardize for each subset, removing predictors trivial in one subset
stand_matrix_trivial(data, subindex[0][0], num_samples_use, bitlength, mults+bitstart);
stand_matrix_trivial(data+subindex[0][0], subindex[1][0], num_samples_use, bitlength, mults+bitstart);

//get correlations for subset 1
alpha=1.0/subindex[0][0];beta=0.0;
dgemm_("T", "N", &bitlength, &bitlength, subindex[0], &alpha, data, &num_samples_use, data, &num_samples_use, &beta, cors2, &bitlength);

//get correlations for subset 2
alpha=1.0/subindex[1][0];beta=0.0;
dgemm_("T", "N", &bitlength, &bitlength, subindex[1], &alpha, data+subindex[0][0], &num_samples_use, data+subindex[0][0], &num_samples_use, &beta, cors3, &bitlength);

//now loop through predictors in chunk, getting required sums and printing (unbias=coverage=0)
for(j=bitstart;j<bitend;j++)
{
if(mults[j]!=-9999)	//non trivial
{
...
tally1[j]++;
if(windex[j]==1){tally2[j]++;}
tally3[j]+=pweights[0][j]*exps[j];
tally3[j+data_length]+=pweights[0][j]*exps[j];
tally3[j+2*data_length]+=pweights[0][j]*exps[j];
...
tally1[k]++;
if(windex[j]==1&&windex[k]==1)	//both are regression SNPs
{tally2[j]+=value;tally2[k]+=value;}
if(value>=mincor)
{
//subset 1
value=pow(cors2[(size_t)(j-bitstart)*bitlength+(k-bitstart)],2);
tally3[j]+=pweights[0][k]*value*exps[k];
tally3[k]+=pweights[0][j]*value*exps[j];

//subset 2
value=pow(cors3[(size_t)(j-bitstart)*bitlength+(k-bitstart)],2);
tally3[j+data_length]+=pweights[0][k]*value*exps[k];
tally3[k+data_length]+=pweights[0][j]*value*exps[j];

//cross
value=cors2[(size_t)(j-bitstart)*bitlength+(k-bitstart)]*cors3[(size_t)(j-bitstart)*bitlength+(k-bitstart)];
tally3[j+2*data_length]+=pweights[0][k]*value*exps[k];
tally3[k+2*data_length]+=pweights[0][j]*value*exps[j];
}
}}	//end of using k and k loop
...
//print taggings for subset 1, subset 2, then cross
fprintf(output2, " %.4f %.4f %.4f\n", tally3[j], tally3[j+data_length], tally3[j+2*data_length]);
}
}}	//end of using j and j loop
}
...
{
if(partition==-9999){sprintf(filename5,"%s.pathway.tagging", outfile);}
else{sprintf(filename5,"%s.partition%d.pathway.tagging", outfile,partition);}
...
for(j=0;j<data_length;j++)
{
if(mults[j]!=-9999&&windex[j]==1)
{
if(fourdp==0)
{
if(fabs(tally3[j+q*data_length])<.05){fprintf(output5," 0");}
else{fprintf(output5," %.1f", tally3[j+q*data_length]);}
}
else
{
if(fabs(tally3[j+q*data_length])<.00005){fprintf(output5," 0");}
...
if(num_subs==0&&strcmp(genpreds,"blank")==0)	//normal case - compute full ssums and add to bottom of tagging file
{
for(q=0;q<num_parts+addpart;q++)
{
...
{
if(num_subs==2)	//subsets - categories contribute only to themselves
{
sum=0;
count=0;
for(j=0;j<data_length;j++)
{
if(vindex[j]==1)	//predictor is part of q and a heritability predictor
{
sum+=pweights[0][j]*exps[j];
count++;
}
}
ssums[0][0]=sum;ssums[0][1]=0;ssums[0][2]=0;ssums[0][3]=count;
ssums[1][0]=0;ssums[1][1]=sum;ssums[1][2]=0;ssums[1][3]=count;
ssums[2][0]=0;ssums[2][1]=0;ssums[2][2]=sum;ssums[2][3]=count;
...
else	//genic - compute reduced version of ssums and print separately
{
if(partition==-9999){sprintf(filename6,"%s.pathway.sums", outfile);}
else{sprintf(filename6,"%s.partition%d.pathway.sums", outfile,partition);}
...
fclose(output2);
if(num_subs==0&&strcmp(genpreds,"blank")==0){fclose(output3);}
if(cover==1){fclose(output4);}

if(num_subs==0&&strcmp(genpreds,"blank")==0)
{
printf("Taggings saved in %s, with heritability matrix in %s", filename2, filename3);
if(cover==1){printf(" and coverages in %s", filename4);}
printf("\n\n");
}
else
{
if(num_subs==2){printf("Cross taggings saved in %s\n\n", filename2);}
else
{
if(partition==1)    //make a merging file
{
sprintf(filename,"%s.merge.list",outfile);
if((output=fopen(filename,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename);exit(1);}
for(j=0;j<1+(num_anns-1)/100;j++){fprintf(output, "%s.partition%d\n", outfile, j+1);}
fclose(output);
}

printf("Base taggings saved in %s, with pathway taggings in %s and %s\n\n", filename2, filename5, filename6);

if(partition!=-9999){printf("Once all partitions are completed, you can merge using the command \"--merge-tagging %s\" with \"--pathlist %s.merge.list\n\n", outfile, outfile);}
}
}
...
free(exps);
free(cors);
if(num_subs==2){free(cors2);free(cors3);}
