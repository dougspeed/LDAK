...
{bstarts[p]=(double)(p)*data_length/num_blocks;bends[p]=(double)(p+1)/num_blocks;}
...
printf("Total heritability %.4f (SE %.4f)\n\n", hers[total], hersds[total]);
...
if((output3=fopen(filename3,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename3);exit(1);}
if(mode==127){fprintf(output3, "Component Effect SE P\n");}
...
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename7);exit(1);}
fprintf(output7, "Component Heritability SE\n");
for(q=0;q<total;q++)
{
sum=0;sum2=0;
for(q2=0;q2<total;q2++)
...
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename8);exit(1);}
fprintf(output8, "Component Share SE Expected Enrichment SE\n");
for(q=0;q<total;q++)
{
sum=0;sum2=0;
for(q2=0;q2<total;q2++)
