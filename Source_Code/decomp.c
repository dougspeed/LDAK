...
//update rkTrk and get betaks = rk+1Tr+1k / rkTrk
for(j=0;j<ncol;j++)
{
value=0;
...
double cg_solve_lower_norm(double *cors, int length, double *rhos, double tol)
{
//solve cors xk = rhos, where cors is symmetric, and base convergence on sum (xk*rhos)
int i, count, one=1;
double alpha, beta, sum, sum2, value, *xk, *rk, *pk;
double alphak, betak, rkTrk, *Apk;

//can deal with trivial cases
if(length==1){return(pow(rhos[0],2)/cors[0]);}
if(length==2)
{
value=cors[0]*cors[3]-pow(cors[1],2);
sum=pow(rhos[0],2)*cors[3]-2*rhos[0]*rhos[1]*cors[1]+pow(rhos[1],2)*cors[0];
return(sum/value);
}

//allocations
xk=malloc(sizeof(double)*length);
rk=malloc(sizeof(double)*length);
pk=malloc(sizeof(double)*length);
Apk=malloc(sizeof(double)*length);

//initializations

//set x0 to zero
for(i=0;i<length;i++){xk[i]=0;}

//therefore r0=rhos
for(i=0;i<length;i++){rk[i]=rhos[i];}

//p0=r0
for(i=0;i<length;i++){pk[i]=rk[i];}

//get r0Tr0s 
rkTrk=ddot_(&length, rk, &one, rk, &one);

//get sum(xk*rhos) - bit silly as will equal zero when x starts at zero
sum=ddot_(&length, xk, &one, rhos, &one);

for(count=0;count<500;count++)
{
//update Apk
alpha=1.0;beta=0.0;
dsymv_("L", &length, &alpha, cors, &length, pk, &one, &beta, Apk, &one);

//alphak = rkTrk / pkT Apk
alphak=rkTrk/ddot_(&length, pk, &one, Apk, &one);

//xk+1 = xk + alphak pk
for(i=0;i<length;i++){xk[i]+=alphak*pk[i];}

//rk+1 = rk - alphak Apk
for(i=0;i<length;i++){rk[i]-=alphak*Apk[i];}

//update rkTrk and get betak = rk+1Trk+1 / rkTrk
value=ddot_(&length, rk, &one, rk, &one);
betak=value/rkTrk;
rkTrk=value;

//see if sum(xk*rhos) has converged (also break if rkTrk=0, as this means have exact solution)
sum2=sum;
sum=ddot_(&length, xk, &one, rhos, &one);
if(rkTrk==0||fabs(sum-sum2)<tol){break;}

//pk+1 = rk + betak pk
for(i=0;i<length;i++){pk[i]=rk[i]+betak*pk[i];}
}	//end of while loop
...
if(sum!=sum||isinf(sum)){printf("Error II12, can not compute rho inv(cors) rho with size %d - please tell Doug \n\n", length);}
...
{
//ncol=0 - get "cholesky", ncol=-1 - get inverse, ncol=-2 - get inv cholesky, ncol>0 - solve mat X = mat3
...
if(info!=0){printf("Error, eigen priming failed; please tell Doug (info %d, length %d)\n\n", info, length);exit(1);}
lwork=(int)wkopt;
work=malloc(sizeof(double)*lwork);

dsyev_("V", "U", &length, mat, &length, mat2, work, &lwork, &info);
if(info!=0){printf("Error, eigen decomp failed; please tell Doug (info %d, length %d)\n\n", info, length);exit(1);}
free(work);
...
int eigen_solve_check(double *mat, int length, double *mat2, double *mat3)
{
//find inv(mat) mat2, with requirement that mat must be positive definite
int i, j, count, lwork, info, one=1;
double *scales, *scales2, alpha, beta, wkopt, *work, *mat4;


scales=malloc(sizeof(double)*length);
scales2=malloc(sizeof(double)*length);
mat4=malloc(sizeof(double)*length);
...
for(j=0;j<length;j++)
{
if(mat2[j]<=0){mat2[j]=1e-10;count++;}
}
...
//get UT mat3
alpha=1.0;beta=0.0;
dgemv_("T", &length, &length, &alpha, mat, &length, mat3, &one, &beta, mat4, &one);

//divide by eigenvalues
for(i=0;i<length;i++){mat4[i]*=pow(mat2[i],-1);}

//pre-multiply by U
alpha=1.0;beta=0.0;
dgemv_("N", &length, &length, &alpha, mat, &length, mat4, &one, &beta, mat3, &one);

free(scales);free(scales2);free(mat4);
...
if(info!=0){printf("Error, eigen priming failed; please tell Doug (info %d, count %d)\n\n", info, count);exit(1);}
lwork=(int)wkopt;
work=malloc(sizeof(double)*lwork);
dsyev_("V", "U", &count, U, &count, E, work, &lwork, &info);
if(info!=0){printf("Error, eigen decomp failed; please tell Doug (info %d, count %d)\n\n", info, count);exit(1);}
free(work);
...
if(*info!=0){printf("Error LDLT invert failed, please tell Doug (info %d, length %d)\n\n", *info, length);exit(1);}
for(i=0;i<length;i++)
{
...
if(*info!=0){printf("Error Cholesky invert failed, please tell Doug (info %d, length %d)\n\n", *info, length);exit(1);}
for(i=0;i<length;i++)
{
for(j=0;j<i;j++){mat[(size_t)j*length+i]=mat[(size_t)i*length+j];}
}
...
if(*info!=0){printf("Error Cholesky solve failed, please tell Doug (info %d, length %d)\n\n", *info, length);exit(1);}
}
}
else	//decomposition failed
{
