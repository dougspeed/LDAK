...
int read_scores(double **blupcentres, double **blupfactors, int *keeppreds_use, int num_scores, char *efffile, int num_preds, char **allpreds, char *allal1, char *allal2, int *predorder, char *bimfile, int num_preds_use, int *keeppreds, int type, int nonsnp)
//type=0 - scores, type=1 - multi-scores, type=2 - find tags
{
int j, j2, j3, r, count, count2, count3, flag;
int *usedpreds, *indexer;
double value, *centres, **factors, **centres2;
char **preds, *al1, *al2;
...
if(count==0)	//add one fake predictor (can not be type=2)
{
printf("Warning, there are no predictors in %s\n", efffile);
for(r=0;r<num_scores;r++)
{
...
if(type==1)
{
centres2=malloc(sizeof(double*)*num_scores);
for(r=0;r<num_scores;r++){centres2[r]=malloc(sizeof(double)*count);}
}
...
if(type==0||type==2) //read one centre, then scores
{
if(fscanf(input, "%s ", readstring)!=1)
{printf("Error reading centre for Predictor %s in %s\n\n", preds[j], efffile);exit(1);}
if(strcmp(readstring,"NA")==0)
...
else    //read pairs of centres and scores
{
for(r=0;r<num_scores;r++)
{
if(fscanf(input, "%s %s ", readstring, readstring2)!=2)
{printf("Error reading Centre and Effect %d for Predictor %s in %s\n\n", r+1, preds[j], efffile);exit(1);}
if(strcmp(readstring,"NA")==0&&strcmp(readstring2,"NA")!=0)
{printf("Error reading Centre and Effect %d for Predictor %s in %s; the centre is NA, but the effect is not NA\n\n", r+1, preds[j], efffile);exit(1);}
if(strcmp(readstring,"NA")!=0&&strcmp(readstring2,"NA")==0)
{printf("Error reading Centre and Effect %d for Predictor %s in %s; the effect is NA, but the centre is not NA\n\n", r+1, preds[j], efffile);exit(1);}

if(strcmp(readstring,"NA")==0&&strcmp(readstring2,"NA")==0)
{centres2[r][j]=-9999;factors[r][j]=0;}
else
{
if(sscanf(readstring, "%lf%c", centres2[r]+j, &readchar)!=1)
{printf("Error reading %s; Centre %d for Predictor %s does not appear to be numeric (%s)\n\n", efffile, r+1, preds[j], readstring);exit(1);}
if(centres2[r][j]<0||centres2[r][j]>2)
{printf("Error reading %s; Centre %d for Predictor %s is %s (all centres should be within [0,2])\n\n", efffile, r+1, preds[j], readstring);exit(1);}
if(sscanf(readstring2, "%lf%c", factors[r]+j, &readchar)!=1)
{printf("Error reading %s; Effect %d for Predictor %s does not appear to be numeric (%s)\n\n", efffile, r+1, preds[j], readstring2);exit(1);}
}
}
}
}	//end of j loop
...
if((type==0||type==1)&&num_preds_use<num_preds)	//allow for filtering
{
...
if(nonsnp==1&&al1[j3]!=allal1[j2])
{printf("Error, the allele order for %s (%c and %c) does not match that in %s (%c and %c); this is required when using \"--SNP-data NO\"\n\n", allpreds[j2], al1[j3], al2[j3], bimfile, allal1[j2], allal2[j2]);exit(1);}

//set centres and effects assuming alleles align, then turn if necessary (note that we might be missing centre)
if(type==0||type==2){blupcentres[r][j]=centres[j3];}
else{blupcentres[r][j]=centres2[r][j3];}
...
free(factors[r]);
if(type==1){free(centres2[r]);}
}	//end of r loop

if(type==0)	//see whether centres provided
{
...
if(count==0){printf("Error, after excluding predictors with zero effect sizes, none remain\n\n");exit(1);}
if(count<count3){printf("Warning, after excluding predictors with zero effect sizes, only %d remain\n", count);}

if(type==1) //check there are some predictors with all centres
{
count2=0;
for(j=0;j<count;j++)
{
...
for(r=0;r<num_scores;r++)
{
if(blupcentres[r][j]==-9999){flag=0;break;}
}
count2+=flag;
}

if(count2==0){printf("Error, no predictors have centres for all scores\n\n");exit(1);}
if(count2<100){printf("Warning, only %d predictors have centres and effects for all scores, so it will be hard to reliably estimate ancestries\n\n", count2);}
}
}	//end of count!=0
printf("\n");
...
//type=0, save scores, type=1, save scores+counts, type=2, scores+sds, type=3, save comb
{
int i, r;
...
if(type==3)	//save combination
{
fprintf(output,"ID1\tID2\tPhenotype\tCovariates\tCombined_Profile\tBLANK\n");
...
fprintf(output, "%.6f\t%.6f\tNA\n", guesses[num_scores][i], guesses[num_scores+1][i]);
}
}
...
if(type==3)	//also save individuals
{
sprintf(filename,"%s.profile.single",outfile);
if((output=fopen(filename,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename);exit(1);}
