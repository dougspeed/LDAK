...
if(exsame==0)
{
if(wcount<5){printf("Warning, at least two predictors are called %s (you can remove these using \"--exclude-same-names YES\")\n", Xpreds[k][predorder[j]]);}
}
else
{
if(wcount<5){printf("Warning, at least two predictors are called %s; these will be ignored\n", Xpreds[k][predorder[j]]);}
...
{
//will record which have consistent alleles
usedpreds=malloc(sizeof(int)*num_preds_use);
for(j=0;j<num_preds_use;j++){usedpreds[j]=1;}

while(1)    //will repeat this if necessary to exclude predictors
{
for(k=0;k<num_files;k++)
{
wantpreds=malloc(sizeof(char*)*XNall[k]);
...
if(usedpreds[j3]==1&&(strcmp(Xalong1[k][j2],along1[j3])!=0||strcmp(Xalong2[k][j2],along2[j3])!=0)&&(strcmp(Xalong1[k][j2],along2[j3])!=0||strcmp(Xalong2[k][j2],along1[j3])!=0))
{
if(exincons==0){printf("Error, the alleles for Predictor %s are not consistent across datasets (%s & %s and %s & %s); to remove predictors with inconsistent alleles use \"--exclude-inconsistent YES\"\n\n", preds[j3], Xalong1[k][j2], Xalong2[k][j2], along1[j3], along2[j3]);exit(1);}

if(wcount<5){printf("Warning, Predictor %s will be excluded because its alleles are not consistent across datasets (%s & %s and %s & %s)\n", preds[j3], Xalong1[k][j2], Xalong2[k][j2], along1[j3], along2[j3]);}
usedpreds[j3]=0;
wcount++;
}
}
}
if(wcount>5){printf("In total, %d predictors have inconsistent alleles and will be excluded\n", wcount);}

if(wcount==0){break;}

//if still here, must be excluding predictors
count=0;
for(j=0;j<num_preds_use;j++)
{
if(usedpreds[j]==1)
...
if(count!=j)
{
chr[count]=chr[j];cm[count]=cm[j];bp[count]=bp[j];al1[count]=al1[j];al2[count]=al2[j];
...
for(j=count;j<num_preds_use;j++){free(preds[j]);free(along1[j]);free(along2[j]);}

if(count==0){printf("Error, no predictors remain\n\n");exit(1);}
printf("The number of predictors has been reduced from %d to %d\n\n", num_preds_use, count);
num_preds_use=count;
...
if((output2=fopen(filename2,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename2);exit(1);}
fprintf(output2,"Predictor A1\tA2\tNon_Missing\tA1_Mean_D1\tMAF_D1\tA1_Mean_D2\tMAF_D2\tCorrelation\tConcordance\n");
}
...
sum=0;sum2=0;sumsq=0;sumsq2=0;sumsq3=0;indcount=0;count=0;
...
count+=(data[(size_t)j*num_samples_use+i]==data2[(size_t)j2*Xnuse[k]+i2]);
indcount++;
}}
...
if(nonsnp==0){fprintf(output2, "%.6f\t%.6f\t%.6f\t%.6f\t", mean, mean/2+(mean>1)*(1-mean), mean2, mean2/2+(mean2>1)*(1-mean2));}
else{fprintf(output2,"%.6f\tNA\t%.6f\tNA\t", mean, mean2);}
...
if(value>.99995||value<-0.99995){fprintf(output2, "%d\t",(value>0)-(value<0));}
else{fprintf(output2, "%.4f\t", value);}
}
else{fprintf(output2, "0\t");}
}
else{fprintf(output2, "NA\t");}
fprintf(output2,"%.4f\n", (double)count/indcount);
}
...
{
printf("Data for %d samples and %d predictors saved in %s, %s and %s, with statistics in %s", resample, count, filename3, filename4, filename5, filename2);
if(passqc==1){printf("; %s indicates which predictors failed quality control",filename6);}
printf("\n\n");
}
}
