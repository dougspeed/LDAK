...
{
int i, count, count2, cflag, rflag, one=1, two=2;
...
if(count2>0)	//test for convergence
{
...
if(count2==maxiter)
{
if(rflag==total2-1){printf("Warning, the optimizer failed to converge; consider using \"--max-iter\" and/or \"--tolerance\" to increase the iteration limit and tolerance (currently %d and %.4e)\n", maxiter, tol);}
cflag=0;
break;
}
...
void mle_fam(int ns, int num_covars, double *Y, double *Z, char *relfile, double prev, double prev2, double tol, int maxiter, char **ids3, char *outfile, int constrain, int type, int cordups, int num_blocks, int flag, double missingvalue)
//type=0 - quant, type=1 - binary
//flag=0, univariate; flag=1, bivariate
{
int i, j, p, start, end, total, total2, total3, cflag;
...
int *order, *firsts, *seconds, *thirds, *tallys;
...
//deal with relfile (not allowed self pairs)

total3=countrows(relfile);
total2=countcols(relfile)-4;

firsts=malloc(sizeof(int)*total3);
seconds=malloc(sizeof(int)*total3);
relats=malloc(sizeof(double)*total3);
relats2=malloc(sizeof(double)*total3);

//read pairs and relatedness
printf("Reading details for %d related pairs from %s\n", total3, relfile);
for(i=0;i<total3;i++){relats2[i]=0;}
if(total2==1){total=read_relfile(relfile, total3, firsts, seconds, relats, NULL, ns, ids3, 0);}
else{total=read_relfile(relfile, total3, firsts, seconds, relats, relats2, ns, ids3, 0);}

if(num_blocks==-1||num_blocks>total){num_blocks=total;}
if(num_blocks!=-9999)	//shuffle order of pairs
{
order=malloc(sizeof(int)*total);
thirds=malloc(sizeof(int)*total);
relats3=malloc(sizeof(double)*total);

for(i=0;i<total;i++){order[i]=i;}
permute_int(order,total);

for(i=0;i<total;i++){thirds[i]=firsts[i];}
for(i=0;i<total;i++){firsts[i]=thirds[order[i]];}
for(i=0;i<total;i++){thirds[i]=seconds[i];}
for(i=0;i<total;i++){seconds[i]=thirds[order[i]];}
...
free(order);free(thirds);free(relats3);
}
...
if(type==0)
{
phen1[i]=Yadj[firsts[i]];
phen2[i]=Yadj[seconds[i]];
}
else
{
phen1[i]=Y[firsts[i]];
phen2[i]=Y[seconds[i]];
thresh1[i]=Yadj[firsts[i]];
thresh2[i]=Yadj[seconds[i]];
}
}
...
if(type==0)
{
phen1[i+total]=Yadj2[firsts[i]];
phen2[i+total]=Yadj2[seconds[i]];
}
else
{
phen1[i+total]=Y[firsts[i]+ns];
phen2[i+total]=Y[seconds[i]+ns];
thresh1[i+total]=Yadj2[firsts[i]];
thresh2[i+total]=Yadj2[seconds[i]];
}
}
}
...
for(i=0;i<total;i++){tallys[firsts[i]]++;tallys[seconds[i]]++;}

if(cordups==1)	//work out weightings for each pair
{
//for(i=0;i<total;i++){weights[i]=pow(tallys[indexer[i]]*tallys[indexer2[i]],-.5);}
for(i=0;i<total;i++){weights[i]=.5*(pow(tallys[firsts[i]],-1)+pow(tallys[seconds[i]],-1));}
}
...
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename3);exit(1);}
fprintf(output3, "Component Log_Odds SE P\n");
...
fprintf(output4, "Component Heritability SE\n");
...
fprintf(output5, "Component Heritability SE\n");
...
fprintf(output2, "Component Value SE\n");
...
if((output4=fopen(filename4,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename4);exit(1);}
fprintf(output4, "Component Log_Odds SE P\n");
fprintf(output4, "Intercept %.4e %.4e %.4e\n", thetas2[0], thetasds2[0], thetapvas2[0]);
for(j=1;j<num_covars;j++){fprintf(output4, "Covariate_%d %.4e %.4e %.4e\n",j, thetas2[j], thetasds2[j], thetapvas2[j]);}
fclose(output4);
...
free(firsts);free(seconds);free(relats);free(relats2);
