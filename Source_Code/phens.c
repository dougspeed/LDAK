...
if(gxe==1)  //uncentre data then scale by environmental variable
{
for(j=0;j<bitlength;j++)
{
for(i=0;i<num_samples_use;i++)
{data[(size_t)j*num_samples_use+i]=(data[(size_t)j*num_samples_use+i]+centres[bitstart+j])*covar[i+num_covars*num_samples_use];}
}
}
...
if(gxe==1)  //must centre breeding values
{
for(m=0;m<num_phenos;m++)
{
sum=0;for(i=0;i<num_samples_use;i++){sum+=unders[m][i];}
mean=sum/num_samples_use;
for(i=0;i<num_samples_use;i++){unders[m][i]-=mean;}
}
}

//have breeding values, now add noise

if(bivar2==-9999)	//do for one trait at a time (might have sample weights)
{
for(m=0;m<num_phenos;m++)
{
...
{
value=pow(var*(1-her)/her,.5);
if(oddhalf==1&&m%2==0){value=pow(var*(1-her/2)/her*2,.5);}
}

//add on noise
for(i=0;i<num_samples_use;i++){phens[m][i]=unders[m][i]+rnorm_safe()*value/sweights[i];}
}
}
else	//jointly sample noise for pairs of traits (will not have sample weights)
{
//get "cholesky"
...
{
value=pow(var*(1-her)/her,.5);
if(oddhalf==1){value=pow(var*(1-her/2)/her*2,.5);}
}
...
if(prev!=-9999){printf("Phenotypes saved in %s, with liabilities in %s, breeding values in %s and details of causal predictors in %s\n\n", filename2, filename5, filename3, filename4);}
else{printf("Phenotypes saved in %s, with breeding values in %s and details of causal predictors in %s\n\n", filename2, filename3, filename4);}
