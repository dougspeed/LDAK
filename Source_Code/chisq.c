...
//at the bottom is code from chatgpt for computing dchisq
...
double dchisq_central(double x, double df)
{
    if (x < 0.0 || df <= 0.0)
        return 0.0;
    ...
    if (x == 0.0) {
        ...
        if (df < 2.0) return INFINITY;
        return 0.0;
    }

    return exp((df/2.0 - 1.0) * log(x)
             - x/2.0
             - (df/2.0) * log(2.0)
             - lgamma(df/2.0));
}
...
double dchisq(double x, double df, double ncp)
{
    if (ncp <= 0.0)
        return dchisq_central(x, df);

    double lambda2 = ncp / 2.0;

    /* Initial Poisson weight */
    ...
    for (int j = 0; j < 10000; j++) {
        ...
        double term = weight * dchisq_central(x, df + 2.0*j);
        sum += term;
        ...
        /* Update Poisson weight:
           w_{j+1} = w_j * lambda2 / (j+1)
        */
        weight *= lambda2 / (j + 1.0);
    }
