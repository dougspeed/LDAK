...
//Gene-based analyses (for num_kins=0, now start by regressing Y and X on Z; with summaries, might have missing values
...
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename2);exit(1);}
fprintf(output2,"Gene_Number Gene_Name Num_Predictors Heritability SE Null_Likelihood Alt_Likelihood LRT_Stat LRT_P_Raw");
...
stand_data(data+(size_t)shuffle*num_samples_use, centres+gstarts[g]+shuffle, mults+gstarts[g]+shuffle, sqdevs+gstarts[g]+shuffle, rates+gstarts[g]+shuffle, infos+gstarts[g]+shuffle, num_samples_use, total-shuffle, missingvalue, power, 0, hwestand, weights+gstarts[g]+shuffle, 1);
...
{
if(strcmp(sumsfile,"blank")==0) //find number of non-trivial predictors (indexed by retain)
{count3=prune_gene(retain, gprune, data, num_samples_use, total, mults+gstarts[g], weights+gstarts[g], X, XTCX, NULL, NULL, NULL, &var2);}
else    //find number of non-missing predictors, and also get a weighted variance explained
{count3=prune_gene(retain, gprune, data, num_samples_use, total, mults+gstarts[g], weights+gstarts[g], X, XTCX, nss+gstarts[g], rhos+gstarts[g], chis+gstarts[g], &var2);}

if(count3==0)	//gene trivial
{
printf("Warning, all %d predictors in Gene/Chunk %s are trivial", total, gnames[g]);
if(strcmp(weightsfile,"blank")!=0){printf(" or have weight zero");}
if(strcmp(sumsfile,"blank")!=0){printf(" or are missing summary statistics");}
printf("\n");
