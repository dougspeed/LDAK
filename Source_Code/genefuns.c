...
{
//type=0 - standard cutting, type=1 - condensing, type=2 - finding tags, type=3 - same as type=0 but folder is a filename, type=4 - pathway cutting
...
read_genefile(genefile, gnames, gchr, gbp1, gbp2, gstrand, (type==4));}
...
if(overlap==0)	//deal with overlap between genes (might result in genes becoming empty)
{
for(g=0;g<count;g++)
{
if(gstarts[g]!=-9999)
...
if(type==0||type==3||type==4)	//print out
{
if(type==0||type==4){sprintf(filename,"%sgenes.details",folder);}
...
else{gstarts[g]=-9999;gends[g]=-9999;}
}}	//end of using g and g loop

fclose(output);

//store the genic predictors
if(type==0||type==4){sprintf(filename2,"%sgenes.predictors.used",folder);}
...
{
if(type==0||type==4){sprintf(filename3,"%sgenes.distances",folder);}
...
int prune_gene(int *retain, double gprune, double *data, int ns, int length, double *mults, double *weights, double *data2, double *cors, double *nss, double *rhos, double *chis, double *var2)
...
if(nss==NULL)  //set retain to exclude trivial
{
for(j=0;j<length;j++){retain[j]=(mults[j]!=-9999&&weights[j]>0);}
}
else    //set retain to exclude trivial and those missing summaries
{
for(j=0;j<length;j++){retain[j]=(mults[j]!=-9999&&weights[j]>0&&nss[j]>0);}
}
count=0;for(j=0;j<length;j++){count+=retain[j];}

if(gprune<=1||nss!=NULL)	//get correlations
{
//get correlations
...
if(nss!=NULL)	//get weighted variance explained
{
*var2=0;
for(j=0;j<length;j++)
{
