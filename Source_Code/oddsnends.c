...
void fprint_top(double *mat, int a, int b, char *filename)
{
int i, j;
FILE *output;


if((output=fopen(filename,"w"))==NULL){printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename);exit(1);}

for(i=0;i<a;i++)
{
for(j=0;j<b;j++){fprintf(output,"%.4e ", mat[i+j*a]);}
fprintf(output,"\n");
}
fclose(output);
}
...
for(i=0;i<a;i++)
{
for(j=0;j<b;j++){printf("%.4e ", mat[i+j*c]);}
printf("\n");
}
}
...
void print_top_single(float *mat, int a, int b, int c)
{
int i, j;
...
void print_top_int(int *mat, int a, int b, int c)
{
int i, j;
...
for(i=0;i<a;i++)
{
for(j=0;j<b;j++){printf("%d ", mat[i+j*c]);}
printf("\n");
}
}
...
if((input=fopen(filename,"r"))==NULL){printf("Error opening %s\n\n",filename);exit(1);}

if(fscanf(input,"%c",&readchar)!=1){printf("Error, %s appears to be empty\n\n", filename);exit(1);}
if(readchar==10){printf("Error, %s appears to start with an empty line\n\n", filename);exit(1);}
if(readchar==9||readchar==32){printf("Error, %s appears to start with a space\n\n", filename);exit(1);}
...
if(readchar==9||readchar==32){printf("Error, Row %d of %s appears to start with a space\n\n", count+1, filename);exit(1);}
}
}
...
{
if(fscanf(input,"%c",&readchar2)==1){printf("Error, Row %d of %s appears to be empty\n\n", count+1, filename);exit(1);}
break;
}
}
}
...
{
if(count2!=ncols){printf("Error reading %s, Row %d appears to have %d columns (whereas Row 1 has %d columns)\n\n", filename, count+1, count2, ncols);exit(1);}
count++;
}
...
if(readchar==9||readchar==32){printf("Error reading %s, Row %d appears to start with a space\n\n", filename, count+1);exit(1);}
...
void fillcols(char *filename, int *numcols, int length)
{
int j, count;
...
for(j=0;j<length;j++)
{
if(fscanf(input,"%c",&readchar)!=1){printf("Error, Row %d of %s appears to be empty\n\n", j+1, filename);exit(1);}
if(readchar==10){printf("Error, Row %d of %s appears to start with an empty line\n\n", j+1, filename);exit(1);}
if(readchar==9||readchar==32){printf("Error, Row %d of %s appears to start with a space\n\n", j+1, filename);exit(1);}

count=1;readchar2=readchar;
while(fscanf(input,"%c", &readchar)==1)
{
if(readchar==10){numcols[j]=count;break;}
if((readchar2==9||readchar2==32)&&readchar!=9&&readchar!=32){count++;}
readchar2=readchar;
}
}
...
if(count<0){printf("Error reading %s; has %d rows but should have at least %d\n\n", strfile, count+head, head+1);exit(1);}
if(indexer!=NULL)
{
if(indexer[length-1]>=count){printf("Error reading %s; has %d rows but should have at least %d\n\n", strfile, count+head, head+indexer[length-1]+1);exit(1);}
}

if(countcols(strfile)<col){printf("Error reading %s; has %d columns but should have at least %d\n\n", strfile, countcols(strfile), col);exit(1);}

//open and skip headers
if((input=fopen(strfile,"r"))==NULL){printf("Error opening %s\n\n", strfile);exit(1);}
j=0;
...
for(k=0;k<col-1;k++)
{
if(fscanf(input, "%s ", rs)!=1){printf("Error reading Element %d of Row %d of %s\n\n", k+1, head+j+1, strfile);exit(1);}
}
if(fscanf(input, "%s%c", rs, &readchar)!=2){printf("Error reading Element %d of Row %d of %s\n\n", col, head+j+1, strfile);exit(1);}
if(j==want)
{
...
if(count<0){printf("Error, %s has %d rows but should have at least %d\n\n", valfile, count+head, head+1);exit(1);}
if(indexer!=NULL)
{
if(indexer[length-1]>=count){printf("Error, %s has %d rows but should have at least %d\n\n", valfile, count+head, head+indexer[length-1]+1);exit(1);}
}

if(countcols(valfile)<col){printf("Error, %s has %d columns but should have at least %d\n\n", valfile, countcols(valfile), col);exit(1);}

//open and skip headers
if((input=fopen(valfile,"r"))==NULL){printf("Error opening %s\n\n", valfile);exit(1);}
j=0;
while(j<head)	//skip rows
{
readchar=0;while(readchar!=10){readchar=10;(void)fscanf(input, "%c", &readchar);}j++;
...
if(strcmp(rs,"NA")==0)
{
if(miss==0){printf("Error reading %s; Element %d of Row %d is NA\n\n", valfile, col, head+j+1);exit(1);}
values[found]=-9999;
}
else
{
if(sscanf(rs, "%lf%c", values+found, &readchar)!=1){printf("Error reading %s; Element %d of Row %d does not appear to be numeric (%s)\n\n", valfile, col, head+j+1, rs);exit(1);}
}
...
{
if(sscanf(rs, "%d%c", values+found, &readchar)!=1){printf("Error reading %s; Element %d of Row %d does not appear to be an integer (%s)\n\n", valfile, col, head+j+1, rs);exit(1);}
}
...
{
if(fscanf(input, "%s%c", rs, &readchar)!=2){printf("Error reading Element %d of Row %d of %s\n\n", col, head+j+1, valfile);exit(1);}
if(strcmp(rs,"NA")!=0){printf("Error reading %s; Element %d of Row %d is unrecognisable (%s)\n\n", valfile, col, head+j+1, rs);exit(1);}
}
if(j==want)
{
...
rs1=malloc(sizeof(char)*1000000);rs2=malloc(sizeof(char)*1000000);
...
if(countcols(idsfile)<shift+2){printf("Error reading %s; has %d columns but should have at least %d\n\n", idsfile, countcols(idsfile),shift+2);exit(1);}

if((input=fopen(idsfile,"r"))==NULL){printf("Error opening %s\n\n",idsfile);exit(1);}
...
if(fscanf(input, "%s %s%c", rs1, rs2, &readchar)!=3){printf("Error reading Elements %d and %d from Row %d of %s\n", shift+1, shift+2, head+i+1, idsfile);exit(1);}
...
if((input=fopen(bgenfile,"rb"))==NULL){printf("Error opening %s\n\n", bgenfile);exit(1);}

fseeko(input, 4, SEEK_SET);
if(fread(&hblock, 4, 1, input)!=1){printf("Error reading second value of %s\n\n", bgenfile);exit(1);}

fseeko(input, 12, SEEK_SET);
if(fread(&count, 4, 1, input)!=1){printf("Error reading fourth value of %s\n\n", bgenfile);exit(1);}

fseeko(input, 4+hblock+4, SEEK_SET);
if(fread(&count2, 4, 1, input)!=1){printf("Error reading second sample header value of %s\n\n", bgenfile);exit(1);}
...
if(fread(&idlen, 2, 1, input)!=1){printf("Error reading length of Sample %d from %s\n\n", i+1, bgenfile);exit(1);}
if(fread(rs, 1, idlen, input)!=idlen){printf("Error reading name of Sample %d from %s\n\n", i+1, bgenfile);exit(1);}
rs[idlen]='\0';
...
if(hwestand==0){printf("where V is the observed variance of the predictor; to instead scale based on expected variance use \"--hwe-stand YES\"\n\n");}
else
...
head=0;
if((input=fopen(filename,"r"))==NULL){printf("Error opening %s\n\n", filename);exit(1);}
if(fscanf(input, "%s %s ", readstring, readstring2)!=2){printf("Error reading first two elements of %s\n\n", filename);exit(1);}
...
head=0;
if((input=fopen(filename,"r"))==NULL){printf("Error opening %s\n\n", filename);exit(1);}
if(fscanf(input, "%s ", readstring)!=1){printf("Error reading first element of %s\n\n", filename);exit(1);}
...
if(length==-1){length=countcols(filename);}
readlist=malloc(sizeof(char*)*length);

if((input=fopen(filename,"r"))==NULL){printf("Error opening %s\n\n", filename);exit(1);}
for(j=0;j<length;j++)
{
if(fscanf(input, "%s ", rs)!=1){printf("Error reading name of Column %d of %s\n\n", j+1, filename);exit(1);}
copy_string(readlist, j, rs);
for(k=0;k<j;k++)
{
if(strcmp(readlist[k],readlist[j])==0){printf("Error reading %s; Columns %d and %d have the same name (%s)\n\n", filename, k+1, j+1, readlist[k]);exit(1);}
}
}
fclose(input);
...
{
if(type==1){printf("Warning reading %s; the PID (%s) for Sample %s %s does not match that previously seen (%s)\nAll PIDs will be set to missing (0)\n\n",filename,name,id1,id2,str[i]);}
if(type==2){printf("Warning reading %s; the MID (%s) for Sample %s %s does not match that previously seen (%s)\nAll MIDs will be set to missing (0)\n\n",filename,name,id1,id2,str[i]);}
if(type==3){printf("Warning reading %s; the sex (%s) for Sample %s %s does not match that previously seen (%s)\nAll sexes will be set to missing (0)\n\n",filename,name,id1,id2,str[i]);}
if(type==4){printf("Warning reading %s; the phenotype (%s) for Sample %s %s does not match that previously seen (%s)\nAll phenotypes will be set to missing (NA)\n\n",filename,name,id1,id2,str[i]);}
return(1);
}
...
if(size>1){printf("Warning, to store the region predictors requires %.1f Gb\n\n", size);}
}
...
if(size>1){printf("Warning, to store the eigen-decomposition requires %.1f Gb; sorry, this can not be reduced\n\n", size);}
}
...
if(size>1){printf("Warning, to perform the decomposition requires %.1f Gb; sorry, this can not be reduced\n\n", size);}
}
...
if(size>1){printf("Warning, to process the data requires %.1f Gb; sorry this can not be reduced\n\n", size);}
}
...
size=(double)length1/1024*length2/1024*8/1024;
if(size>1){printf("Warning, to process the data requires %.1f Gb; if this is too high, you can reduce using \"--bit-size\" (currently %d)\n\n", size, length1);}
}
...
size=(double)length1/1024*length2/1024*8/1024;
if(size>1){printf("Warning, to process the data requires %.1f Gb; if this is too high, you can reduce using \"--bit-size\"\n\n", size);}
}
...
size=(double)length1/1024*length2/1024*8/1024;
if(size>1){printf("Warning, to perform the analysis requires %.1f Gb\n\n", size);}
}
...
size=(double)length1/1024*length2/1024*8/1024;
if(size>1){printf("Warning, to perform the analysis requires %.1f Gb; if this is too high, you can reduce using \"--bit-size\" (currently %d)\n\n", size, length1);}
}
...
size=(double)length1/1024*length2/1024*8/1024;
if(size>1){printf("Warning, to store the heritability model requires %.1f Gb\n\n", size);}
}
...
if(covarnums[strlen(covarnums)-1]==','){printf("Error, reading the argument to \"--covar-numbers\" (%s); the final character can not be a comma\n\n", covarnums);exit(1);}
if(covarnums[strlen(covarnums)-1]=='-'){printf("Error, reading the argument to \"--covar-numbers\" (%s); the final character can not be a dash\n\n", covarnums);exit(1);}
...
if(covarnames[strlen(covarnames)-1]==','){printf("Error, reading the argument to \"--covar-names\" (%s); the final character can not be a comma\n\n", covarnames);exit(1);}

//check covarfile has header, and none of the names contain a comma
if((input=fopen(covarfile,"r"))==NULL){printf("Error opening %s\n\n", covarfile);exit(1);}
readchar=0;
while(readchar!=10)
{
...
for(j=0;j<max;j++)
{
if(indexer[j]>1){printf("Error, the argument to \"--covar-names\" (%s) specifies Covariate %d more than once\n\n", covarnames, j+1);exit(1);}
}
...
void check_windows(char *filename, int length, char *filename2, int length2)
{
int j, found;

int *startchr, *startbp, *endchr, *endbp;
int *startchr2, *startbp2, *endchr2, *endbp2;
...
startchr=malloc(sizeof(int)*length);
startbp=malloc(sizeof(int)*length);
endchr=malloc(sizeof(int)*length);
endbp=malloc(sizeof(int)*length);

startchr2=malloc(sizeof(int)*length2);
startbp2=malloc(sizeof(int)*length2);
endchr2=malloc(sizeof(int)*length2);
endbp2=malloc(sizeof(int)*length2);


//read first set of windows
if((input=fopen(filename,"r"))==NULL){printf("Error re-opening %s\n\n",filename);exit(1);}
readchar=0;while(readchar!=10){readchar=10;(void)fscanf(input, "%c", &readchar);}

for(j=0;j<length;j++)
{
if(fscanf(input, "%d %d %d %d:%d %d:%d ", &readint, &readint, &readint, startchr+j, startbp+j, endchr+j, endbp+j)!=7){printf("Error reading Row %d of %s\n\n", j+2, filename);exit(1);}
}
fclose(input);
...
for(j=1;j<length;j++)
{
if(startchr[j]==endchr[j-1]&&startbp[j]<=endbp[j-1]){printf("Error, Windows %d and %d in %s are not ordered, suggesting the file has been changed since creation with \"--calc-cors\" or \"--join-cors\"\n\n", j, j+1, filename);exit(1);}
}

//read second set of windows
if((input2=fopen(filename2,"r"))==NULL){printf("Error re-opening %s\n\n",filename2);exit(1);}
readchar=0;while(readchar!=10){readchar=10;(void)fscanf(input2, "%c", &readchar);}j++;

for(j=0;j<length2;j++)
{
if(fscanf(input2, "%d %d %d %d:%d %d:%d ", &readint, &readint, &readint, startchr2+j, startbp2+j, endchr2+j, endbp2+j)!=7){printf("Error reading Row %d of %s\n\n", j+2, filename2);exit(1);}
}
fclose(input2);
...
for(j=1;j<length2;j++)
{
if(startchr2[j]==endchr2[j-1]&&startbp2[j]<=endbp2[j-1]){printf("Error, Windows %d and %d in %s are not ordered, suggesting the file has been changed since creation with \"--calc-cors\" or \"--join-cors\"\n\n", j, j+1, filename2);exit(1);}
}

found=0;
for(j=0;j<length;j++)
{
//increase found until get to window j
while(startchr[j]>endchr2[found]||(startchr[j]==endchr2[found]&&startbp[j]>endbp2[found]))
{
found++;
if(found==length2){break;}
}

if(found>=length2-1){break;}

//start of window j is before end of window found, and there is a window found+1
if(endchr[j]>startchr2[found+1]||(endchr[j]==startchr2[found+1]&&endbp[j]>startbp2[found+1])){printf("Error, Window %d of %s (%d:%d to %d:%d) overlaps with Windows %d and %d of %s (%d:%d to %d:%d and %d:%d to %d:%d); make sure that the correlations were created using the same breakpoints\n\n", j+1, filename, startchr[j], startbp[j], endchr[j], endbp[j], found+1, found+2, filename2, startchr2[found], startbp2[found], endchr2[found], endbp2[found], startchr2[found+1], startbp2[found+1], endchr2[found+1], endbp2[found+1]);exit(1);}
}

free(startchr);free(startbp);free(endchr);free(endbp);
free(startchr2);free(startbp2);free(endchr2);free(endbp2);
}

///////////////////////////

void abs_median(double *X, double *Yadj, double *Y, int ns, int *order, double missingvalue, int c0, int c1, int c2)
{
int i, f0, f1, f2, d0, d1, d2, s0, s1, s2, count, need;
double m0, m1, m2, sum, mean;


//how many non-missing genotypes
count=c0+c1+c2;

//how many medians do we need to find?
need=(c0>0)+(c1>0)+(c2>0);

//work out first and second values we require for each genotype (might be same)
f0=(c0-1)/2;s0=c0/2;
f1=(c1-1)/2;s1=c1/2;
f2=(c2-1)/2;s2=c2/2;

//loop through individuals until find required values
d0=0;d1=0;d2=0;
for(i=0;i<ns;i++)
{
if(X[order[i]]==0)
{
if(d0==f0){m0=Y[order[i]]/2;}
if(d0==s0){m0+=Y[order[i]]/2;need--;}
d0++;
}
if(X[order[i]]==1)
{
if(d1==f1){m1=Y[order[i]]/2;}
if(d1==s1){m1+=Y[order[i]]/2;need--;}
d1++;
}
if(X[order[i]]==2)
{
if(d2==f2){m2=Y[order[i]]/2;}
if(d2==s2){m2+=Y[order[i]]/2;need--;}
d2++;
}
if(need==0){break;}
}

//now compute absolute differences (for non-missing genotypes) and get mean
sum=0;
for(i=0;i<ns;i++)
{
if(X[i]==0){Yadj[i]=fabs(Y[i]-m0);sum+=Yadj[i];}
if(X[i]==1){Yadj[i]=fabs(Y[i]-m1);sum+=Yadj[i];}
if(X[i]==2){Yadj[i]=fabs(Y[i]-m2);sum+=Yadj[i];}
}
mean=sum/count;

if(count<ns)    //for missing genotypes, set phenotypes to mean
{
for(i=0;i<ns;i++)
{
if(X[i]==missingvalue){Yadj[i]=mean;}
}
}
}
...
double mean_gig(double rho, double a, double b, int type)
//evaluate either E(x) or E(1/x) when x has gig distribution - only works when rho=0.5
//notation matches https://en.wikipedia.org/wiki/Generalized_inverse_Gaussian_distribution
{
double value, value2;


if(rho!=0.5){printf("Error, mean_gig only work swhen rho=0.5 (not %f)\n\n", rho);exit(1);}

if(type==0)
{
value=pow(a*b,.5);
value2=pow(b/a,.5)*(value+1)/value;
}
if(type==1)
{
value=pow(a*b,.5);
value2=pow(a/b,.5)*(value+1)/value-2*rho/b;
}
...
double gig_psi(double x, double alpha, double lam)
{
return(-alpha*(cosh(x)-1.0)-lam*(exp(x)-x-1.0));
}

double gig_dpsi(double x, double alpha, double lam)
{
return(-alpha*sinh(x)-lam*(exp(x)-1.0));
}

double gig_g(double x, double sd, double td, double f1, double f2)
{
...
double sample_gig(double rho, double a, double b)
//sample from gig distribution
//notation matches https://en.wikipedia.org/wiki/Generalized_inverse_Gaussian_distribution
//based on "Random variate generation for the generalized inverse gaussian distribution", Luc Devroye 2012
{
int swap;
double lam, omega, alpha, x, t, s, value;
double eta, zeta, theta, xi, p, r, td, sd, q, U, V, W, f1, f2, rnd;


//will generate using the two-parameter version
lam=rho;
omega=pow(a*b,.5);

swap=0;
if(lam<0){lam=-lam;swap=1;}

alpha=pow(pow(omega,2)+pow(lam,2),.5)-lam;

//compute x
x=-gig_psi(1.0,alpha,lam);

//find t (value depends on x)

if(x<0.5)
{
if(alpha==0&&lam==0){t=1.0;}
else{t=log(4.0/(alpha+2.0*lam));}
}
...
{
if(alpha==0&&lam==0){t=1.0;}
else{t=pow(2.0/(alpha+lam),.5);}
}

//find s (value depends on x)
...
{
value=log(1.0+1.0/alpha+pow(pow(alpha,-2)+2.0/alpha,.5));
if(lam==0){s=value;}
else
{
if(value<1.0/lam){s=value;}
else{s=1.0/lam;}
}
}
}

if(x>=0.5&&x<=2.0){s=1.0;}

if(x>2.0)
{
if(alpha==0&&lam==0){s=1.0;}
else{s=pow(4.0/(alpha*cosh(1)+lam),.5);}
}

//other parameters
eta=-gig_psi(t, alpha, lam);
zeta=-gig_dpsi(t, alpha, lam);
theta=-gig_psi(-s, alpha, lam);
xi=gig_dpsi(-s, alpha, lam);
...
if(U<q/(q+p+r)){rnd=-sd+q*V;}
else
{
if(U<(q+r)/(p+q+r)){rnd=td-r*log(V);}
else{rnd=-sd+p*log(V);}
}
...
if(W*gig_g(rnd, sd, td, f1, f2)<=exp(gig_psi(rnd,alpha,lam))){break;}
}

//transform back to the three-parameter version
rnd=exp(rnd)*(lam/omega+pow(1.0+pow(lam,2)*pow(omega,-2),.5));
if(swap==1){rnd=1.0/rnd;}
