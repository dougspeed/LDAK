...
for(r=0;r<num_extra;r++)	//and for rands and regions
{
...
{
for(i=0;i<ns;i++){RHS_single[i+(num_fixed+1+g)*ns]=R_single[i+g*ns];}
}
...
for(g=0;g<num_vects;g++)
{
for(i=0;i<ns;i++){VR[i+g*ns]=RHS_single[i+(num_fixed+1+g)*ns];}
}
}	//end of shortcut=2
