...
int num_fixed, num_extra, total, total2, cflag, rflag, *fixed, *fixedsave, nlost, nlost2;
...
//set num_fixed, num_extra, nfree, total and stotal
num_fixed=num_covars+num_envs+num_tops;
num_extra=num_rands+num_regs;
nfree=ns-num_fixed;
total=1+num_kins+num_extra;
...
if(num_kins+num_extra>0)
{
mg=malloc(sizeof(double*)*(num_kins+num_extra));
for(k=0;k<num_kins+num_extra;k++){mg[k]=malloc(sizeof(double)*ns);}
mg2=malloc(sizeof(double*)*(num_kins+num_extra));
for(k=0;k<num_kins+num_extra;k++){mg2[k]=malloc(sizeof(double)*ns);}
}
...
for(r=0;r<num_extra;r++){scales[1+num_kins+r]=1;}

//set starting vars, hers, shares and fixed
if(total==1){hers[0]=1;}
else
{
if(strcmp(hersfile,"blank")!=0)	//read from file - have already checked correct size and their sum (will have num_rands=0)
{
...
if(shortcut==0||shortcut==1){he_starts(hers, ns, num_covars, num_envs, num_tops, num_kins, num_extra, Y, Z, Mkins, NULL, kintraces, X, Xtotal, Xstarts, Xends, Xsums, memsave, maxthreads, kinstems, ids3, missingvalue);}
else{he_starts(hers, ns, num_covars, num_envs, num_tops, num_kins, num_extra, Y, Z, NULL, Mkins_single, kintraces, X, Xtotal, Xstarts, Xends, Xsums, memsave, maxthreads, kinstems, ids3, missingvalue);}
...
for(r=0;r<num_rands;r++){printf("Her_C%d\t", r+1);}
...
for(r=0;r<num_rands;r++){fprintf(output, "Her_C%d\t", r+1);}
...
//print update
sum=0;for(k=0;k<num_kins+num_extra;k++){sum+=hers[1+k];}
...
for(k=0;k<num_kins+num_extra;k++){printf("%.4f\t", hers[1+k]);}
printf("%.4f\t", sum);
...
for(k=0;k<num_kins+num_extra;k++){fprintf(output, "%.6f\t", hers[1+k]);}
...
if(num_kins+num_extra==0){break;}	//null model
if(nlost>=num_kins+num_extra){printf("All heritabilities are constrained\n");break;}
...
for(k=0;k<num_kins+num_extra;k++)	//random effects are Kg invV Yadj = g KPY - get also g PY (redundant for regions)
{
...
{
if(vars[1+num_kins+num_rands+r]!=0)
{
for(j=Xstarts[num_rands+r];j<Xends[num_rands+r];j++)
{
j2=regindex[r][1+j-Xstarts[num_rands+r]];
effects[r][rkeeppreds[j2]]+=XTPY[j]*vars[1+num_kins+num_rands+r]/Xsums[num_rands+r]*rmults[j2]*pow(rweights[j2],.5);
...
if(num_covars>1||num_tops>0){printf("\n");}
...
for(r=0;r<num_extra;r++){sum+=Xsums[r];}
}

sprintf(filename2,"%s.reml", outfile);
if((output2=fopen(filename2,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename2);exit(1);}
fprintf(output2, "Num_Kinships %d\nNum_Randoms %d\nNum_Regions %d\nNum_Top_Predictors %d\nNum_Covariates %d\nNum_Environments %d\n", num_kins, num_rands, num_regs, num_tops, num_covars, num_envs);
if(num_kins+num_extra>0){fprintf(output2, "Blupfile %s.indi.blp\n", outfile);}
...
if(num_kins+num_extra==1){fprintf(output2, "LRT_Stat %.4f\nLRT_P %.4e\n", lrtstat, lrtpva);}
...
for(r=0;r<num_rands;r++){fprintf(output2, "Her_C%d %.6f %.6f %.2f %.6f %.6f\n", r+1, hers[1+num_kins+r], hersds[1+num_kins+r], Xsums[r], hers[1+num_kins+r]/Xsums[r]*1000000, hersds[1+num_kins+r]/Xsums[r]*1000000);}
for(r=0;r<num_regs;r++){fprintf(output2, "Her_R%d %.6f %.6f %.2f %.6f %.6f\n", r+1, hers[1+num_kins+num_rands+r], hersds[1+num_kins+num_rands+r], Xsums[num_rands+r], hers[1+num_kins+num_rands+r]/Xsums[num_rands+r]*1000000, hersds[1+num_kins+num_rands+r]/Xsums[num_rands+r]*1000000);}
fprintf(output2, "Her_Top %.6f NA NA NA NA\n", topher);
if(num_kins+num_extra==0){fprintf(output2, "Her_All %.6f NA NA NA NA\n", topher);}
...
for(r=0;r<num_rands;r++){fprintf(output2, "Her_C%d %.6f %.6f NA NA NA\n", r+1, hers[1+num_kins+r], hersds[1+num_kins+r]);}
for(r=0;r<num_regs;r++){fprintf(output2, "Her_R%d %.6f %.6f NA NA NA\n", r+1, hers[1+num_kins+num_rands+r], hersds[1+num_kins+num_rands+r]);}
fprintf(output2, "Her_Top %.6f NA NA NA NA\n", topher);
...
for(r=0;r<num_rands;r++){fprintf(output4, "Share_C%d %.6f %.6f %.6f %.6f %.6f\n", r+1, shares[1+num_kins+r], sharesds[1+num_kins+r], Xsums[r]/sum, shares[1+num_kins+r]/Xsums[r]*sum, sharesds[1+num_kins+r]/Xsums[r]*sum);}
for(r=0;r<num_regs;r++){fprintf(output4, "Share_R%d %.6f %.6f %.6f %.6f %.6f\n", r+1, shares[1+num_kins+num_rands+r], sharesds[1+num_kins+num_rands+r], Xsums[num_rands+r]/sum, shares[1+num_kins+num_rands+r]/Xsums[num_rands+r]*sum, sharesds[1+num_kins+num_rands+r]/Xsums[num_rands+r]*sum);}
}
else
{
for(k=0;k<num_kins;k++){fprintf(output4, "Share_K%d %.6f %.6f NA NA NA\n", k+1, shares[1+k], sharesds[1+k]);}
for(r=0;r<num_rands;r++){fprintf(output4, "Share_C%d %.6f %.6f NA NA NA\n", r+1, shares[1+num_kins+r], sharesds[1+num_kins+r]);}
for(r=0;r<num_regs;r++){fprintf(output4, "Share_R%d %.6f %.6f NA NA NA\n", r+1, shares[1+num_kins+num_rands+r], sharesds[1+num_kins+num_rands+r]);}
}
fclose(output4);
}
...
for(r=0;r<num_rands;r++){fprintf(output5, "Var_C%d %.6f %.6f\n", r+1, vars[1+num_kins+r], varsds[1+num_kins+r]);}
for(r=0;r<num_regs;r++){fprintf(output5, "Var_R%d %.6f %.6f\n", r+1, vars[1+num_kins+num_rands+r], varsds[1+num_kins+num_rands+r]);}
...
for(r=0;r<num_extra;r++){fprintf(output7, "0\t%.6f\t", mg[num_kins+r][i]);}
fprintf(output7, "\n");
}
fclose(output7);
}
...
for(r=0;r<num_rands;r++){fprintf(output10, "Her_C%d\t", r+1);}
...
if(num_kins+num_extra>0){fprintf(output, "Blupfile %s.indi.blp.liab\n", outfile);}
...
if(num_kins+num_extra==1){fprintf(output, "LRT_Stat %.4f\nLRT_P %.4e\n", lrtstat, lrtpva);}
...
for(r=0;r<num_rands;r++){fprintf(output, "Her_C%d %.6f %.6f %.2f %.6f %.6f\n", r+1, hers[1+num_kins+r]*factor, hersds[1+num_kins+r]*factor, Xsums[r], hers[1+num_kins+r]/Xsums[r]*1000000*factor, hersds[1+num_kins+r]/Xsums[r]*1000000*factor);}
for(r=0;r<num_regs;r++){fprintf(output, "Her_R%d %.6f %.6f %.2f %.6f %.6f\n", r+1, hers[1+num_kins+num_rands+r]*factor, hersds[1+num_kins+num_rands+r]*factor, Xsums[num_rands+r], hers[1+num_kins+num_rands+r]/Xsums[num_rands+r]*1000000*factor, hersds[1+num_kins+num_rands+r]/Xsums[num_rands+r]*1000000*factor);}
fprintf(output, "Her_Top %.6f NA NA NA NA\n", topher*factor);
if(num_kins+num_extra==0){fprintf(output, "Her_All %.6f NA NA NA NA\n", topher*factor);}
...
for(r=0;r<num_rands;r++){fprintf(output, "Her_C%d %.6f %.6f NA NA NA\n", r+1, hers[1+num_kins+r]*factor, hersds[1+num_kins+r]*factor);}
for(r=0;r<num_regs;r++){fprintf(output, "Her_R%d %.6f %.6f NA NA NA\n", r+1, hers[1+num_kins+num_rands+r]*factor, hersds[1+num_kins+num_rands+r]*factor);}
fprintf(output, "Her_Top %.6f NA NA NA NA\n", topher*factor);
...
for(r=0;r<num_extra
;r++){fprintf(output, "0\t%.6f\t", mg[num_kins+r][i]*pow(factor,.5));}
fprintf(output, "\n");
}
fclose(output);
...
if(num_kins+num_extra>0)
{
for(k=0;k<num_kins+num_extra;k++){free(mg[k]);}free(mg);
for(k=0;k<num_kins+num_extra;k++){free(mg2[k]);}free(mg2);
}
if(num_regs>0||num_tops>0)
{
