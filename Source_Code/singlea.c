...
//will divide thetas and SDs by prop non-missing
...
{
count=0;for(i=0;i<num_samples_use;i++){count+=(Y[i]!=missingvalue);}
...
{
//get thetas, adjust response and get variance
...
{Pspamax[m]=2000*pow(num_samples_use*Pvarphens[m],-.5);}
...
if(mults[bitstart+j]!=-9999)	//will be testing - remember to scale theta and SD by Pscales
{
...
stats[1+mark]=pow(value2,.5)*Pscales[m];
stats[2+mark]=stats[0+mark]/stats[1+mark];
...
spastatus[j]=spa_test(YTdata[j+m*bitlength], data+(size_t)j*num_samples_use, num_samples_use, num_knots, knots+m*num_knots, num_bins, bins, CGF0+m*num_knots, CGF1+m*num_knots, CGF2+m*num_knots, CGF3+m*num_knots, stats+mark, 1);
}
