...
//Get per-predictor heritabilities using fast-he (data must be in binary format) - also get savethetas
...
if(num_fixed>1)	//adjust data
{
if(dichot==0||mpheno==-1)	//can use actual residuals
{reg_covar_thetas(savethetas+bitstart*num_fixed, data, Z, num_samples_use, bitlength, num_fixed, NULL, 1);}
else	//hard to use proper residuals (requires multiple copies of data), so instead use non-weighted residuals
...
if(value2>maxcor)	//blank the column of RTdata and regress top predictor out of Yadj2
{
