...
current=read_data_fly(datafile, dtype, data2, NULL, num_samples_use, keepsamps, 0, numb, keepb, datainputgz, current, num_samples, num_preds, genskip, genheaders, genprobs, bedbytes, missingvalue, -9999, -9999, nonsnp, maxthreads);

stand_data(data2, bcentres, bmults, bsqdevs, num_samples_use, numb, missingvalue, -1, 0, 0, NULL, 1, bpreds);
...
current=read_data_fly(datafile, dtype, data, NULL, num_samples_use, keepsamps, bitstart, bitend, keepa, datainputgz, current, num_samples, num_preds, genskip, genheaders, genprobs, bedbytes, missingvalue, -9999, -9999, nonsnp, maxthreads);

stand_data(data, acentres+bitstart, amults+bitstart, asqdevs+bitstart, num_samples_use, bitlength, missingvalue, -1, 0, 0, NULL, 1, apreds+bitstart);
...
free(Z);
free(cors);
free(acentres);free(amults);free(asqdevs);free(bcentres);free(bmults);free(bsqdevs);
