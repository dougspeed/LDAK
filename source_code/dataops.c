...
if(dtype==1)
{read_bed_fly(datafile, data+(size_t)(threadstart-start)*num_samples_use, num_samples_use, keepsamps, threadstart, threadend, keeppreds_use, num_samples, num_preds, bedbytes);}
...
//type=3 - calc centre and mult, then set missing to mean
{
...
for(j=0;j<length;j++)
{
if(type==0||type==1||type==3)	//calculate centre, mult and sqdev
{
