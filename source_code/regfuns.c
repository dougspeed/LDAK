...
void copy_matrix(int ns, int length, double *X, double *X2, int type, double *nullweights)
//type=0, just copy; type=1, scale by nullweights; type=2, permute
...
if(type==1)	//copy and scale by nullweights
{
#pragma omp parallel for private(j,i) schedule(static)
for(j=0;j<length;j++)
{
for(i=0;i<ns;i++){X2[(size_t)j*ns+i]=X[(size_t)j*ns+i]*nullweights[i];}
}
}
...
void impute_matrix_missing(double *X, int ns, int ns2, int length, double missingvalue, double *centres)
{
int i, j, indcount;
double sum, mean;
...
if(centres!=NULL){centres[j]=mean;}
...
//type=4 - fill Yadj with Y - probabilities
{
...
double *Zthetas, *probs, *probs2, *Z2, *AI, *AI2, *AI3, *BI, *thetadiffs, *Ks, *thresh, *regs, *thetas2;
...
{
for(i=0;i<ns;i++){probs2[i]=pow(1+exp(-Zthetas[i]-offsets[i]),-1);}
}

for(i=0;i<ns;i++)
{
if(probs2[i]==0){probs2[i]=1e-10;}
if(probs2[i]==1){probs2[i]=1-1e-10;}
}
...
if(prev!=-9999)	//get thresholds, then maybe fill Yadj, get covarvar and topher 
{
//get Ks (population probs of being case given covariates), thresholds and regressions - probs was set above
value=prev*(1-ascer)/ascer/(1-prev);
for(i=0;i<ns;i++){Ks[i]=value*probs[i]/(1+value*probs[i]-probs[i]);thresh[i]=normal_inv(1-Ks[i]);}

if(Yadj!=NULL)
{
if(type==0)	//set Yadj to (Y-probs)/root(probs*(1-probs)), divided by regressions
{
for(i=0;i<ns;i++)
{
...
free(Zthetas);free(probs);free(probs2);free(Z2);free(AI);free(AI2);free(AI3);free(BI);free(thetadiffs);
if(prev!=-9999){free(Ks);free(thresh);free(regs);}
}	//end of reg_covar_log

////////

void reg_covar_log_missing(double *Y, double *Z, int ns, int num_fixed, double *offsets, double *thetas, double *thetasds, double *thetapvas, double *Yadj, int type, double tol, int maxiter, int flag)
//regress Y on Z logistically, then maybe fill Yadj
//type=2 - fill Yadj with probabilities, type=3 - fill Yadj with probabilities*(1-probabilities)
//type=4 - fill Yadj with Y - probabilities
//flag=0, use default starts; flag=1, use current thetas
{
int i, j, j2, count, indcount, one=1;
int *indexer;
double sum, relax;
double ascer, like, like2, likeold, alpha, beta;
double *Zthetas, *probs, *probs2, *AI, *AI2, *AI3, *BI, *thetadiffs;
...
//work out which individuals can be used
indcount=0;
for(i=0;i<ns;i++)
{
if(Y[i]==0||Y[i]==1){indexer[indcount]=i;indcount++;}
}

if(flag==0)	//set thetas to default starts
{
//get ascer
...
AI[j+j*num_fixed]=0;for(i=0;i<indcount;i++){AI[j+j*num_fixed]+=probs[indexer[i]]*(1-probs[indexer[i]])*pow(Z[indexer[i]+j*ns],2);}
for(j2=j+1;j2<num_fixed;j2++)
{
AI[j+j2*num_fixed]=0;for(i=0;i<indcount;i++){AI[j+j2*num_fixed]+=probs[indexer[i]]*(1-probs[indexer[i]])*Z[indexer[i]+j*ns]*Z[indexer[i]+j2*ns];}
...
free(indexer);free(Zthetas);free(probs);free(probs2);free(AI);free(AI2);free(AI3);free(BI);free(thetadiffs);
...
void reg_single_log(double *Y, double *Z, int ns, int num_fixed, double *offsets, double *stats, double *thetas2, double tol, int maxiter)
...
if(thetas2==0)	//use default starting values
{
...
double secant_trun(double obs, double sd, double thresh, double tol, int maxiter)
{
int count;
...
count++;
if(count==maxiter){printf("Failed to converge, values are %f %f and deriv is %f thre %f\n", obs, x1, f1, thresh);}
}	//end of while loop
