...
cpreds=malloc(sizeof(char*)*ncal);
}
...
//solve null model - get thetas and Yadj (and nullweights if dichot=1)
if(dichot==0)
{
...
{
reg_covar_log_missing(Y+m*num_samples_use, Z, num_samples_use, num_fixed, NULL, thetas+m*num_fixed, thetasds+m*num_fixed, thetapvas+m*num_fixed, nullprobs+m*num_samples_use, 2, 0.001, 100, 0);
...
for(i=0;i<num_samples_use;i++){Yadj[i+m*num_samples_use]=(Y[i+m*num_samples_use]-nullprobs[i+m*num_samples_use])/nullweights[i+m*num_samples_use];}
}
...
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename3);exit(1);}
if(dichot==0){fprintf(output3, "Component Effect SD P\n");}
else{fprintf(output3, "Component LogOR SD P\n");}
...
if(ncal>0)	//deal with calibration predictors
{
...
for(j=0;j<ncal;j++){copy_string(cpreds,j,preds[cindex[j]]);}

(void)read_data_fly(datafile, dtype, cdata, NULL, num_samples_use, keepsamps, 0, ncal, cindex2, datainputgz, -9999, num_samples, num_preds, genskip, genheaders, genprobs, bedbytes, missingvalue, -9999, -9999, nonsnp, maxthreads);
stand_data(cdata, ccentres, cmults, csqdevs, num_samples_use, ncal, missingvalue, -1, 0, 0, NULL, 1, cpreds);

count=0;
for(j=0;j<ncal;j++){count+=(cmults[j]==-9999);}
...
//set useped and forceridge assuming no structure
useped=0;
forceridge=0;

if(checkped==1)	//test for structure, and maybe make pedigree prs / perform fastGWA
{
#include "findrels.c"
}
...
if(fastgwa==0&&useped==0)	//continuing
{
//set and save parameters (will set num_pows and num_try, and allocate powers tryps and tryf2s)
#include "setparamsa.c"

//decide if using calibration predictors (only if loco and using ridge model)
if(loco==1&&(mode==151||forceridge==1)){usecal=1;}
else{usecal=0;}

if(skipcv==0)	//set num_small and num_full
{
if(her==-9999){num_small=num_try+num_hers2*(1+nmcmc);}
else{num_small=num_try;}

if(loco==0){num_full=1;}
else{num_full=1+num_chr+usecal*ncal;}
}
...
else{data_warn2(bitsize,num_samples_use+(num_samples_use+data_length)*num_resps_use);}

//estimate memory for rhe
if(her!=-9999){value=0;}
else{value=(double)num_samples_use/1024/1024/1024*8*(nmcmc+num_resps_use)*ndivs*num_pows;}

//estimate memory for vb
value2=(double)data_length/1024/1024/1024*8*(3*total+num_resps_use)+num_samples_use/1024/1024/1024*8*2*total;
...
{
data=malloc(sizeof(double)*num_samples_use*bitsize*num_resps_use);
data2=malloc(sizeof(double)*num_samples_use*bitsize);
...
if(num_fixed>1)
{
if(dichot==0){savethetas=malloc(sizeof(double)*num_fixed*data_length);}
else{savethetas=malloc(sizeof(double)*num_fixed*data_length*num_resps_use);}
}
...
bitactive=malloc(sizeof(int)*bittotal*total);
...
if(skipcv==0)	//sort out chrindex - the chromosome corresponding to each phenotype/calibration residual
{
chrindex[0]=-1;

if(loco==1)
{
//one residual for each chromosome
chrindex[1]=chr[0];
count=2;
...
if(usecal)	//now add calibration models
{
...
//note that both getindhersa and getindhersb fill centres and mults and compute savethetas
...
if(skipcv==0)	//chrprops is sum of exps not on target chromosome
{
for(m=0;m<num_resps_use;m++)
{
...
if(her==-9999)	//work out what heritabilities to test when performing MC REML
{
for(m=0;m<num_resps_use;m++)
{
if(hers[m]>0.01)
...
if(num_fixed>1){free(savethetas);}
if(skipcv==0){free(chrindex);free(chrprops);}
free(YTdata);
free(hers);free(hersold);free(exps);
if(her==-9999){free(tryhers);free(polates);}
free(lambdas);free(lambdas2);free(lambdas3);free(lambdas4);
free(pens);free(likes);free(likesold);
free(cgammas);free(csds);
if(fast==1){free(bitdo);free(bitactive);free(bitdet1);free(bitdet2);free(bitdiffs);free(bitpens);}
free(Mtops);free(Mbests);free(Mincs);free(Mscales);free(Mmses);free(Mneffs);
if(mpheno==-1&&skipcv==0){free(PT);free(PTP);free(PTP2);free(Q);free(Mcals);free(Mcombs);}

free(effs);free(effs2);free(probs);free(residuals);free(residuals2);free(changes);
}	//end of continuing
...
if(ncal>0)
{
free(cindex);free(cindex2);free(cdata);free(ccentres);free(cmults);free(csqdevs);
for(j=0;j<ncal;j++){free(cpreds[j]);}free(cpreds);
}
