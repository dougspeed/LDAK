...
void read_bimfile(char *bimfile, int *chr, char **preds, double *cm, double *bp, char *al1, char *al2, int length, int *keeppreds_use, int type, double window_cm, int flag, int multi)
...
{
if(multi==0)	//will ignore
{
if(type==1)
{
if(lcount<5){printf("Warning, Predictor %s has multi-character alleles (%s and %s) and will be ignored (to instead retain this predictor add \"--allow-multi YES\")\n", preds[found], ra1, ra2);}
}
bp[found]=-1;
}
else	//will rename
{
if(lcount<5){printf("Warning, Predictor %s has multi-character alleles (%s and %s); these will be replaced by \"X\" and \"Y\" respectively\n", preds[found], ra1, ra2);}
ra1[0]='X';ra2[0]='Y';
}
lcount++;
}
...
for(j=0;j<count;j++){free(gotpreds[j]);};free(gotpreds);
free(probsb);free(indexer);free(indexer2);free(usedpreds);
}	//end of read_probs
...
//read mean and variance then make sure at end of row
if(fscanf(input, "%[0-9eE.+-] %[0-9eE.+-]%c", rs, rs2, &readchar)!=3)
{
if(fscanf(input, "%s %s%c", rs, rs2, &readchar)!=2)
{printf("Error reading mean and variance on Row %d of %s\n\n", head+j+1, priorsfile);exit(1);}
if(strcmp(rs,"NA")==0||strcmp(rs2,"NA")==0)
{printf("Error reading %s; mean and/or variance on Row %d is NA\n\n", priorsfile,  head+j+1);exit(1);}
else
{printf("Error reading %s; mean and variance on Row %d is unrecognisable (%s)\n\n", priorsfile,  head+j+1, rs);exit(1);}
}
while(readchar!=10){readchar=10;(void)fscanf(input, "%c", &readchar);}

//load up
if(al1[j2]==ra1[0]){rmeans[j2]=atof(rs);}
else{rmeans[j2]=-atof(rs);}
rvars[j2]=atof(rs2);
...
free(rs);free(rs2);free(ra1);free(ra2);
}	//end of read_priors
...
void read_respfile(char *respfile, double *resp, int ns, char **ids3, int num_resps_use, int *keepresps, int num_resps, int *respcounts, int *respinds, double missingvalue, double binary, int pad, int type)
{
...
//check counts and variances, fill respinds, and possibly pad
count=0;
for(m=0;m<num_resps_use;m++)
{
...
respinds[i+m*ns]=(resp[i+m*ns]!=missingvalue);
}
...
void read_covarfile(char *covarfile, double *covar, int ns, char **ids3, int num_covars, double missingvalue)
{
...
indexer=malloc(sizeof(int)*count);
count2=find_strings(wantids, count, ids3, ns, NULL, indexer, offsetfile, NULL, NULL, NULL, 3);
if(count2==0){printf("Error, can not find any of these samples\n\n");exit(1);} 
...
void read_tops(char *datafile, double *covar, int ns, char **ids3, int num_tops, int *tkeeppreds, double *tcentres, double *tvars, char *famfile, int famhead, int dtype, int binary, int genskip, int genheaders, int genprobs, double *bedbytes, int num_preds, char **allpreds, double missingvalue, int nonsnp, char *sumsfile)
{
...
(void)read_data_fly(datafile, dtype, datatemp, NULL, count2, indexer, 0, num_tops, tkeeppreds, datainputgz, 0, count, num_preds, genskip, genheaders, genprobs, bedbytes, missingvalue, -9999, -9999, nonsnp,1);
...
if(length<num_preds)
{
if(count2==0){printf("Error, none of the %d predictors in %s are in the filtered data\n\n", count, filename);exit(1);}
if(count2<count){printf("Warning, only %d of the %d predictors in %s are in the filtered data\n", count2, count, filename);}
...
for(j=0;j<count;j++){free(wantpreds[j]);}free(wantpreds);
free(usedpreds);free(avalues);free(indexer);free(indexer2);
}
